...
int numLensFlareVerts = sizeof( lensFlareVerts )/ sizeof( Tpoint );
float lensAlphas[] =
{
	0.28f,
	0.37f,
...
void Drawable2D::Draw( class RenderOTW *renderer, int LOD )
{
	...
	*/
	...
	{
		if ( /*(typeData->flags & ALPHA_PER_TEXEL ) && */ !sGreenMode )
		{
		...
		else
		{
			if ( typeData->flags & USES_TREE_MATRIX )
				...
				renderer->context.RestoreState( STATE_ALPHA_TEXTURE_GOURAUD_TRANSPARENCY );
		}
	...
	else
	{
		...
		if ( typeData->flags & USES_TREE_MATRIX )
			...
			renderer->context.RestoreState( STATE_TEXTURE_TRANSPARENCY );
	}
	...
	// OW
	renderer->context.SelectTexture( curTex->TexHandle() );
...
void Drawable2D::SetupTexturesOnDevice(DXContext *rc)
{
	...
	for ( i = 0; i < NUM_TEX_SHEETS; i++ )
	{
		if ( i >= 0 && i < NUM_TEXTURES_USED || i >= 15 && i < 15 + NUM_TEXTURES_USED )
		{
			...
			{
				gGlobTextures[i].palette = gGlobTextures[0].palette;
			}
			else if ( i > 15 && i < 15 + NUM_TEXTURES_USED )
			{
				gGlobTextures[i].palette = gGlobTextures[15].palette;
			}
			...
			if ( i == 0 )
			{
				...
				for ( j = 0; j < 256; j++ )
				{
					intalp = (gGlobTextures[0].palette->paletteData[j] >> 24);
					gGlobTextures[0].palette->paletteData[j] &= 0x00ffffff;
					if ( j == 0 )
						continue;
					alp = 255.0f * 0.5f +  255.0f * 0.5f * NRANDPOS;
					intalp = FloatToInt32( alp );

					r = (gGlobTextures[0].palette->paletteData[j] & 0x000000ff);
					g = (gGlobTextures[0].palette->paletteData[j] & 0x0000ff00) >> 8;
					b = (gGlobTextures[0].palette->paletteData[j] & 0x00ff0000) >> 16;

					intalp = max( ( r + b + g )/3, 120 );
					gGlobTextures[0].palette->paletteData[j] |= ( intalp << 24 );
				}
				gGlobTextures[0].palette->UpdateMPR( gGlobTextures[0].palette->paletteData );
			}
	...
	*/
	...
	for ( j = 0; j < 256; j++ )
	{
		intalp = (gAplTextures[0].palette->paletteData[j] >> 24);
		gAplTextures[0].palette->paletteData[j] &= 0x00ffffff;
		if ( j == 0 )
			continue;
		alp = (float)intalp;
		alp = alp * 0.5f + alp * 0.5f * PRANDFloat();
		intalp = FloatToInt32( alp );
		gAplTextures[0].palette->paletteData[j] |= ( intalp << 24 );
	}
	gAplTextures[0].palette->UpdateMPR( gAplTextures[0].palette->paletteData );
	...
	for (i=1; i<NUM_APL_TEXTURES; i++)
	{
		...
		for ( j = 0; j < 256; j++ )
		{
			intalp = (gAplTextures[i].palette->paletteData[j] >> 24);
			gAplTextures[i].palette->paletteData[j] &= 0x00ffffff;
			if ( j == 0 )
				continue;
			alp = (float)intalp;
			if ( i < 6 )
			...
			intalp = FloatToInt32( alp );
			gAplTextures[i].palette->paletteData[j] |= ( intalp << 24 );
		}
		gAplTextures[i].palette->UpdateMPR( gAplTextures[i].palette->paletteData );
	...
	for (i=1; i<NUM_APL_TEXTURES; i++)
	{
		...
		gAplTexturesGreen[i].palette = gAplTexturesGreen[0].palette;
...
void Drawable2D::TimeUpdateCallback( void *)
{
	...
	gAplTextures[6].palette->LightTexturePalette( &light );
	gAplTextures[7].palette->LightTexturePalette( &light );
	gAplTextures[8].palette->LightTexturePalette( &light );
	gAplTextures[9].palette->LightTexturePalette( &light );
	gAplTextures[10].palette->LightTexturePalette( &light );
	gAplTexturesGreen[0].palette->LightTexturePalette( &light );
...
void Drawable2D::DrawGlowSphere( class RenderOTW *renderer, int)
{
	...
	// renderer->context.SetState( MPR_STA_SRC_BLEND_FUNCTION, MPR_BF_SRC_ALPHA );
	// renderer->context.SetState( MPR_STA_SRC_BLEND_FUNCTION, MPR_BF_ONE );
	// renderer->context.SetState( MPR_STA_DST_BLEND_FUNCTION, MPR_BF_ONE );
	// renderer->context.InvalidateState();
...
{
	Tpoint				position, center, os, lv, av;
	ThreeDVertex		v0, v1, v2, vLast, vLasti, v3;
	int					i,j;
	float				dotp;
	float				radius;
	float				alphaPct;
	...
	// reverse the direction of the light vector -- this is the direction
	// we'll walk the circles out in
	// lv.x = -lv.x;
	// lv.y = -lv.y;
	// lv.z = -lv.z;
	...
	curTex = &gGlobTextures[ 4 + sGreenMode ];
	...
	// renderer->context.SetState( MPR_STA_SRC_BLEND_FUNCTION, MPR_BF_SRC_ALPHA );
	// renderer->context.SetState( MPR_STA_SRC_BLEND_FUNCTION, MPR_BF_ONE );
	// renderer->context.SetState( MPR_STA_DST_BLEND_FUNCTION, MPR_BF_SRC_ALPHA_INV );
	// renderer->context.SetState( MPR_STA_DST_BLEND_FUNCTION, MPR_BF_ONE );
	// renderer->context.InvalidateState();
	...
	for ( j = 0; j < NUM_FLARE_CIRCLES; j++ )
	{
		...
		v2.g = lensRGB[j].g;
		v2.b = lensRGB[j].b;
		v0.r = lensCenterRGB[j].r;
		v0.g = lensCenterRGB[j].g;
		v0.b = lensCenterRGB[j].b;
		v3.r = lensCenterRGB[j].r;
		v3.g = lensCenterRGB[j].g;
		v3.b = lensCenterRGB[j].b;

		// set alphas for this circle
		v0.a = v1.a = v2.a = v3.a = lensAlphas[j]*alphaPct*lightLevel * 0.80f;
		if ( v0.a < 0.001 )
			continue;
		...
		if ( j == 2 || j == 3 || j == 5 || j == 7 )
		{
			...
			vLast.u = v1.u + TEX_UV_DIM;
			vLast.v = v1.v + TEX_UV_DIM;
			...
			os.y =  1.0f * radius;
			os.z =  1.0f * radius;
			renderer->TransformBillboardPoint( &os,  &center, &vLast );
			os.y = -1.0f * radius;
			os.z =  1.0f * radius;
			renderer->TransformBillboardPoint( &os,  &center, &v0 );

			renderer->context.SelectTexture( curTex->TexHandle() );
			renderer->context.RestoreState( STATE_ALPHA_TEXTURE_TRANSPARENCY );
	
			renderer->DrawTriangle( &v1, &v2, &vLast, CULL_ALLOW_ALL );
			renderer->DrawTriangle( &vLast, &v0, &v1, CULL_ALLOW_ALL );
			continue;
		}
		*/
		...
		// do 1st point
		os.x = lensFlareVerts[0].x * radius;
		...
		if ( j == 2 || j == 8 || j == 9 )
		{
			os.x = lensFlareVerts[0].x * radius * 0.80f;
			os.y = lensFlareVerts[0].y * radius * 0.80f;
			os.z = lensFlareVerts[0].z * radius * 0.80f;
			renderer->TransformBillboardPoint( &os,  &center, &v0 );
			...
			{
				os.x = lensFlareVerts[i].x * radius;
				os.y = lensFlareVerts[i].y * radius;
				os.z = lensFlareVerts[i].z * radius;
				renderer->TransformBillboardPoint( &os,  &center, &v2 );

				os.x = lensFlareVerts[i].x * radius * 0.80f;
				os.y = lensFlareVerts[i].y * radius * 0.80f;
				os.z = lensFlareVerts[i].z * radius * 0.80f;
				renderer->TransformBillboardPoint( &os,  &center, &v3 );
...
void Drawable2D::DrawTexturedCone( class RenderOTW *renderer, int)
{
	...
	/*
	if ( renderer->GetAlphaMode() )
		renderer->context.RestoreState( STATE_ALPHA_TEXTURE_TRANSPARENCY_PERSPECTIVE );
	else
		renderer->context.RestoreState( STATE_TEXTURE_TRANSPARENCY_PERSPECTIVE );
	*/
...
void Drawable2D::DrawTexturedCone( class RenderOTW *renderer, int LOD )
{
	Tpoint				rpos, os;
	ThreeDVertex		v0, v1, v2, v3;
	UV					uvStart;
	...
	v1.u = uvStart.u + TEX_UV_DIM;
	v1.v = uvStart.v;
	v2.u = uvStart.u + TEX_UV_DIM;
	v2.v = uvStart.v + TEX_UV_DIM;
	v3.u = uvStart.u;
	v3.v = uvStart.v + TEX_UV_DIM;

	// now we do 8 points, 2 rects

	os.x = cruxVerts[0].x * scale2d * radius;
	os.y = cruxVerts[0].y * scale2d * radius;
	os.z = cruxVerts[0].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	// transform the point
	renderer->TransformPoint( &rpos,  &v0 );
	v0.q = v0.z * 0.001f;

	os.x = cruxVerts[1].x * scale2d * radius;
	os.y = cruxVerts[1].y * scale2d * radius;
	os.z = cruxVerts[1].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	// transform the point
	renderer->TransformPoint( &rpos,  &v1 );
	v1.q = v1.z * 0.001f;

	os.x = cruxVerts[2].x * scale2d * radius;
	os.y = cruxVerts[2].y * scale2d * radius;
	os.z = cruxVerts[2].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	// transform the point
	renderer->TransformPoint( &rpos,  &v2 );
	v2.q = v2.z * 0.001f;

	os.x = cruxVerts[3].x * scale2d * radius;
	os.y = cruxVerts[3].y * scale2d * radius;
	os.z = cruxVerts[3].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	// transform the point
	renderer->TransformPoint( &rpos,  &v3 );
	v3.q = v3.z * 0.001f;

	renderer->DrawSquare( &v0, &v1, &v2, &v3, CULL_ALLOW_ALL );

	os.x = cruxVerts[4].x * scale2d * radius;
	os.y = cruxVerts[4].y * scale2d * radius;
	os.z = cruxVerts[4].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	...
	os.x = cruxVerts[5].x * scale2d * radius;
	os.y = cruxVerts[5].y * scale2d * radius;
	os.z = cruxVerts[5].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	...
	os.x = cruxVerts[6].x * scale2d * radius;
	os.y = cruxVerts[6].y * scale2d * radius;
	os.z = cruxVerts[6].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
	...
	os.x = cruxVerts[7].x * scale2d * radius;
	os.y = cruxVerts[7].y * scale2d * radius;
	os.z = cruxVerts[7].z * scale2d * radius;
	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
	rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
	rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
...
void Drawable2D::DrawTexturedCone( class RenderOTW *renderer, int LOD )
{
	...
	uvStart.u = gTexUV[ typeData->startTexture + curFrame ].u;
	uvStart.v = gTexUV[ typeData->startTexture + curFrame ].v;
	v0.u = uvStart.u + TEX_UV_DIM/2;
	v0.v = uvStart.v + TEX_UV_DIM/2;
	v0.q = v0.z * 0.001f;
	...
	if ( typeData->flags & GLOW_RAND_POINTS )
	{
		...
		randradius = radius + radius * PRANDFloat();

		os.x = coneVerts[0].x * scale2d * randradius;
		os.y = coneVerts[0].y * scale2d * randradius;
		os.z = coneVerts[0].z * scale2d * randradius;
	}
	else
	{
		os.x = coneVerts[0].x * scale2d * radius;
		os.y = coneVerts[0].y * scale2d * radius;
		os.z = coneVerts[0].z * scale2d * radius;
	}

	// rotate the object space point using the orientation matrix
	rpos.x = orientation.M11 * os.x + orientation.M12 * os.y + orientation.M13 * os.z + position.x;
	rpos.y = orientation.M21 * os.x + orientation.M22 * os.y + orientation.M23 * os.z + position.y;
	rpos.z = orientation.M31 * os.x + orientation.M32 * os.y + orientation.M33 * os.z + position.z;

	renderer->TransformPoint( &rpos, &v1 );
	v1.u = uvStart.u + coneUVs[0].u * TEX_UV_DIM;
	v1.v = uvStart.v + coneUVs[0].v * TEX_UV_DIM;
	v1.q = v1.z * 0.001f;

	// save it for last tri in strip
	vLast = v1;

	for ( i = 1; i < numConeVerts; i++ )
	{
		// get 3rd point of triangle
		if ( !(i & 1) && (typeData->flags & GLOW_RAND_POINTS) )
		{
			...
			os.x = coneVerts[i].x * scale2d * randradius;
			os.y = coneVerts[i].y * scale2d * randradius;
			os.z = coneVerts[i].z * scale2d * randradius;
		}
		else
		{
			os.x = coneVerts[i].x * scale2d * radius;
			os.y = coneVerts[i].y * scale2d * radius;
			os.z = coneVerts[i].z * scale2d * radius;
		}
		// rotate the object space point using the orientation matrix
		rpos.x = orientation.M11 * os.x + orientation.M21 * os.y + orientation.M31 * os.z + position.x;
		rpos.y = orientation.M12 * os.x + orientation.M22 * os.y + orientation.M32 * os.z + position.y;
		rpos.z = orientation.M13 * os.x + orientation.M23 * os.y + orientation.M33 * os.z + position.z;
		renderer->TransformPoint( &rpos, &v2 );
		v2.u = uvStart.u + coneUVs[i].u * TEX_UV_DIM;
		v2.v = uvStart.v + coneUVs[i].v * TEX_UV_DIM;
		v2.q = v2.z * 0.001f;
