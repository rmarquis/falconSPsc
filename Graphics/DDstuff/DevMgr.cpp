...
{
	if(driverNum < 0 || driverNum >= m_arrDDDrivers.size())
		return NULL;
...
{
	for(int i=0;i<m_arrDDDrivers.size();i++)
		...
		LPSTR lpDeviceName, LPD3DDEVICEDESC7 lpD3DHWDeviceDesc, LPVOID lpContext)
{
	...
	if(lpD3DHWDeviceDesc) pThis->m_arrD3DDevices.push_back(D3DDeviceInfo(*lpD3DHWDeviceDesc, lpDeviceName, lpDeviceDescription));
...
{
	for(int i=0;i<m_arrModes.size();i++)
	{
...
{
	for(int i=0;i<m_arrD3DDevices.size();i++)
...
*/
		...
		else if(IsEqualIID(m_guidD3D, IID_IDirect3DHALDevice))
...
bool DXContext::SetRenderTarget(IDirectDrawSurface7 *pRenderTarget)
{
	try
	{
		if(!m_pD3DD)
		{
			...
			/* OW FIXME: remove comment when HW TnL support gets enabled
			// Setup and attach the depth Buffer if necessary
			...
			*/
			...
			// OW FIXME: remove comment when HW TnL support gets enabled
			// CheckHR(m_pD3DD->SetRenderState(D3DRENDERSTATE_ZENABLE, TRUE));
...
//			CheckCaps();
		...
		{
			// OW: hack compares pointers which can produce wrong results (but this cant be avoided if we dont want to loose a whole lot FPS)
			IDirectDrawSurface7Ptr pDDS;
			if(FAILED(m_pD3DD->GetRenderTarget(&pDDS)) || pDDS.GetInterfacePtr() != pRenderTarget)
			{
				// Assign an existing depth buffer to the new render target
				...
				if(pRenderTargetOld)
				{
					...
					if(pDDSZB)
					{
						...
						CheckHR(pRenderTarget->AddAttachedSurface(pDDSZB));
					}
				}
...
{
	// OW FIXME: remove when HW TnL support gets enabled
	return;
