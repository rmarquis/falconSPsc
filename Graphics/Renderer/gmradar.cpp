...
\***************************************************************************/
void RenderGMRadar::StartFrame( void ) {
	Render2D::StartFrame();
}
...
void RenderGMRadar::DrawBlip( DrawableObject* drawable )
{
	float			x,  y;
	float			dx, dy;
	float			r;
...
float RenderGMRadar::ComputeReflectedIntensity( Tpost *post )
{
	...
	cosAngle = (float)sin(lightPhi)*(float)sin(post->phi) + 
			   (float)cos(lightPhi)*(float)cos(post->phi)*(float)cos(lightTheta-post->theta);
