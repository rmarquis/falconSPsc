...
void RenderGMComposite::Setup( ImageBuffer *output, void(*tgtDrawCallback)(void*,RenderGMRadar*), void *tgtDrawParam )
{
...
void RenderGMComposite::SetBeam( Tpoint *from, Tpoint *at, Tpoint *center, float platformHdg, float beamAngle, int beamPercent, float cursorAngle, BOOL movingRight )
{
	float		Px, Py;
	float		dx, dy;
	float		hdg;
	...
	bool bRender = BackgroundGeneration( from, at, platformHdg, beamPercent, movingRight );
...
void RenderGMComposite::DrawComposite( Tpoint *center, float platformHdg )
{
	...
	if (num >= 3) {
		...
		context.SelectTexture( (UInt) rTexHandle );
	...
	if (num >= 3) {
		...
		context.RestoreState( STATE_TEXTURE );
		context.SelectTexture( (UInt) lTexHandle );
		ClipAndDraw2DFan( vertArray, num );
	}
...
bool RenderGMComposite::BackgroundGeneration( Tpoint *from, Tpoint *at, float platformHdg, int beamPercent, BOOL movingRight )
{
	OpName	operation;
	float	dx, dy;
	Tpoint	center;
	...
	if ((prevBeamRight != movingRight) || (beamPercent < prevBeamPercent)) {
		BackgroundGeneration( from, at, platformHdg, 101, prevBeamRight );
	...
	while (beamPercent >= nextOperation->percent)
	{
		...
		switch( operation ) {
		  ...
		  case Targets:
			...
			tgtDrawCB( tgtDrawCBparam, &radar );
			radar.FinishFrame();
			break;
		  case Finish:
			radar.StartFrame();
		  ...
		  case Replace:
			...
			NewImage( &center, radar.GetHdg(), movingRight );
			nextOperation = &OperationList[0];
			beamPercent = 0;
			break;
		  default:
...
void RenderGMComposite::NewImage( Tpoint *at, float platformHdg, BOOL replaceRight )
{
...
//	nTexHandle->PreLoad();	// it will be used soon
	...
	// and action
	radar.context.RestoreState(STATE_ALPHA_TEXTURE_TRANSPARENCY);
	radar.context.SetState(MPR_STA_DST_BLEND_FUNCTION, MPR_BF_ONE);
	//MI TEST
	radar.context.SelectTexture((GLint) nTexHandle);
	...
	ImageBuffer *pSrcBuffer = PlayerOptions.bFastGMRadar ? m_pRenderTarget : m_pRenderBuffer;
...
void RenderGMComposite::DebugDrawLeftTexture( Render2D *renderer )
{
	...
	renderer->context.SelectTexture( (UInt) lTexHandle );
