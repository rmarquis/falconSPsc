...
// OW - constants made match the D3D 7 constants
enum
{
	MPR_BF_ZERO=1,
	MPR_BF_ONE,
...
class DXContext
{
	...
	DXContext();
	~DXContext();
	// DXContext& operator=(DXContext &ref);
	...
	struct _DDCAPS_DX7 *m_pcapsDD;	// HAL caps
	...
	int AddRef() { return ++refcount; }; // JPO - use simple reference counting
	int Release() { int rc = --refcount; if (refcount <= 0) delete this; return rc;};
...
class TextureHandle
{
	...
	struct TEXTURESEARCHINFO
	{
		...
		struct _DDPIXELFORMAT* pddpf; // Output of texture format search
	};

	static _DDPIXELFORMAT m_arrPF[TEX_CAT_MAX];

	static IDirect3DDevice7 *m_pD3DD;		// Warning: Not addref'd
	...
	// Implementation
	public:
	bool Create(char *strName, UInt16 info, UInt16 bits, UInt16 width, UInt16 height, DWORD dwFlags = 0);
