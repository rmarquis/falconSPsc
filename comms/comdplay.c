...
int					ComDPLAYHostIDGet(ComAPIHandle c, char *buf);
...
//		int			iResult = 0;
		...
		c->recv_buffer						= malloc(buffersize + 1);
		c->send_buffer						= malloc(buffersize + 1);
...
int     ComDPLAYHostIDGet(ComAPIHandle comhandle, char *buf)
{
    ComDPLAY *c = (ComDPLAY *)comhandle;
...
static void  ComSetSerialData(LPDPCOMPORTADDRESS ComPortAddress,char *datastring)
{
      ...
      /* determine Com Port */
      string = malloc(strlen(datastring) + 1);
