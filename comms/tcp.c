...
int           ComTCPSend(ComAPIHandle c, int msgsize, int oob);
int           ComTCPSendX(ComAPIHandle c, int msgsize,ComAPIHandle Xcom);
...
int ComTCPSendX(ComAPIHandle c, int msgsize,ComAPIHandle Xcom)
{
   ...
   {
	   return ComTCPSend (c, msgsize, FALSE);
   }
}
...
static int Posupdbwused = 0;
	...
	if(c)
	{
		...
		if (check_bandwidth (ctcp->send_buffer.len))
		{
...
int  ComGROUPAddress(ComAPIHandle c, char *buf)
{
...
ComAPIHandle CAPIIsInGroup (ComAPIHandle grouphandle, unsigned long ipAddress)
{
	...
	for( curr = group->GroupHead; curr; curr = curr -> next ) 
	{
		...
		else if(curr->com->protocol == CAPI_UDP_PROTOCOL)
		{
			...
			c = (ComIP *)curr->com;
			cip = CAPI_ntohl(c->address.sin_addr.s_addr);
...
void ComAPISetName (ComAPIHandle c, char *name_in)
{
	...
	c->name = malloc (strlen (name_in) + 1);
...
#ifdef _DEBUG
	MonoPrint ("ComAPIAddToGroup GH:\"%s\" CH:\"%s\"\n", grouphandle->name, memberhandle->name);
#endif
...
//	MonoPrint ("Add To Group CH:\"%s\"\n", memberhandle->name);
	...
	group->buffer_size = min(group->buffer_size,(int)memberhandle->query_func(memberhandle,COMAPI_ACTUAL_BUFFER_SIZE)+group->max_header);
	
	/* If Group HostID is NULL, take HostID from the first added member*/
	if(group->HostID == 0)
	{
		memberhandle->addr_func(memberhandle,(char *)&group->HostID);
	}
...
int ComGROUPSend(ComAPIHandle c, int msgsize, int oob)
{
	int senderror=0;
	int bytesSent=0;
...
//		// Check bandwidth for UDP Group Sends
//
//		if ((group->GroupHead) && (group->GroupHead->com->protocol == CAPI_UDP_PROTOCOL) && (!oob))
//		{
//			for (curr = group->GroupHead; curr; curr = curr -> next)
//			{
//				count ++;
//			}
//
//			if (!check_bandwidth (count * msgsize))
//			{
//				MonoPrint ("UDP Blocked %d %d\n", count, msgsize);
//				return COMAPI_WOULDBLOCK;
//			}
//		}
		...
		for( curr = group->GroupHead; curr; curr = curr -> next ) 
		{
		    assert(FALSE==IsBadReadPtr(curr, sizeof *curr)); // JPO
		    assert(FALSE==IsBadReadPtr(curr->com, sizeof *curr->com)); // JPO

			if (IsBadReadPtr(curr->com, sizeof(ComAPIHandle))) // JB 010221 CTD
			...
			if(curr->com->protocol == CAPI_TCP_PROTOCOL)
			{
				...
				assert(FALSE==IsBadReadPtr(this_ctcp, sizeof *this_ctcp)); // JPO
				...
				if (this_ctcp && !IsBadCodePtr((FARPROC) (*curr->com->send_func))) // JB 010401 CTD
				{
					...
					{
						ret = curr->com->send_func(curr->com, msgsize, oob);
					}
			...
			else if(curr->com->protocol == CAPI_UDP_PROTOCOL)
			{
				...
				assert(FALSE==IsBadReadPtr(this_cudp, sizeof *this_cudp)); // JPO

				//if(this_cudp) // JB 010222 CTD
				if (this_cudp  && (this_cudp->send_buffer.buf) && 
					!IsBadReadPtr(this_cudp->send_buffer.buf, sizeof(char)) && // JB 010222 CTD
					!IsBadCodePtr((FARPROC) (*curr->com->send_func))) // JB 010401 CTD
				{
...
//							oob = TRUE; // force other UDP packets to go OOB if the first one went
			...
			else if(curr->com->protocol == CAPI_GROUP_PROTOCOL)  /* another group */
			{
				...
				assert(FALSE==IsBadReadPtr(this_group, sizeof *this_group)); // JPO
				//if(this_group)
				if(this_group && !IsBadCodePtr((FARPROC) (*curr->com->send_func))) // JB 010401 CTD
				{
...
int ComGROUPSendX(ComAPIHandle c, int msgsize, ComAPIHandle Xcom)
{
	int senderror=0;
	int bytesSent=0;
...
ComAPIHandle  ComAPICreateGroup(char *name_in, int BufferSize, ... )
{
	...
	GlobalGroupListHead->com->name = malloc (strlen (name_in) + 1);
