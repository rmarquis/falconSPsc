...
void AddHorizontalLineToBrief (C_Window *window)
	...
	if (window)
		...
		short		CBXtemp = 0;
		AddHorizontalLineToWindow (window ,&CBXtemp, &CBYtemp, 0, window->ClientArea_[0].right - window->ClientArea_[0].left - 10, CBColor);
		}
	}
...
void AddStringToBrief (_TCHAR *buffer, C_Window *window, _TCHAR *output)
	...
	if (!buffer[0])
		return;
	if (window)
		AddWordWrapTextToWindow(window, &CBX, &CBY, CLineStart, window->ClientArea_[0].right - window->ClientArea_[0].left - 10, CBColor, buffer);
	else
		{
...
void AddTabToBrief (int tab, _TCHAR *buffer, C_Window *window, _TCHAR *output)
{
	...
	do {
	    _TCHAR		space[2] = " ";
	    AddStringToBrief(space, window, output);
	} while (CBX < tab-1);
    }
}
...
*/
				...
				else if (strcmp(token,"#IF_ECM")==0)
				{
					...
					{
					int		cc =	(((WeatherClass*)TheWeather)->GetCloudCover(mec->tx,mec->ty) +
									((WeatherClass*)TheWeather)->GetCloudCover(mec->tx-1,mec->ty) +
									((WeatherClass*)TheWeather)->GetCloudCover(mec->tx,mec->ty-1) +
									((WeatherClass*)TheWeather)->GetCloudCover(mec->tx+1,mec->ty) +
									((WeatherClass*)TheWeather)->GetCloudCover(mec->tx,mec->ty+1))/5;
					if (cc < 2)
						stack_active[curr_stack] = 1;
					else
						stack_active[curr_stack] = 0;
...
// 2002-02-13 MN added AWACSAbort
				if (success != Incomplete /*&& success != AWACSAbort*/)
					...
					if (!tar	|| (tar->IsUnit() && (((Unit)tar)->Broken() || ((Unit)tar)->Dead())) 
...
//  			AddNumberToBuffer(FloatToInt32(((WeatherClass*)TheWeather)->WindHeading*RTD),current_line);
			float reversedWindHeading = ((WeatherClass*)TheWeather)->WindHeading*RTD + 180.0F;
...
//end fix
				...
				//AddNumberToBuffer(FloatToInt32(((WeatherClass*)TheWeather)->WindSpeed),current_line);
				//MI fix to show Knots/H instead of KM/H
				AddNumberToBuffer(FloatToInt32((((WeatherClass*)TheWeather)->WindSpeed + 0.5F) * 
				KPH_TO_FPS * FTPSEC_TO_KNOTS),current_line);
			else if (strcmp(token,"TEMPERATURE")==0)
				AddNumberToBuffer(FloatToInt32(((WeatherClass*)TheWeather)->Temp),current_line);
				...
				AddNumberToBuffer((((WeatherClass*)TheWeather)->GetCloudLevel(mec->tx,mec->ty)/10),current_line);
			else if (strcmp(token,"CON_LAYER")==0)
				AddNumberToBuffer(((WeatherClass*)TheWeather)->TodaysConLow,current_line);
