...
void AdjustForceRatios (void)
	...
	for (t=0; t<NUM_TEAMS; t++)
		...
		if (t == pteam)
			...
			{
			TeamInfo[t]->max_vehicle[RCLASS_AIR]		= VEHICLES_PER_UNIT;
			TeamInfo[t]->max_vehicle[RCLASS_GROUND]		= VEHICLES_PER_UNIT;
			TeamInfo[t]->max_vehicle[RCLASS_AIRDEFENSE]	= VEHICLES_PER_UNIT;
			TeamInfo[t]->max_vehicle[RCLASS_NAVAL]		= VEHICLES_PER_UNIT;
			}
		}
	...
	while (u)
		...
		if (!u->IsFlight())
			...
			for (int slot=0; slot<VEHICLES_PER_UNIT; slot++)
...
void ChopCompany (Unit u, int crating)
	...
	for (slot = max_veh[crating]; slot < VEHICLES_PER_UNIT; slot++)
