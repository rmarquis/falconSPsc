...
//					if (GetCover(rx,ry) != Water || GetCover(rx+PAK_MAP_RATIO-1,ry) != Water || GetCover(rx,ry+PAK_MAP_RATIO-1) != Water)
					if (own)
						{
						float	last = -1.0F;
		...
		default:
			for (x=0; x<MRX; x++)
				...
				for (y=0; y<MRY; y++)
					...
					rx = x*MAP_RATIO;
					ry = y*MAP_RATIO;
					if (GetCover(rx,ry) != Water || GetCover(rx+MAP_RATIO-1,ry) != Water || GetCover(rx,ry+MAP_RATIO-1) != Water)
						{
