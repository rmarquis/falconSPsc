...
TeamClass::TeamClass (int typeindex, Control owner) : FalconEntity(typeindex)
	{
	_TCHAR	towner[41];
	short		tid;
...
TeamClass::TeamClass (VU_BYTE **stream) : FalconEntity(VU_LAST_ENTITY_TYPE)
	{
	VU_ID				tmpID;
	...
	memcpy(&who, *stream, sizeof(Team));								*stream += sizeof(Team);
	ShiAssert (TeamInfo[who] && TeamInfo[who]->share_.id_ == share_.id_)
	
	memcpy(&cteam, *stream, sizeof(Team));								*stream += sizeof(Team);
	ShiAssert((cteam > 0 || who == 0) && cteam < NUM_TEAMS);
	memcpy(&flags, *stream, sizeof(short));								*stream += sizeof(short);
	memcpy(member, *stream, sizeof(uchar)*NUM_COUNS);					*stream += sizeof(uchar)*NUM_COUNS;
	memcpy(stance, *stream, sizeof(short)*NUM_TEAMS);					*stream += sizeof(short)*NUM_TEAMS;
	memcpy(&firstColonel, *stream, sizeof(short));						*stream += sizeof(short);
	memcpy(&firstCommander, *stream, sizeof(short));					*stream += sizeof(short);
	memcpy(&firstWingman, *stream, sizeof(short));						*stream += sizeof(short);
	memcpy(&lastWingman, *stream, sizeof(short));						*stream += sizeof(short);

	memcpy(&airExperience, *stream, sizeof(uchar));						*stream += sizeof(uchar);
	memcpy(&airDefenseExperience, *stream, sizeof(uchar));				*stream += sizeof(uchar);
	memcpy(&groundExperience, *stream, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(&navalExperience, *stream, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(&initiative, *stream, sizeof(short));						*stream += sizeof(short);
	memcpy(&supplyAvail, *stream, sizeof(ushort));						*stream += sizeof(ushort);
	memcpy(&fuelAvail, *stream, sizeof(ushort));						*stream += sizeof(ushort);
	memcpy(&replacementsAvail, *stream, sizeof(ushort));				*stream += sizeof(ushort);
	memcpy(&playerRating, *stream, sizeof(float));						*stream += sizeof(float);
	memcpy(&lastPlayerMission, *stream, sizeof(CampaignTime));			*stream += sizeof(CampaignTime);
	memcpy(&currentStats, *stream, sizeof(TeamStatusType));				*stream += sizeof(TeamStatusType);
	memcpy(&startStats, *stream, sizeof(TeamStatusType));				*stream += sizeof(TeamStatusType);
	memcpy(&reinforcement, *stream, sizeof(short));						*stream += sizeof(short);
	memcpy(bonusObjs, *stream, sizeof(VU_ID)*MAX_BONUSES);				*stream += sizeof(VU_ID)*MAX_BONUSES;
	memcpy(bonusTime, *stream, sizeof(CampaignTime)*MAX_BONUSES);		*stream += sizeof(CampaignTime)*MAX_BONUSES;
	memcpy(objtype_priority, *stream, sizeof(uchar)*MAX_TGTTYPE);		*stream += sizeof(uchar)*MAX_TGTTYPE;
	memcpy(unittype_priority, *stream, sizeof(uchar)*MAX_UNITTYPE);		*stream += sizeof(uchar)*MAX_UNITTYPE;
	memcpy(mission_priority, *stream, sizeof(uchar)*AMIS_OTHER);		*stream += sizeof(uchar)*AMIS_OTHER;
	memcpy(max_vehicle, *stream, sizeof(uchar)*4);						*stream += sizeof(uchar)*4;
	memcpy(&teamFlag, *stream, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(&teamColor, *stream, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(&equipment, *stream, sizeof(uchar));							*stream += sizeof(uchar);
	memcpy(name, *stream, sizeof(_TCHAR)*MAX_TEAM_NAME_SIZE);			*stream += sizeof(_TCHAR)*MAX_TEAM_NAME_SIZE;
	memcpy(teamMotto, *stream, sizeof(_TCHAR)*MAX_MOTTO_SIZE);			*stream += sizeof(_TCHAR)*MAX_MOTTO_SIZE;
	memcpy(&groundAction, *stream, sizeof(TeamGndActionType));			*stream += sizeof(TeamGndActionType);
	memcpy(&defensiveAirAction, *stream, sizeof(TeamAirActionType));	*stream += sizeof(TeamAirActionType);
	memcpy(&offensiveAirAction, *stream, sizeof(TeamAirActionType));	*stream += sizeof(TeamAirActionType);
	...
	memcpy(&tmpID, *stream, sizeof(VU_ID));								*stream += sizeof(VU_ID);
	memcpy(&tmpID, *stream, sizeof(VU_ID));								*stream += sizeof(VU_ID);
	memcpy(&tmpID, *stream, sizeof(VU_ID));								*stream += sizeof(VU_ID);
...
TeamClass::TeamClass (FILE *file) : FalconEntity(VU_LAST_ENTITY_TYPE)
	{
	// Read vu stuff here
...
//#endif
	...
	if (gRenameIds)
		...
		// Rename this ID
		for (new_id.num_ = LAST_NON_VOLITILE_VU_ID_NUMBER-1; new_id.num_ > FIRST_NON_VOLITILE_VU_ID_NUMBER ; new_id.num_--)
			{
...
int TeamClass::Handle(VuFullUpdateEvent *event)
	{
	TeamClass*	tmpTeam = (TeamClass*)(event->expandedData_);
	int			retval;
...
void NewInitiativePointSetting(Team who) {
	...
	oloss = (os_start*1.0 - os*1.0);
	if (oloss==0)
		oloss = 1;
	tloss = (ts_start*1.0 - ts*1.0);
	if ( tloss==0)
		tloss = 1;
	...
	while (u)
		...
		if (u->GetTeam() == who && !u->Cargo() && u->GetUnitReinforcementLevel() <= TeamInfo[who]->GetReinforcement() && u->Parent())
			...
			while (e)
				{
				e->GetLocation(&x,&y);
				o = FindNearestObjective(x,y,NULL,100);
				if (!o || o->GetTeam() == who)
					{
...
// Fix by Alfred, ApplyPlayerInput is called every hour!
	...
	if (TheCampaign.CurrentTime - TeamInfo[FalconLocalSession->GetTeam()]->lastPlayerMission > g_nNoPlayerPlay*CampaignHours)
...
void TeamClass::ReadDirty (unsigned char **stream)
{
	uchar
		*ptr;
	...
	//MonoPrint ("  TM %08x", bits);

	if (bits & DIRTY_MISSION_PRIORITY)
	{
		memcpy (mission_priority, ptr, sizeof (mission_priority));
		ptr += sizeof (mission_priority);
	}

	if (bits & DIRTY_UNITTYPE_PRIORITY)
	{
		memcpy (unittype_priority, ptr, sizeof (unittype_priority));
		ptr += sizeof (unittype_priority);
	}

	if (bits & DIRTY_OBJTYPE_PRIORITY)
	{
		memcpy (objtype_priority, ptr, sizeof (objtype_priority));
		ptr += sizeof (objtype_priority);
	}

	if (bits & DIRTY_SUPPLY_FUEL_AVAIL)
	{
		supplyAvail = *(ushort*)ptr;
		ptr += sizeof (ushort);

		fuelAvail = *(ushort*)ptr;
		ptr += sizeof (ushort);

		replacementsAvail = *(ushort*)ptr;
		ptr += sizeof (ushort);
	}

	if (bits & DIRTY_TEAM_INITIATIVE)
	{
		initiative = *(short*)ptr;
		ptr += sizeof (short);
	}
	...
	{
		memcpy (&currentStats, ptr, sizeof (currentStats));
		ptr += sizeof (currentStats);
	}

	if (bits & DIRTY_GROUND_ACTION)
	{
		memcpy (&groundAction, ptr, sizeof (groundAction));
		ptr += sizeof (groundAction);
	}

	if (bits & DIRTY_OFFAIR_ACTION)
	{
		memcpy (&offensiveAirAction, ptr, sizeof (offensiveAirAction));
		ptr += sizeof (offensiveAirAction);
	}

	if (bits & DIRTY_DEFAIR_ACTION)
	{
		memcpy (&defensiveAirAction, ptr, sizeof (defensiveAirAction));
		ptr += sizeof (defensiveAirAction);
	}

	if (bits & DIRTY_TEAM_RELATIONS)
	{
		memcpy (&cteam, ptr, sizeof (uchar));
		ShiAssert(cteam > 0 && cteam < NUM_TEAMS);
		ptr += sizeof (uchar);
		memcpy (member, ptr, sizeof (uchar) * NUM_COUNS);
		ptr += sizeof (uchar) * NUM_COUNS;
		memcpy (stance, ptr, sizeof (short) * NUM_TEAMS);
		ptr += sizeof (short) * NUM_TEAMS;
