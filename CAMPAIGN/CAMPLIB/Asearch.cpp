...
BasePathClass::BasePathClass (uchar **stream)
	{
	memcpy(&max_length,*stream,sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(&length,*stream,sizeof(uchar));						*stream += sizeof(uchar);
	memcpy(&current_location,*stream,sizeof(uchar));			*stream += sizeof(uchar);
	memcpy(path,*stream,sizeof(uchar)*(length+1)/PATH_DIV);		*stream += sizeof(uchar)*(length+1)/PATH_DIV;
