...
float Distance (GridIndex ox, GridIndex oy, GridIndex dx, GridIndex dy)
	{
	return (float)sqrt((double)((ox-dx)*(ox-dx) + (oy-dy)*(oy-dy))) * GRID_SIZE_KM;
	}
...
Unit FindNearestEnemyUnit (GridIndex X, GridIndex Y, GridIndex max)
	{
	...
	VuGridIterator	myit(RealUnitProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(max_dist));
#else
	VuGridIterator	myit(RealUnitProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(max_dist));
#endif
...
Unit FindNearestRealUnit (GridIndex X, GridIndex Y, float *last, GridIndex max)
	{
	GridIndex	max_dist = MAX_GROUND_SEARCH;
...
Objective FindNearestObjective (GridIndex X, GridIndex Y, float *last, GridIndex maxdist)
	{
#ifdef VU_GRID_TREE_Y_MAJOR
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(maxdist));
#else
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(maxdist));
#endif
...
{
#ifdef VU_GRID_TREE_Y_MAJOR
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(5));
#else
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(5));
#endif
	int         d,nd;
	Objective   o,n=NULL;
	GridIndex   x,y;
...
Objective FindNearestFriendlyAirbase (Team who, GridIndex X, GridIndex Y)
{
	...
	while (o != NULL)
	{
		//if ( (GetTTRelations(o->GetTeam(),who) <= Neutral) && o->GetType () == TYPE_AIRBASE && o->brain->NumOperableRunways())
		if ( (GetTTRelations(o->GetTeam(),who) <= Neutral) && o->GetType () == TYPE_AIRBASE && o->brain && o->brain->NumOperableRunways()) // JB 010729 CTD
		{
			o->GetLocation (&x, &y);
...
Objective FindNearestFriendlyObjective(Team who, GridIndex *x, GridIndex *y, int flags)
	...
	Int32		d,bd=999;
#ifdef VU_GRID_TREE_Y_MAJOR
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(*x),(BIG_SCALAR)GridToSim(*y),(BIG_SCALAR)GridToSim(MAX_GROUND_SEARCH));
#else
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(*y),(BIG_SCALAR)GridToSim(*x),(BIG_SCALAR)GridToSim(MAX_GROUND_SEARCH));
#endif
...
Objective GetObjectiveByXY (GridIndex X, GridIndex Y)
	{
#ifdef VU_GRID_TREE_Y_MAJOR
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(1));
#else
	VuGridIterator	myit(ObjProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(1));
#endif
	Objective	o;
	GridIndex   x,y;
...
int AnalyseThreats (GridIndex X, GridIndex Y, MoveType mt, int alt, int roe_check, Team who, int flags)
	...
	while (e)
		...
		if (e->GetDetectionRange(mt) > d && GetRoE(e->GetTeam(),who,roe_check))
			{
			if (e->IsUnit() && !(flags & FIND_NOAIR && e->GetDomain() == DOMAIN_AIR) &&
...
int CollectThreats (GridIndex X, GridIndex Y, int Z, Team who, int flags, F4PFList foundlist)
	...
	while (pass < 2)
		...
		if (!pass)
			{
#ifdef VU_GRID_TREE_Y_MAJOR
			myit = new VuGridIterator(RealUnitProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(MAX_AIR_SEARCH));
#else
			myit = new VuGridIterator(RealUnitProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(MAX_AIR_SEARCH));
#endif
			}
		else
			{
#ifdef VU_GRID_TREE_Y_MAJOR
			myit = new VuGridIterator(ObjProxList,(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(MAX_AIR_SEARCH));
#else
			myit = new VuGridIterator(ObjProxList,(BIG_SCALAR)GridToSim(Y),(BIG_SCALAR)GridToSim(X),(BIG_SCALAR)GridToSim(MAX_AIR_SEARCH));
#endif
			}
		e = (CampEntity) myit->GetFirst();
		while (e)
...
int CollectThreatsFast (GridIndex X, GridIndex Y, int altlevel, Team who, int flags, F4PFList foundlist)
	...
	while (pass < 2)
		...
		{
		    //if (tteam[e->GetTeam()] && (!e->IsUnit() || !((Unit)e)->Moving()) && !foundlist->Find(e->Id())) // !ThreatSearch[e->GetCampID()]) // JB 010223 CTD
		    //if (!F4IsBadCodePtr((FARPROC) e) && (tteam[e->GetTeam()] && (!e->IsUnit() || !((Unit)e)->Moving()) && !foundlist->Find(e->Id()))) // JB 010223 CTD
		    // This is a bit of a campaign performance hog ~ 36% of campaign thread in this routine
		    // of which about 30% of that is in the two following IsBad checks JPO

		    // if (!F4IsBadCodePtr((FARPROC) e) && e->GetTeam() >= 0 && !F4IsBadReadPtr(&(tteam[e->GetTeam()]), sizeof(uchar)) && (tteam[e->GetTeam()] && (!e->IsUnit() || !((Unit)e)->Moving()) && !foundlist->Find(e->Id()))) // JB 010304 CTD
		    ShiAssert(FALSE == F4IsBadReadPtr(e, sizeof *e));
		    ShiAssert(e->GetTeam() >= 0 && e->GetTeam() < NUM_TEAMS);
		    if (e == NULL || e->GetTeam() < 0 || e->GetTeam() >= NUM_TEAMS)
			continue; // something bogus
		    if (tteam[e->GetTeam()] == ROE_NOT_ALLOWED) // not enemy
			continue;
		    if ((!e->IsUnit() || !((Unit)e)->Moving()) && !foundlist->Find(e->Id()))
		    {
			e->GetLocation(&x,&y);
			d = FloatToInt32(Distance(X,Y,x,y));
...
FalconSessionEntity* FindPlayer(Flight flight, uchar planeNum, uchar pilotSlot)
	...
	if(flight)
		...
		while (curSession)
			{
			if(curSession->GetPlayerFlightID() == flight->Id() && curSession->GetAircraftNum() == planeNum && curSession->GetPilotSlot() == pilotSlot)
				return (curSession);
			curSession = (FalconSessionEntity*)sessionWalker.GetNext();
			}
		}
