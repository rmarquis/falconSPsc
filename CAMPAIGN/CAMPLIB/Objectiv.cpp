...
/* 2001-04-06 S.G. 'CanDetect' */ #include "Graphics\Include\TMap.h"
...
ObjectiveClass::ObjectiveClass (int typeindex) : CampBaseClass(typeindex)
	{
	int			size;
...
ObjectiveClass::ObjectiveClass (VU_BYTE **stream) : CampBaseClass(stream)
	{
	uchar	size,nsize,i;
	...
	memcpy(&obj_data.last_repair, *stream, sizeof(CampaignTime));	*stream += sizeof(CampaignTime);
	if (gCampDataVersion > 1)
		{
		memcpy(&obj_data.obj_flags, *stream, sizeof(ulong));		*stream += sizeof(ulong);
		}
	else
		{
		short		temp;
		memcpy(&temp, *stream, sizeof(short));						*stream += sizeof(short);
		obj_data.obj_flags = temp;
		}
	memcpy(&obj_data.supply, *stream, sizeof(uchar));				*stream += sizeof(uchar);
	memcpy(&obj_data.fuel, *stream, sizeof(uchar));					*stream += sizeof(uchar);
	memcpy(&obj_data.losses, *stream, sizeof(uchar));				*stream += sizeof(uchar);
	...
	if (nsize == size)
		...
		memcpy(obj_data.fstatus, *stream, size);					*stream += size;
		}
	else if (nsize > size)
		{
		...
		memcpy(obj_data.fstatus, *stream, nsize);					*stream += size;
		}
	memcpy(&obj_data.priority, *stream, sizeof(uchar));				*stream += sizeof(uchar);
	obj_data.status = 100; // JPO init

	memcpy(&static_data.nameid, *stream, sizeof(short));			*stream += sizeof(short);
	memcpy(&static_data.parent, *stream, sizeof(VU_ID));			*stream += sizeof(VU_ID);
#ifdef DEBUG
	static_data.parent.num_ &= 0x0000ffff;
#endif
	memcpy(&static_data.first_owner, *stream, sizeof(Control));		*stream += sizeof(Control);
	memcpy(&static_data.links, *stream, sizeof(uchar));				*stream += sizeof(uchar);
	...
	for (i=0; i<static_data.links; i++)
		{
		memcpy(&link_data[i], *stream, sizeof(CampObjectiveLinkDataType));	*stream += sizeof(CampObjectiveLinkDataType);
#ifdef DEBUG
	...
	if (gCampDataVersion >= 20)
		{
		memcpy(&i, *stream, sizeof(uchar));							*stream += sizeof(uchar);
		if (i)
			{
			#ifdef USE_SH_POOLS
			...
			#endif
			memcpy(static_data.radar_data, *stream, sizeof(RadarRangeClass));	*stream += sizeof(RadarRangeClass);
			}
		else
			static_data.radar_data = 0;
		}
...
void ObjectiveClass::UpdateFromData (VU_BYTE **stream)
	{
	uchar		len;
	uchar		owner;

	memcpy(&obj_data.last_repair, *stream, sizeof(CampaignTime));	*stream += sizeof(CampaignTime);
	memcpy(&owner, *stream, sizeof(uchar));							*stream += sizeof(uchar);		
	SetOwner(owner);
	memcpy(&obj_data.supply, *stream, sizeof(uchar));				*stream += sizeof(uchar);		
	memcpy(&obj_data.fuel, *stream, sizeof(uchar));					*stream += sizeof(uchar);		
	memcpy(&obj_data.losses, *stream, sizeof(uchar));				*stream += sizeof(uchar);		
	memcpy(&len, *stream, sizeof(uchar));							*stream += sizeof(uchar);		
	#ifdef USE_SH_POOLS
	...
	if (gCampDataVersion < 64)
		{
		memcpy(obj_data.fstatus, *stream, 1);						*stream += 1;
		memset(obj_data.fstatus+1, 0, len-1);
		}
	else
		{
		memcpy(obj_data.fstatus, *stream, len);						*stream += len;	
		}
	ResetObjectiveStatus();
	}
...
VU_ERRCODE ObjectiveClass::Handle(VuFullUpdateEvent *event)
	...
	Objective tmp_ent = (Objective)(event->expandedData_);
...
int ObjectiveClass::Deaggregate (FalconSessionEntity *session)
	...
	MonoPrint ("Deaggregating Objective #%d! - owner is: %d\n",GetCampID(), session->Id().creator_.value_);
#endif
...
//	uchar					value,*ddptr = NULL;
	...
	for (f=0; f<oc->Features; f++,fid++)
		...
		if (classID)
			{
			fc = GetFeatureClassData (classID);
			if (!fc || fc->Flags & FEAT_VIRTUAL) // || fc->Priority > PlayerOptions.BuildingDetailLevel())
				continue;
			simdata.campSlot = f;
...
#ifdef DEAG_DEBUG
	MonoPrint ("- Deaggregation of Objective #%d took %d msec, %d features, %d added\n", GetCampID(), GetTickCount() - startTime, oc->Features, added);
#endif
...
#ifdef DEAG_DEBUG
	unsigned long startTime = GetTickCount();
	MonoPrint ("Reaggregating objective #%d!\n",GetCampID());
#endif
...
#else
			((SimBaseClass*)feature)->SetRemoveFlag();
#endif
			feature = myit.GetNext();
			}
...
#ifdef DEAG_DEBUG
	MonoPrint ("+ Reaggregation of Objective #%d took %d msec\n", GetCampID(), GetTickCount() - startTime);
#endif
	return 1;
	}
...
void ObjectiveClass::DeaggregateFromData (int size, uchar* data)
	{
	...
	memcpy(&creator, data, sizeof(VU_SESSION_ID));				data += sizeof(VU_SESSION_ID);
	memcpy(&obj_data.obj_flags, data, sizeof(short));			data += sizeof(short);
	memcpy(&len, data, sizeof(uchar));							data += sizeof(uchar);
	memcpy(obj_data.fstatus, data, len);						data += len;
	memcpy(&features, data, sizeof(uchar));						data += sizeof(uchar);

	//MonoPrint ("Features %08x:%08x = %d (%d)\n", Id(), features, static_data.class_data->Features);

	session = (FalconSessionEntity*) vuDatabase->Find(vuid);
	if (!session)
		return;
	// Set the owner of the newly created deaggregated entities
	SetDeagOwner (session->Id());
...
#ifdef DEAG_DEBUG
	MonoPrint ("Got remote deaggregation event for Objective #%d - owner is %d!\n",GetCampID(),session->Id().creator_.value_);
#endif

	SetAggregate(0);

	//ShiAssert( static_data.class_data->Features == features);
...
void ObjectiveClass::ReaggregateFromData (int size, uchar* data)
	{
	if (IsLocal() || IsAggregate())
		return;
...
void ObjectiveClass::TransferOwnershipFromData (int size, uchar* data)
	{
...
Objective NewObjective (short tid, VU_BYTE **stream)
	{
	Objective   o;
	int			i;
...
int LoadBaseObjectives(char* scenario)
	...
	Objective   o;
	short       num,i,type;
	long		size,newsize,rsize;
	uchar		*buffer,*bufptr;
	uchar		*data,*data_ptr;

	old_version = gCampDataVersion;

	if ((data = (uchar *) ReadCampFile (scenario, "obj")) == NULL)
	{
	...
	num = *((short *) data_ptr); data_ptr += sizeof (short);
	size = *((long *) data_ptr); data_ptr += sizeof (long);
	newsize = *((long *) data_ptr); data_ptr += sizeof (long);

	buffer = new uchar[size + MAX_POSSIBLE_OVERWRITE];

	rsize = LZSS_Expand(data_ptr, buffer, size);
	bufptr = buffer;

	for (i=0; i<num; i++)
		{
		memcpy(&type, bufptr, sizeof(short));				bufptr += sizeof(short);
		o = NewObjective(type, &bufptr);
		o->SetAggregate(1);
		}
...
int LoadObjectiveDeltas (char* savefile)
	...
	if ((data = (uchar *) ReadCampFile (savefile, "obd")) == NULL)
		return 0;

	data_ptr = data;

	csize = *((long *) data_ptr); data_ptr += sizeof (long);

	DecodeObjectiveDeltas(&data_ptr, NULL);
	delete data;
	return 1;
	}
...
void CaptureObjective(Objective co, Control who, Unit u)
{
	...
	// Now remove any enemy flights which are based here
	unit = (Unit) fit.GetFirst();
	while (unit)
	{
		if (unit->IsFlight() && GetRoE(newown,unit->GetTeam(),ROE_GROUND_CAPTURE) == ROE_ALLOWED)
		{
...
int EncodeObjectiveDeltas(VU_BYTE **stream, FalconSessionEntity *owner)
	...
	ent = (CampEntity) myit.GetFirst();	// Use AllCamp iterator - just in case we don't have an ObjList (Dogfight)
	while (ent) 
		{
		if ((ent->EntityType())->classInfo_[VU_CLASS] == CLASS_OBJECTIVE && (!owner || ent->OwnerId() == ownerid))
			{
			o = (Objective)ent;
			if (o->HasDelta())
				{
...
int DecodeObjectiveDeltas(VU_BYTE **stream, FalconSessionEntity *)
	{
	long            size;
	short			count;
	VU_ID			vuid;
	...
	while (count)
	{
		memcpy(&vuid, buf, sizeof(VU_ID));				buf += sizeof(VU_ID);
...
void ObjectiveClass::ReadDirty (unsigned char **stream)
{
	int
		size,
		num;
	...
	//MonoPrint ("  OB %08x", bits);

	if (bits & DIRTY_STATUS)
	{
		obj_data.status = *(unsigned char*) ptr;
		ptr += sizeof (unsigned char);
