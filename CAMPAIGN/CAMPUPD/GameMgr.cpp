...
void GameManagerClass::AnnounceEntry (void)
{
...
void GameManagerClass::AnnounceExit (void)
{
...
SimMoverClass* GameManagerClass::AttachPlayerToVehicle (FalconSessionEntity *player, SimMoverClass *simEntity, int playerSlot)
{
	...
	if (player == FalconLocalSession)
	{
		...
		SimDriver.SetPlayerEntity(simEntity);
...
int GameManagerClass::DetachPlayerFromVehicle (FalconSessionEntity *player, SimMoverClass* simEntity)
{
	...
	if (simEntity)
	{
		simEntity->UnSetFalcFlag(FEC_INVULNERABLE); // JB 010617 (Fixes MP bug where ejected planes can't be destroyed)
	...
	if (simEntity)
	{
		...
		if(simEntity->OnGround() && simEntity->IsAirplane())
		{
			...
			if(((AircraftClass *)simEntity)->DBrain()->CreateTime() + 2*CampaignSeconds < SimLibElapsedTime)
			{
				...
				ObjectiveClass *atc = (ObjectiveClass*)vuDatabase->Find(((AircraftClass*)simEntity)->DBrain()->Airbase());
				
				if( ((AircraftClass*)simEntity)->DBrain()->IsSetATC(DigitalBrain::PermitTakeoff) )
				{
					radioMessage = CreateCallFromATC (atc, (AircraftClass*)simEntity, rcCLEAREDONRUNWAY, FalconLocalSession);
					radioMessage->dataBlock.edata[3] = atc->brain->GetRunwayName(((AircraftClass*)simEntity)->DBrain()->Runway());
					
				}
				else if( ((AircraftClass*)simEntity)->DBrain()->IsSetATC(DigitalBrain::PermitRunway) )
				{
					radioMessage = CreateCallFromATC (atc, (AircraftClass*)simEntity, rcPOSITIONANDHOLD, FalconLocalSession);
					radioMessage->dataBlock.edata[3] = atc->brain->GetRunwayName(((AircraftClass*)simEntity)->DBrain()->Runway());
				}
				else
				{
					radioMessage = CreateCallFromATC (atc, (AircraftClass*)simEntity, rcCLEARTOTAXI, FalconLocalSession);
					radioMessage->dataBlock.edata[2] = atc->brain->GetRunwayName(((AircraftClass*)simEntity)->DBrain()->Runway());
				}
