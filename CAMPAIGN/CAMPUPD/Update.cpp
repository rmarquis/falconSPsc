...
#define AIRLIFT_SUPPLIES		20			// Amount of stuff that C-130 brings in.. ;-)
#define AIRLIFT_FUEL			2
#define AIRLIFT_REPLACEMENTS	2
...
int UpdateUnit (Unit u, CampaignTime DeltaTime)
	...
	u->UnsetChecked();
	if (TheCampaign.CurrentTime - u->GetLastCheck() > u->UpdateTime())
		{
		...
		if (u->IsBattalion() && FalconLocalGame->GetGameType() == game_TacticalEngagement && !u->GetCurrentUnitWP())
			{
			GridIndex		x,y;
			u->GetLocation(&x,&y);
...
//	if (a->IsUnit() && a->GetAproxWeaponRange(em) >= d && a->GetWeaponRange(em) >= d) // REMOVED BY S.G.
	...
	if (a->IsUnit() && a->GetAproxWeaponRange(em) >= d && ((am == Air && em == Air && d <= 37 /*20NM*/ && a->GetWeaponRange(em) > 1) || a->GetWeaponRange(em, e) >= d)) // 2002-03-08 MODIFIED BY S.G. Added 'e' at the end of a->GetWeaponRange so we test the min/max weapon range against this guy
		*ran = 1;
	}
...
//noprint			MonoPrint("Unit %d taking off at time %d / %d.\n", u->GetCampID(), Camp_GetCurrentTime(), w->GetWPArrivalTime());
		...
		case WP_LAND:		
			...
			if (u->GetUnitMission() == AMIS_AIRLIFT)
				{
				int		team = u->GetTeam();
				TeamInfo[team]->SetSupplyAvail(TeamInfo[team]->GetSupplyAvail() + AIRLIFT_SUPPLIES);
				TeamInfo[team]->SetFuelAvail(TeamInfo[team]->GetFuelAvail() + AIRLIFT_FUEL);
				TeamInfo[team]->SetReplacementsAvail(TeamInfo[team]->GetReplacementsAvail() + AIRLIFT_REPLACEMENTS);
#ifdef DEBUG
