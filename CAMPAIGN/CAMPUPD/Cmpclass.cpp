...
int gCurrentDataVersion = 73;				// Current version of campaign data files
int gCampDataVersion = gCurrentDataVersion;	
int gClearPilotInfo = 0;
int gTacticalFullEdit = 0;
#ifdef DEBUG
int gPlaceholderOnly = 0;
char WildFile[2][40] = { "campaign\\save\\*.CAM", "campaign\\save\\*.TAC" };
char gPlaceholderFile[80];
int LoadPlaceholderTeams (char* scenario);
...
extern void ResetNamespace (void);
...
#ifdef DEBUG
extern void ObjectiveReport(void);
extern void PackageReport(void);
extern void UnitReport(void);
#endif
...
F4THREADHANDLE CampaignClass::InitCampaign (FalconGameType gametype, FalconGameEntity *joingame)
{
	...
	((WeatherClass*)TheWeather)->Init();
	
	if (!LoadTheater(TheaterName)) // JB 010731 return on fail
		return 0;
	...
	// Begin Uplink stuff
	if(m_pUplink != NULL && FalconLocalGame && FalconLocalGame->IsLocal() && gCommsMgr && gCommsMgr->Online())
	{
		try
		{
			if(gametype == game_InstantAction)
				m_pUplink->PutGameType("IA");
			else if(gametype == game_Dogfight)
				m_pUplink->PutGameType("DF");
			else if(gametype == game_TacticalEngagement)
				m_pUplink->PutGameType("TE");
			else if(gametype == game_Campaign)
				m_pUplink->PutGameType("CA");
			else m_pUplink->PutGameType("UNK");

			m_pUplink->PutMaxPlayers(FalconLocalGame->GetRules()->MaxPlayers);
...
//            ent->SetCampID(id);
	...
	if (!(Flags & CAMP_LIGHT) && !(Flags & CAMP_TACTICAL))
	{
		...
		((WeatherClass*)TheWeather)->CampLoad(savefile, 0);
		StandardRebuild();
	...
	{
		((WeatherClass*)TheWeather)->CampLoad(savefile, gametype);
	}

//	ChillTypes();
	ShiAssert( TeamInfo[0] );
	Flags |= CAMP_LOADED;

	// Insert our game into the database - which will broadcast it if we're online
	vuDatabase->QuickInsert(gCommsMgr->GetTargetGame ());
...
int CampaignClass::JoinCampaign (FalconGameType gametype, FalconGameEntity *game)
	...
	if (Flags & CAMP_NEED_MASK)
		...
		if (!LoadCampaignEvents(Scenario,Scenario))
			NewCampaignEvents(Scenario);
		if (!(Flags & CAMP_LIGHT))
			((WeatherClass*)TheWeather)->CampLoad(TheCampaign.Scenario, game_Campaign);
...
//				SaveUnitDeltas(savefile);
				...
				((WeatherClass*)TheWeather)->Save(savefile);
...
void CampaignClass::EndCampaign (void)
	...
	{
		HRESULT hr = m_pUplink->raw_Stop();
		// ShiAssert(SUCCEEDED(hr));

		#ifdef _DEBUG
		if(FAILED(hr))
			MonoPrint("CampaignClass::EndCampaign: IUplink::Stop failed with 0x%X!", hr);
		#endif
	}
	// End Uplink stuff
	...
	// Now clean up.
	CampEnterCriticalSection();
	VuEnterCriticalSection();		// KCK: I loath to do this, but VU just isn't threadsafe during shutdown
...
//		DisposeRunwayList();
	...
	// flush memory
	/*
	** edg: Tried this, but too dangerous I think
	#ifdef USE_SH_POOLS
...
int CampaignClass::Decode (VU_BYTE **stream)
	{
	...
	memcpy(&datasize, *stream, sizeof(long));					*stream += sizeof(long);
	
	buffer = new VU_BYTE[datasize + 1024];
	bufhead = buffer;
	LZSS_Expand(*stream, buffer, datasize);

	memcpy(&CurrentTime, buffer, sizeof(CampaignTime));			buffer += sizeof(CampaignTime);
	if (CurrentTime == 0)
		CurrentTime = 1;
	SetTime (CurrentTime);
	if (gCampDataVersion >= 48)
	{
		memcpy(&TE_StartTime, buffer, sizeof(CampaignTime));	buffer += sizeof(CampaignTime);
		memcpy(&TE_TimeLimit, buffer, sizeof(CampaignTime));	buffer += sizeof(CampaignTime);
		if (gCampDataVersion > 49)
			{
			memcpy(&TE_VictoryPoints, buffer, sizeof(long));	buffer += sizeof(long);
			}
		else
		TE_VictoryPoints=0;
	}
	...
	{
		memcpy(&TE_type, buffer, sizeof(long));						buffer += sizeof(long);
		memcpy(&TE_number_teams, buffer, sizeof(long));				buffer += sizeof(long);
		memcpy(TE_number_aircraft, buffer, sizeof(long) * 8);		buffer += sizeof(long) * 8;
		memcpy(TE_number_f16s, buffer, sizeof(long) * 8);			buffer += sizeof(long) * 8;
		memcpy(&TE_team, buffer, sizeof(long));						buffer += sizeof(long);
		memcpy(TE_team_pts, buffer, sizeof(long) * 8);				buffer += sizeof(long) * 8;
		memcpy(&TE_flags, buffer, sizeof(long));					buffer += sizeof(long);

		for (loop = 0; loop < 8; loop ++)
		{
			memcpy(&team_flags[loop], buffer, 1);					buffer += 1;
			memcpy(&team_colour[loop], buffer, 1);					buffer += 1;
			memcpy(&team_name[loop], buffer, 20);					buffer += 20;
			memcpy(&team_motto[loop], buffer, 200);					buffer += 200;
		}
	}
	else
	{
	...
	if (gCampDataVersion >= 19)
		{
		memcpy(&lastMajorEvent, buffer, sizeof(CampaignTime));	buffer += sizeof(CampaignTime);
		}
	memcpy(&lastResupply, buffer, sizeof(CampaignTime));		buffer += sizeof(CampaignTime);
	memcpy(&lastRepair, buffer, sizeof(CampaignTime));			buffer += sizeof(CampaignTime);
	memcpy(&lastReinforcement, buffer, sizeof(CampaignTime));	buffer += sizeof(CampaignTime);
	memcpy(&TimeStamp, buffer, sizeof(short));					buffer += sizeof(short);
	memcpy(&Group, buffer, sizeof(short));						buffer += sizeof(short);
	// Dont read flags
	memcpy(&GroundRatio, buffer, sizeof(short));				buffer += sizeof(short);
	memcpy(&AirRatio, buffer, sizeof(short));					buffer += sizeof(short);
	memcpy(&AirDefenseRatio, buffer, sizeof(short));			buffer += sizeof(short);
	memcpy(&NavalRatio, buffer, sizeof(short));					buffer += sizeof(short);
	memcpy(&Brief, buffer, sizeof(short));						buffer += sizeof(short);
	// Dont read Processor
	memcpy(&TheaterSizeX, buffer, sizeof(short));				buffer += sizeof(short);
	memcpy(&TheaterSizeY, buffer, sizeof(short));				buffer += sizeof(short);
	// Adjust some globals
	...
	// Continue reading...
	memcpy(&CurrentDay, buffer, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(&ActiveTeams, buffer, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(&DayZero, buffer, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(&EndgameResult, buffer, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(&Situation, buffer, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(&EnemyAirExp, buffer, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(&EnemyADExp, buffer, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(&BullseyeName, buffer, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(&BullseyeX, buffer, sizeof(GridIndex));				buffer += sizeof(GridIndex);
	memcpy(&BullseyeY, buffer, sizeof(GridIndex));				buffer += sizeof(GridIndex);
	memcpy(TheaterName, buffer, sizeof(char)*CAMP_NAME_SIZE);	buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(Scenario, buffer, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(SaveFile, buffer, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(UIName, buffer, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
	// Might as well get the other guy's squadron
	memcpy(&PlayerSquadronID, buffer, sizeof(VU_ID));			buffer += sizeof(VU_ID);
	FalconLocalSession->SetPlayerSquadronID (PlayerSquadronID);
	// Load the recent event queues
	DisposeEventLists();
	memcpy(&entries, buffer, sizeof(short));					buffer += sizeof(short);
	for (i=0; i<entries; i++)
		{
		event = new CampUIEventElement();
		memcpy(event, buffer, sizeof(uieventnode));				buffer += sizeof(uieventnode);
		memcpy(&size, buffer, sizeof(short));					buffer += sizeof(short);
		event->eventText = new _TCHAR[size+1];
		memcpy(event->eventText, buffer, sizeof(_TCHAR)*size);	buffer += sizeof(_TCHAR)*size;
	...
	// Read the map data
	FreeCampMaps();
	memcpy(&CampMapSize, buffer, sizeof(short));				buffer += sizeof(short);
	if (CampMapSize > 0)
		{
		CampMapData = new uchar[CampMapSize];
		memcpy(CampMapData, buffer, CampMapSize);				buffer += CampMapSize;
		}

	// Read the squadron data
	FreeSquadronData();
	memcpy(&LastIndexNum, buffer, sizeof(short));				buffer += sizeof(short);
	memcpy(&NumAvailSquadrons, buffer, sizeof(short));			buffer += sizeof(short);
	if (NumAvailSquadrons > 0)
		{
		if (gCampDataVersion < 42)
			{
			OldSquadUIInfoClass		*osic = new OldSquadUIInfoClass[NumAvailSquadrons];
			memcpy(osic, buffer, sizeof(OldSquadUIInfoClass)*NumAvailSquadrons);		buffer += sizeof(OldSquadUIInfoClass)*NumAvailSquadrons;
			...
			{
			CampaignSquadronData = new SquadUIInfoClass[NumAvailSquadrons];
			memcpy(CampaignSquadronData, buffer, sizeof(SquadUIInfoClass)*NumAvailSquadrons);		buffer += sizeof(SquadUIInfoClass)*NumAvailSquadrons;
			}
		}
	if (gCampDataVersion >= 31)
		{
		memcpy(&Tempo, buffer, sizeof(uchar));					buffer += sizeof(uchar);
		}

	if (gCampDataVersion >= 43)
		{
		memcpy(&CreatorIP, buffer, sizeof(long));		buffer += sizeof(long);
		memcpy(&CreationTime, buffer, sizeof(long));	buffer += sizeof(long);
		memcpy(&CreationRand, buffer, sizeof(long));	buffer += sizeof(long);
		}
...
int CampaignClass::LoadScenarioStats (FalconGameType type, char *savefile)
	...
	data = (unsigned char *) ReadCampFile (savefile, "cmp");
...
void CampaignClass::ReadValidAircraftTypes (char *typefile)
	...
	if ((data = ReadCampFile (typefile, "act")) == NULL)
		return;
...
int ReadVersionNumber (char *saveFile)
{
	...
	int
		vers = 1;		// If we can't load a version file, assume the worst ? RH
	
	data = ReadCampFile (saveFile, "ver");
...
//		MakeCampMap(MAP_OWNERSHIP);
	...
	((WeatherClass*)TheWeather)->Save(filename);
