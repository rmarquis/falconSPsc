...
EventClass::EventClass (uchar **data_ptr)
	{
	if (!data_ptr)
		return;

	event = *((short *) *data_ptr); *data_ptr += sizeof (short);
	flags = *((short *) *data_ptr); *data_ptr += sizeof (short);
	}
...
int LoadCampaignEvents(char* filename, char* scenario)
	...
	ReadNumberOfEvents(scenario);
	if ((data = (uchar *) ReadCampFile (filename, "evt")) == NULL)
		return 0;
	...
	for (i=0; i<events; i++)
		CampEvents[i] = new EventClass(&data_ptr);
