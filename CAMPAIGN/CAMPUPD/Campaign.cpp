...
extern int g_nMPStartTime; // JB 0203111 MP takeoff time if g_bMPStartRestricted enabled.
...
//CampaignTime	ReconLossTime[MOVEMENT_TYPES] = { CampaignHours, 20*CampaignMinutes, 10*CampaignMinutes, 10*CampaignMinutes, 2*CampaignMinutes, 2*CampaignMinutes, 10*CampaignMinutes, 10*CampaignMinutes };
CampaignTime	ReconLossTime[MOVEMENT_TYPES] = { CampaignHours, 20*CampaignMinutes, 10*CampaignMinutes, 10*CampaignMinutes, 1*CampaignMinutes, 1*CampaignMinutes, 10*CampaignMinutes, 10*CampaignMinutes };
uchar			DefaultDamageMods[OtherDam+1] = { 0, 100, 100, 0, 0, 100, 100, 0, 0, 0, 0 };	

#define MAX_DISTANT_EFFECT_DIST_SQ	45000000000								// distance squared, in feet of fatherest visual effect

#define PERSIST_BUBBLE_MAX		10*GRID_SIZE_FT		// The max distance (ft) at which we'll deaggregate persistant objects
...
#ifdef DEBUG
int gDeaggregatedCampaignEntities = 0;
int gDeaggregateRequests = 0;
#endif
...
{
	int		want_deaggregate = 0,
			want_in_sim_list =0;
		bool didsimlistcrap = FALSE;
	 int inbobble = (session->InSessionBubble(e,REAGREGATION_RATIO));
	// Here we do the following:
	// a) Set Interested() state on entity. Interested() means it should be deaggregated by the end of rebuild bubble
	...
	// d) Check if it's within our local local sim area and insert or remove it from the sim lists accordingly
	if (e->IsAggregate())
	{
		...
		{	// It's in our bubble, post deaggregate message if host
			...
			if (e->IsLocal())
			{
				VuTargetEntity* target = (VuTargetEntity*) vuDatabase->Find(vuLocalSessionEntity->Game()->OwnerId());
				FalconSimCampMessage	*msg = new FalconSimCampMessage (e->Id(), target);
				//me123 let the host own all the planes
				if (e->IsFlight() || (g_bServer && g_bServerHostAll)) 
				{
	...
	{ 
		...
		if (session == FalconLocalSession )  // Update local sleep/wake state
				...
				if (e->IsAwake() && (!want_deaggregate || g_bSleepAll))
				{
					...
					if (vuLocalSessionEntity->Game()->IsLocal())
					{
						if (!e->IsAirplane() && !e->IsHelicopter())
							...
							(e->IsObjective() && 
							e->GetType () != TYPE_AIRBASE && 
							e->GetType () != TYPE_AIRSTRIP))
							e->Sleep();
					}
				}
		...
		if (vuLocalSessionEntity->Game()->IsLocal() && 
				...
				//handle airplanes helicopters and airbases
					...
					(e->IsObjective() && ((e->GetType () == TYPE_AIRBASE) || (e->GetType () == TYPE_AIRSTRIP)) ) || //and airbases
				...
				e->SetChecked();
						...
						(
						 !e->IsSetFalcFlag(FEC_PLAYERONLY) ||// not a human
							(
							 e->IsSetFalcFlag(FEC_PLAYERONLY) && e->IsSetFalcFlag(FEC_HASPLAYERS)//human but he's attached 
							)
						)
					)
			{
				if ((FalconLocalSession->GetFlyState() == FLYSTATE_IN_UI || 
					 ...
					 FalconLocalSession->GetFlyState() ==FLYSTATE_WAITING))// only wake if the server isn't transitign to/from ui
				{
				if (!e->IsAwake())	e->Wake();
				want_deaggregate = 1;
	...
	if (session == FalconLocalSession && !didsimlistcrap)
	{
		...
		{
			VuEntity *player = session->Camera(0);
			if (player)
			{
...
void ChooseNewSession (CampEntity ent)
{
	...
	if (best_session)
	{
//		MonoPrint ("Changing CampEntity %08x%08x to new session %08x\n", ent->Id(), best_session->Id ().creator_);

		//VuTargetEntity* target = (VuTargetEntity*) vuDatabase->Find(FalconLocalGame->OwnerId());
		FalconSimCampMessage	*msg = new FalconSimCampMessage (ent->Id(), FalconLocalGame); // target);
...
{
#ifdef _TIMEDEBUG
	DWORD start = GetTickCount(), s2;
	int campunits = 0;
	int campobjs = 0;
	int unitProxy = 0, objProxy = 0;
#endif
	float			sx,sy;
//	float			sx2,sy2;
	VuGridIterator	*myit;
				...
				{// host wake missiles and bombs so we can drive them
					...
					static int wokentotal =0;
						...
						if (!object->IsAwake() && 
							...
							numberofobjects++;
	//						MonoPrint ("woken %d  Totalobjs %d   WOKENTOTAL %d \n", wokenthiscycle,numberofobjects,wokentotal );
...
#ifdef DEBUG
	gDeaggregatedCampaignEntities = 0;
	gDeaggregateRequests = 0;
#endif
							...
							if (airbase && airbase->IsObjective() && airbase->brain)
							{
								if(GetTTRelations(airbase->GetTeam(), u->GetTeam()) <= Neutral)
								{
									...
									{								
										...
										ulong nextTOTime = airbase->brain->FindFlightTakeoffTime((Flight)u,GetQueue(rwindex));
										...
										{//me123 
											...
											minDeagTime = max(minDeagTime, nextTOTime - airbase->brain->MinDeagTime());	
											else if (u->IsFlight()&& (((Flight)u)->GetEvalFlags() & FEVAL_START_COLD)) 
											{
												minDeagTime -= CampaignMinutes * PlayerOptionsClass::RAMP_MINUTES;
											}
										}
										else if (u->IsSetFalcFlag(FEC_PLAYER_ENTERING|FEC_HASPLAYERS) && u->IsFlight()) {
...
///////////////////////////////////////////////////////////////////////////////
...
#ifdef DEBUG
	gDeaggregateRequests = deag_ents;
#endif
	...
	if (FalconLocalGame->IsLocal())
	{
		...
		while (session)
		{
		 if (session !=FalconLocalSession)//me123 let the host own the ents even when he's in the ui
		  {
	 			c = (CampEntity) deag_it.GetFirst ();
	 			...
	 			while (c)
	 			{
					 if ((c->IsUnit ()) && (c->GetDeagOwner () == session->Id ()))
					 {
					 	if (!session->Camera (0))
					 	{
	 //						MonoPrint ("Deag Owner of CampEntity %08x has no Camera(0)\n", c);
						  ChooseNewSession (c);
					 	}
					 }
...
//							MonoPrint ("Reaggregating Flight that is a player flight %08x\n", c);
	...
	if (end - start > 0)
	MonoPrint ("RebuildBubble time %d, wait %d, campObjs %d, campUnits %d, unitProxy %d, objProxy %d\n",
	    end - start, s2 - start, campobjs, campunits, unitProxy, objProxy);
#endif
}
...
char *ReadCampFile (char *filename, char *ext)
{
	int
		size,
		index;
...
static int starttimer = FALSE;
	...
	else if (!gCompressTillTime || lastchoice != PlayerOptions.GetStartFlag())
	{
	  // JPO - decide where we start
		...
		gCompressTillTime = sTakeoffTime - 2 * CampaignMinutes;
		break;
...
if (PlayerOptions.GetStartFlag() != PlayerOptionsClass::START_RUNWAY &&
	 timer && timer < 3 * CampaignSeconds &&  
	 gLaunchTime <  vuxGameTime + 3 * CampaignSeconds )
	 gLaunchTime = min(sTakeoffTime, vuxGameTime + 3 * CampaignSeconds); 
...
int CompressCampaignUntilTakeoff (Flight flight)
	...
	if (FalconLocalGame && FalconLocalGame->IsLocal())
	{
		...
		while (sess)
		{
			...
			if (flt)
			{
				if (flt->GetCurrentUnitWP() && flt->GetCurrentUnitWP()->GetWPAction() == WP_TAKEOFF)
				{
				if (!firstentrytime) firstentrytime = flt->GetCurrentUnitWP()->GetWPArrivalTime();
				else if (flt->GetCurrentUnitWP()->GetWPArrivalTime() < firstentrytime)
...
//				if (gMainHandler->GetAppWnd ())
				...
				MonoPrint("Campaign is %d seconds behind. Resynchronizing.\n",(int)(vuxGameTime-Camp_GetCurrentTime())/1000);
#endif
		}
		else
		{
...
void UpdatePlayerSessions (void)
{
	CampEnterCriticalSection ();
...
unsigned int __stdcall HandleCampaignThread (void)
	...
	while (ThreadManager::campaign_active())
		...
		if (TheCampaign.Flags & CAMP_SUSPEND_REQUEST)		// Someone's asked us to suspend
			{
		...
		FalconEntity::DoCampaignDirtyData ();
		...
		if (deltatime > CampaignMinutes)		 					// 1 minute maximum time delta
			{
...
#ifdef CAMPTOOL
			ShowTime(TheCampaign.CurrentTime);
#endif
			}
...
void DoTacticalLoop (int startup)
	...
	if (!SimDriver.InSim())
		((WeatherClass*)TheWeather)->UpdateWeather();						// Sim calls this otherwise
	}
...
void UpdateRealUnits (CampaignTime deltatime)
{
    ...
    while (u)
    {
		UpdateUnit(u,deltatime);
		u = (Unit) rit.GetNext();
		if (enableswitch > 0 && u && i % enableswitch == 0) { // brief pause to let others in
			MonoPrint("Pausing update at %d\n", i);
			CampLeaveCriticalSection();
			Sleep(1);
			CampEnterCriticalSection();
		}
    ...
    DWORD end = GetTickCount();
    if (end - start > 0) {
		MonoPrint ("UpdateRealUnits time %d, wait %d, units %d\n", end -start, sw - start, i - 1);
    }
...
// 2002-04-02 MNLOOK does this make problems in MP ? Now that we've a floating bullseye, perhaps it should be put back in
...
// Rally units (Tactical engagement only)
void RallyUnits (int minutes)
{
	Unit			u;
