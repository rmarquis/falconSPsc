...
int ProduceSupplies (CampaignTime deltaTime)
	...
	{
	    type = o->GetType();
	    if (type == TYPE_FACTORY && o->GetObjectiveOldown() == o->GetOwner())		// Supply
	    {
		if (g_bPowerGrid) {
		    o->GetLocation(&x, &y);
...
// A.S. begin additional variables 2001-12-09
	int				rneeded_a=0, rneeded_v=0, repl_a=0, repl_v=0;	// A.S. We now distinguish between aircrafts and ground vehicles 
