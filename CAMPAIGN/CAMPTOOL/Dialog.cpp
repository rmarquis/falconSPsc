...
BOOL WINAPI WeatherEditProc(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam)   
	...
	switch (message)
		{
		case WM_INITDIALOG:             		/* message: initialize dialog box */
			sprintf(buffer,"%d",(int)(((WeatherClass*)TheWeather)->Temp));
			SetWindowText(GetDlgItem(hDlg,IDC_WD_TEMPVAL),buffer);
			head = (int) (((WeatherClass*)TheWeather)->WindHeading * 180 / PI);
			head = head%360;
			sprintf(buffer,"%d",head);
			SetWindowText(GetDlgItem(hDlg,IDC_WD_WHVAL),buffer);
			sprintf(buffer,"%5.1f",((WeatherClass*)TheWeather)->WindSpeed);
			SetWindowText(GetDlgItem(hDlg,IDC_WD_WSVAL),buffer);
			sprintf(buffer,"%d",((WeatherClass*)TheWeather)->GetCloudCover(CurX,CurY));
			SetWindowText(GetDlgItem(hDlg,IDC_WD_COVEREDIT),buffer);
			sprintf(buffer,"%d",((WeatherClass*)TheWeather)->GetCloudLevel(CurX,CurY));
			SetWindowText(GetDlgItem(hDlg,IDC_WD_LEVELEDIT),buffer);
			sprintf(buffer,"%d",((WeatherClass*)TheWeather)->TodaysTemp);
			SetWindowText(GetDlgItem(hDlg,IDC_WD_FORCASTTEMP),buffer);
			sprintf(buffer,"%d",((WeatherClass*)TheWeather)->TodaysWind);
			SetWindowText(GetDlgItem(hDlg,IDC_WD_FORCASTSPEED),buffer);
			sprintf(buffer,"%d",((WeatherClass*)TheWeather)->TodaysBase);
      		...
      		switch (LOWORD(wParam))
				...
				case IDC_WD_WHVAL:
					...
					if (HIWORD(wParam) == EN_KILLFOCUS)
						{
						head = i;
						((WeatherClass*)TheWeather)->WindHeading = (float)(i*PI/180.0F);
						}
					break;
				case IDC_WD_TEMPVAL:
					...
					i = atoi(buffer);
					if (HIWORD(wParam) == EN_KILLFOCUS)
						((WeatherClass*)TheWeather)->Temp = (float) i;
					break;
				default:
					break;
				}					
