...
void ShowPlayerBubble (MapData md, HDC DC, int show_ok)
	...
	if (PBubbleDrawn)
		{
		SetROP2(DC, R2_NOT);
		DisplaySideRange(DC,0,(short)(POSX(PBx)+(md->CellSize>>1)),(short)(POSY(PBy)+(md->CellSize>>1)),bubble_size*md->CellSize);
		DisplaySideRange(DC,0,(short)(POSX(PBx)+(md->CellSize>>1)),(short)(POSY(PBy)+(md->CellSize>>1)),1*md->CellSize);
	...
	if (show_ok)
		{
		SetROP2(DC, R2_NOT);
		DisplaySideRange(DC,0,(short)(POSX(x)+(md->CellSize>>1)),(short)(POSY(y)+(md->CellSize>>1)),bubble_size*md->CellSize);
		DisplaySideRange(DC,0,(short)(POSX(x)+(md->CellSize>>1)),(short)(POSY(y)+(md->CellSize>>1)),1*md->CellSize);
		SetROP2(DC, R2_COPYPEN);
		PBubbleDrawn = TRUE;
		PBx = x;
		PBy = y;
...
void ChangeCell(GridIndex X, GridIndex Y)
	...
	switch (EditMode)
		...
		case 4:
			...
			break;
		case 5:
			((WeatherClass*)TheWeather)->SetCloudCover(X,Y,DrawWeather);
			GetClientRect(md->hMapWnd,&r);
			InvalidateRect(md->hMapWnd,&r,FALSE);
			DC = BeginPaint(md->hMapWnd, &ps);		 
			if (PBubble)
			...
			break;
		case 6:
			((WeatherClass*)TheWeather)->SetCloudLevel(X,Y,DrawWeather);
			GetClientRect(md->hMapWnd,&r);
			InvalidateRect(md->hMapWnd,&r,FALSE);
			DC = BeginPaint(md->hMapWnd, &ps);		 
			if (PBubble)
...
void FindBorders(MapData md)
	...
	md->CULY=(windim.bottom-windim.top-ysize);
...
//	if (SuspendDraw)
	...
	if (RefreshAll)
		...
		{
		NFX=(rect->left/CellSize)+md->FX;
		NFY=(rect->top/CellSize)+md->FY;
		NLX=(rect->right/CellSize)+md->FX+1;
		NLY=(rect->bottom/CellSize)+md->FY+1;
		FreshDraw = FALSE;
		}
	else
		{
...
*/
			...
			pt.y += coffy;
			xPos = (pt.x-MainMapData->WULX-MainMapData->CULX);		// Get position relative to client area
			yPos = (pt.y-MainMapData->WULY-MainMapData->CULY);			
			CurX = (xPos+MainMapData->PFX)/MainMapData->CellSize;	// Get grid location in Campaign terms
			CurY = (MainMapData->PLY-1-yPos)/MainMapData->CellSize;
			if (fwKeys == MK_LBUTTON && Drawing)
				ChangeCell(CurX,CurY);
			coffx = xPos%CellSize;			// Find our offset, if any
			coffy = yPos%CellSize;
			xn = pt.x-coffx;					// Find new grid location to snap to.
			yn = pt.y-coffy;
   			SetCursor(hCur);
		...
		case WM_RBUTTONDOWN:
			...
			switch (EditMode)
				...
				case 4:
					...
					break;
				case 5:
					DrawWeather = ((WeatherClass*)TheWeather)->GetCloudCover(CurX,CurY);
					break;
				case 6:
					DrawWeather = ((WeatherClass*)TheWeather)->GetCloudLevel(CurX,CurY);
					break;
				case 7:
					StartObjectiveEdit();
					break;
...
//						GlobWP = WPUnit->AddWPAfter(w,X,Y,0,0,0.0F,0,WP_NOTHING);
		...
		case 139:
			switch(Mode)
				...
				case 5:
				case 6:
					i = ((WeatherClass*)TheWeather)->GetCloudLevel(X,Y);
					i++;
					((WeatherClass*)TheWeather)->SetCloudLevel(X,Y,i);
					break;
				default:
					break;
				}
