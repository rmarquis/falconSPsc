...
void FireControlComputer::AirAirMode(void)
{
	...
	bool bCageSound = false ; // Marco - set if there is a target under the diamond but missile slave/caged and radar not locked up
 ...
 if (!(Sms->curWeaponType == wtAim120 || Sms->curWeaponType ==wtAim9)) return;//me123
	...
	if (Sms->curWeapon)
	{
		...
		if (theMissile->launchState == MissileClass::PreLaunch)
		{
			...
			if (Sms->curWeaponType == wtAim9 && missileSpotScanCmd && Sms->curWeapon &&
				((CampBaseClass*)Sms->curWeapon)->GetSPType() != SPTYPE_AIM9P)
			{
			...
			if(Sms->curWeapon && Sms->curWeaponType == wtAim9 &&
				((CampBaseClass*)Sms->curWeapon)->GetSPType() == SPTYPE_AIM9P)
			{
				if(!theMissile->isSpot)
			...
			if ((subMode == Aim9 || masterMode == Dogfight) && Sms->curWeaponType == wtAim9)
			{
...
// 2001-01-31 MODIFIED BY S.G. SO IN HMS PLANE, UNCAGED IR MISSILE DOESN'T GO FIND A TARGET BY THEMSELF.
// THIS IS FOUND BY QUERYING BIT 20000000 OF Flags OF THE VEHICLE. A 1 THERE MEANS THIS VEHICLE IS HMS EQUIPPED AND SHOULD SKIP THIS CODE
				VehicleClassDataType	*vc	= (VehicleClassDataType *)Falcon4ClassTable[platform->Type() - VU_LAST_ENTITY_TYPE].dataPtr;
				if (!playerFCC || ( // don't do this check for ai
					vc && (!(vc->Flags & 0x20000000) || 
					OTWDriver.GetOTWDisplayMode() == OTWDriverClass::Mode2DCockpit) && // JB 010712 Normal behavior for the 2d cockpit
					!theMissile->isCaged)
					)
				{
...
//							theMissile->SetSeekerPos (&missileSeekerAz, &missileSeekerEl);
				...
				if(playerFCC && (Sms->GetCoolState() != SMSClass::COOL && Sms->GetCoolState() != SMSClass::WARMING) && theMissile->targetPtr &&
					Sms->curWeapon && Sms->curWeaponType == wtAim9 && (((CampBaseClass*)Sms->curWeapon)->GetSPType() != SPTYPE_AIM9P))
				{
					theMissile->DropTarget() ;
					irSig = 0.0;
				}
				...
				if (playerFCC)//me123 
				{
					...
					if (g_bRealisticAvionics && !irSig)
					{
						...
						if (FindGroundIntersection (pitch, yaw, &tmpX, &tmpY, &tmpZ))
						{
							...
							{
								F4SoundFXSetDist (SFX_AIM9_ENVIRO_GND, 0, 0.0f, aim9Pitch);
							}
						}
						else
						{
							...
							if (OTWDriver.DisplayInCockpit ())
							{
								F4SoundFXSetDist (SFX_AIM9_ENVIRO_SKY, 0, 0.0f, aim9Pitch);
							}
						}
					}
...
//							MonoPrint ("%f\n", lastIrSig);
							...
							//F4SoundFXSetDist (SFX_GROWL, 0, 0.0f, lastIrSig / 4.5F);
							F4SoundFXSetDist (SFX_NO_CAGE, 0, 0.0f, lastIrSig / 16);
						}
					}
...
//							MonoPrint ("%f\n", lastIrSig);
			...
			{
				missileTarget   = TRUE;
				missileRMax   = theMissile->GetRMax (-platform->ZPos(), platform->Vt(),
				targetPtr->localData->az, targetPtr->BaseData()->Vt(),
					...
					))
				{
					...
					float overtake = MISSILE_SPEED +  (-targetPtr->BaseData()->ZPos()/1000.0f * MISSILE_ALTITUDE_BONUS)+ 
									targetPtr->BaseData()->Vt()* (float)cos(targetPtr->localData->ataFrom);
					overtake = overtake + ((platform->Vt() * FTPSEC_TO_KNOTS - 150.0f)/2 );//me123 platform speed bonus // JB 010215 changed from 250 to 150
				...
				missileActiveRange = theMissile->GetActiveRange (-platform->ZPos(), platform->Vt(), targetPtr->localData->ataFrom, 0.0F, targetPtr->localData->range);
				missileActiveTime  = theMissile->GetActiveTime  (-platform->ZPos(), platform->Vt(), targetPtr->localData->ataFrom, 0.0F, targetPtr->localData->range);
				missileTOF         = theMissile->GetTOF(-platform->ZPos(), platform->Vt(),targetPtr->localData->ataFrom, targetPtr->BaseData()->Vt(), targetPtr->localData->range);				
				//LRKLUDGE
				missileRMin   = (platform->Vt()+targetPtr->BaseData()->Vt() * ((float)cos(targetPtr->localData->ataFrom)))*2.05f + 1400 +
				...
				if (g_nRNESpeed)
				{
					missileRneMax = missileRMax * 1500 / (1500.0F + platform->Vt() + targetPtr->BaseData()->Vt() * (float)cos(targetPtr->localData->az));
					missileRneMax = missileRneMax * (1500.0F + platform->Vt() + -(targetPtr->BaseData()->Vt() + g_nRNESpeed * KNOTS_TO_FTPSEC)) / 1500.0F;
			...
			else if (!theMissile->isCaged)
			{
				...
				{
					missileTarget = TRUE;
					missileActiveRange = theMissile->GetActiveRange (-platform->ZPos(), platform->Vt(),
						theMissile->targetPtr->localData->ataFrom, 0.0F, theMissile->targetPtr->localData->range);
					missileActiveTime  = theMissile->GetActiveTime  (-platform->ZPos(), platform->Vt(),
						theMissile->targetPtr->localData->ataFrom, 0.0F, theMissile->targetPtr->localData->range);
					missileTOF         = theMissile->GetTOF(-platform->ZPos(), platform->Vt(),
						theMissile->targetPtr->localData->ataFrom, theMissile->targetPtr->BaseData()->Vt(), theMissile->targetPtr->localData->range);
					theMissile->RunSeeker();
				}
	...
	if (missileTOF > 0.0F) 
	{
		...
		if (theMissile&&targetPtr)
		lastmissileActiveTime = theMissile->GetActiveTime  (-platform->ZPos(), platform->Vt(),
						targetPtr->localData->ataFrom, 0.0F, lastMissileShootRng);
		else lastmissileActiveTime = -1.0f;

		if (targetPtr) 
			targetspeed = targetPtr->BaseData()->Vt();
		else 
			targetspeed = 0.0f;
