...
SimObjectType *systemTarget;
   ...
   if (theMissile && (MaverickDisplayClass*)(theMissile->display) && Sms->CurHardpoint() >= 0)	
   {
      ...
      if (!playerFCC || ((PlayerOptions.GetAvionicsType() != ATRealistic && PlayerOptions.GetAvionicsType() != ATRealisticAV) && (subMode == SLAVE)))
      {
         ...
         else if (preDesignate)
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  groundPipperEl += cursorYCmd * g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime;
                  groundPipperAz += cursorXCmd * g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime;
            ...
            else
            {
               if (cursorXCmd || cursorYCmd)
               {
									...
									// directly the designated point. A non-orthogonal rotation of the co-ordinate system is necessary 	
				   {
						...
						{
							 deltaX = cursorYCmd * 50000.0F * dz/(cosalpha*cosalpha) * (0.03F/4000) * g_fCursorSpeed * (MAVERICK_SLEW_RATE/2.0F) * SimLibMajorFrameTime; // calibrated for 4000 ft high and 10° pitch, (cos^2 10) = 0.03; "dx = z/(cos^2 alpha)*dalpha"
							 deltaY = cursorXCmd * 12000.0F * range/21000.0F * g_fCursorSpeed * (MAVERICK_SLEW_RATE/2.0F) * SimLibMajorFrameTime; // calibrated for 21000 ft range
						 }
						else
						{
							 deltaX = cursorYCmd * 50000.0F * dz/(cosalpha*cosalpha) * (0.03F/4000)* g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime; // calibrated for 4000 ft high and 10° pitch, (cos^2 10) = 0.03
							 deltaY = cursorXCmd * 12000.0F * range/21000.0F * g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime;  // calibrated for 21000 ft range
						}
...
// 2002-04-22 MN when setting back to SOI_HUD, set designateCmd to false. This way slewing continues at the last position of the MAV SOI
				   ...
				   else  // old SLEW Code
				   {
						...
						if(theDisplay->CurFOV() < (3.5F * DTR))
						{
							pitch += cursorYCmd * g_fCursorSpeed * (MAVERICK_SLEW_RATE/2) * SimLibMajorFrameTime;
							yaw += cursorXCmd * g_fCursorSpeed * (MAVERICK_SLEW_RATE/2) * SimLibMajorFrameTime;
						}
						else
						{
							pitch += cursorYCmd * g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime;
							yaw += cursorXCmd * g_fCursorSpeed * MAVERICK_SLEW_RATE * SimLibMajorFrameTime;
						}
         ...
         else
         {
            ...
            missileTOF    = theMissile->GetTOF(-platform->ZPos(), platform->Vt(), 0.0f, 0.0f, range);
         }
      }
   ...
   curWeapon = ((MissileClass*)Sms->curWeapon);
   ...
   while (curTarget)
	{
      if (fabs(curTarget->localData->az - yaw) < minDist &&
		  ...
		  curTarget->BaseData()->Vt() <= 60*KNOTS_TO_FTPSEC)	//MI Maverik lockup fix
      {
