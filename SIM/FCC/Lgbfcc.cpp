...
// END OF ADDED SECTION
      ...
      else
      {
         ...
         {
            if ((cursorXCmd || cursorYCmd) && !targetingPod->IsLocked())
			{
									...
									// directly the designated point. A non-orthogonal rotation of the co-ordinate system is necessary 	
				{
					...
					if(targetingPod->CurFOV() < 3.5F * DTR) 
					{
						...
						if(g_bRealisticAvionics)
						{
							...
							{
								deltaX = cursorYCmd * 50000.0F * dz/(cosalpha*cosalpha) * (0.03F/4000) * g_fCursorSpeed * (LASER_SLEW_RATE /4.0F) * SimLibMajorFrameTime; // calibrated for 4000 ft high and 10° pitch, (cos^2 10) = 0.03; "dx = z/(cos^2 alpha)*dalpha"
								deltaY = cursorXCmd * 12000.0F * range/21000.0F * g_fCursorSpeed * (LASER_SLEW_RATE /4.0F) * SimLibMajorFrameTime; // calibrated for 21000 ft range
							}
						}
						else
						{
					...
					else
					{
						...
						if(g_bRealisticAvionics)
						{
							if(targetingPod->IsSOI())
							{
								deltaX = cursorYCmd * 50000.0F * dz/(cosalpha*cosalpha) * (0.03F/4000)* g_fCursorSpeed * LASER_SLEW_RATE  * SimLibMajorFrameTime; // calibrated for 4000 ft high and 10° pitch, (cos^2 10) = 0.03
								deltaY = cursorXCmd * 12000.0F * range/21000.0F * g_fCursorSpeed * LASER_SLEW_RATE  * SimLibMajorFrameTime;  // calibrated for 21000 ft range
							}
						}
						else
						{

							deltaX = cursorYCmd * 50000.0F * dz/(cosalpha*cosalpha) * (0.03F/4000) * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime; // calibrated for 4000 ft high and 10° pitch, (cos^2 10) = 0.03; "dx = z/(cos^2 alpha)*dalpha"
							deltaY = cursorXCmd * 12000.0F * range/21000.0F * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime; // calibrated for 21000 ft range
						}
					}
				...
				else // old code
				{
					...
					if (targetingPod->CurFOV() > 3.5F * DTR)
					{
						...
						if(g_bRealisticAvionics)
						{
							if(targetingPod->IsSOI())
							{
								pitch += cursorYCmd * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime;
								yaw += cursorXCmd * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime;
							}
						}
						else
						{
					...
					else
					{
						...
						if(g_bRealisticAvionics)
					   {
						   if(targetingPod->IsSOI())
						   {
								pitch += cursorYCmd * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime * 0.25F;
								yaw += cursorXCmd * g_fCursorSpeed * LASER_SLEW_RATE * SimLibMajorFrameTime * 0.25F;
							}
						}
						else
						{
...
float curMin, dx, dy;
   ...
   if (!targetPtr)
   {
      ...
      while (testObject)
      {
         ShiAssert( testObject->IsSim() );		// Is this true???  If so, see below    vvvv   (and in Lgbfcc.cpp)
...
// 2001-11-01 Added IsDead || IsExploding check by M.N. - we don't want to bomb something that is already destroyed
      ...
      if (closestObj)
      {
		 ...
		 #endif
		 tmpTarget->Reference( SIM_OBJ_REF_ARGS );
         ...
         tmpTarget->localData->droll = groundDesignateDroll = (float)atan2 (sin(groundDesignateAz), sin(groundDesignateEl));
         ...
         targetingPod->SetDesiredTarget(tmpTarget);
 		 tmpTarget->Release( SIM_OBJ_REF_ARGS );
      }
...
{
	if(SimDriver.playerEntity && SimDriver.playerEntity->Sms->MasterArm() != SMSBaseClass::MasterArmState::Arm)
		return;
