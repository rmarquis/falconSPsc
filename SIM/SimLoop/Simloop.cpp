...
void SimulationLoopControl::StartSim(void)
{
	...
	(
		NULL,
		0,
		(unsigned long(__stdcall*)(void*))StartingGraphicsWrapper,
		0,
		0,
		&value
	);
...
void SimulationLoopControl::StopSim(void)
{
	...
	// This blocks until the thread is gone...
...
void SimulationLoopControl::StopGraphics(void)
{
	// Don't stop until we're sure we're running
...
#error Pay special attention to rounding mode and precision effects on floating point ops!
	...
	gInputTime = GetTickCount();
...
#ifdef CHECK_PROC_TIMES
		gInputTime = GetTickCount() - gInputTime;
		if(gInputTime > gMaxInputTime)
			gMaxInputTime = gInputTime;
		
		gBubbleTime = GetTickCount();
#endif

		// Rebuild the bubble here
		if (gRebuildBubbleNow || (static_cast<CampaignTime>(vuxRealTime - lastBubbleTime) > static_cast<CampaignTime>(BUBBLE_REBUILD_TIME*CampaignSeconds)))
		{
			int forced = 0;
			if (gRebuildBubbleNow== 2) forced = 1;
			if ((static_cast<CampaignTime>(vuxRealTime - lastBubbleTime) > static_cast<CampaignTime>(BUBBLE_REBUILD_TIME*CampaignSeconds)))forced =0;
...
#ifdef CHECK_PROC_TIMES
		gBubbleTime = GetTickCount() - gBubbleTime;
		if(gBubbleTime > gMaxBubbleTime)
			gMaxBubbleTime = gBubbleTime;
#endif
...
#ifdef DEBUG
		real_delta = delta = min(500, (vuxRealTime - lastStartTime));
#else
		...
		(
			(FalconLocalGame) &&
			(
				((vuPlayerPoolGroup) && (FalconLocalGame->Id() != vuPlayerPoolGroup->Id())) ||
				(!vuPlayerPoolGroup)
			)
		)
		{
			if ((!FalconLocalGame->IsLocal()) && (lastTimingMessage > 0))
			{
				...
				else if (y <= lookahead + 2000)//  me123 changed this to ajust fluently
...
/*
					MonoPrint
					(
						"Remote Time Compression %d %d %d %d %d %d\n",
						y, 
						ratio,
						targetGameCompressionRatio,
						delta,
						lookahead,
						vuxDeadReconTime - vuxTargetGameTime
					);
*/
	...
	/*			if (tmpTime > vuxTargetGameTime)
				{
					if (vuxGameTime < vuxTargetGameTime)
					{
//						MonoPrint ("Too Far 1\n");
						tmpTime = vuxTargetGameTime;					// We don't want to advance time past here
					}
					else if (vuxGameTime > vuxTargetGameTime + lookahead)
					{
//						MonoPrint ("Too Far 2\n");
...
*/
				...
				//we are host
					...
					while (sess && !flying )
		  			 ...
		  			 if (((FalconSessionEntity*)vuDatabase->Find(sess->Id()))->GetFlyState () ==  FLYSTATE_FLYING||
						  ((FalconSessionEntity*)vuDatabase->Find(sess->Id()))->GetFlyState () ==  FLYSTATE_WAITING||
						  ((FalconSessionEntity*)vuDatabase->Find(sess->Id()))->GetFlyState () ==  FLYSTATE_LOADING) 
						  flying = TRUE;
		  			 sess = Sessioniter.GetNext ();
		  			 }

					 if (flying && gameCompressionRatio > 4 
						&& (gCommsMgr && gCommsMgr->Online())) // JB 010524
		  			 {
			...
			if (FalconLocalSession->GetFlyState () !=  FLYSTATE_FLYING && gCompressTillTime && tmpTime > gLaunchTime + 1000)
			{
			...
			vuxGameTime = tmpTime;
			//ShiAssert(vuxGameTime + SimLibMajorFrameTime*SEC_TO_MSEC >= SimLibElapsedTime);
...
#ifdef DO_SIM_TIME
		// Run one sim cycle (possibly multiple internal time steps)
		gSimTime = GetTickCount ();
#endif
#ifdef CHECK_PROC_TIMES
	gDDTime = GetTickCount();
#endif
	FalconEntity::DoSimDirtyData();
#ifdef CHECK_PROC_TIMES
	gDDTime = GetTickCount() - gDDTime;
	if(gDDTime > gDDMaxTime)
		gDDMaxTime = gDDTime;
#endif
...
#ifdef DO_SIM_TIME
		gSimTime = GetTickCount () - gSimTime;
#endif
...
//				PrintMemUsage ();
				...
				if (now_vr - last_vr > 250)
				{
					PostMessage (FalconDisplay.appWin, FM_CHECK_FOR_VR_INPUT, 0, 0);
					last_vr = now_vr;
				}
#endif
				...
				{
					if ((PlayerOptions.UnlimitedFuel()) && (vuxRealTime > first_frame_time + 10 * 60 * 1000))
					{
						OTWDriver.Timeout ();
					}
				}
				...
				gAveSimGraphicsTime = (gAveSimGraphicsTime * 7 + gSimTime + gGraphicsTimeLast)/8;
#ifndef DAVE_DBG
#ifdef DEBUG
				if(gameCompressionRatio)
				{
					if (!gSimTime) gSimTime ++;
					if (!gGraphicsTimeLast) gGraphicsTimeLast ++;
					if (!gAveSimGraphicsTime) gAveSimGraphicsTime ++;

/*
					MonoPrint ("%4d.%02d %4d.%02d %4d.%02d\n",
						1000 / gSimTime, (100000 / gSimTime) % 100,
						1000 / gGraphicsTimeLast, (100000 / gGraphicsTimeLast) % 100,
						1000 / gAveSimGraphicsTime, (100000 / gAveSimGraphicsTime) % 100
					);
*/
				}
#endif
	...
	if(gameCompressionRatio)
	{
		if(gSimTime > gMaxSimTime)
			gMaxSimTime = gSimTime;
		if(gGraphicsTimeLast > gMaxGraphicsTime)
			gMaxGraphicsTime = gGraphicsTimeLast;
		if(gCampTime > gMaxCampTime)
			gMaxCampTime = gCampTime;

		MonoPrint("Graphics: %4d  Sim: %4d  Terrain: %4d  GrObjs: %4d Objs: %4d\n\n", gGraphicsTimeLast, gSimTime, terrTime - objTime, objTime, gGraphicsTimeLast - terrTime);
		//MonoPrint("GraphicsMax: %4d  SimMax:   %4d  BubbleMax + CampMax: %5d  DDMax:     %3d\n\n", gMaxSimTime, gMaxGraphicsTime, gMaxBubbleTime + gMaxCampTime, gDDMaxTime);
	}
#endif
		...
		// Sleep (20); // Campaign gets fed some food
	}

	//*((unsigned int*)0) = 0;

	//MonoPrint ("Sim Loop Exited\n");
}
...
void SimulationLoopControl::StartLoop(void)
{
	...
	while (1)
	{
		WaitForSingleObject (wait_for_start_graphics, 0xFFFFFFFF);
...
///////////////////////////////////////////////////////////////////////////////
		...
		{			
			...
			// Ask the game to deag. all entities for me
			VuTargetEntity* target = (VuTargetEntity*) vuDatabase->Find(FalconLocalGame->OwnerId());
			if (!target->IsLocal ())
			{
				FalconSimCampMessage	*msg = new FalconSimCampMessage (flight->Id(), target);
			...
			if (!FalconLocalSession->Camera(0)) // JB New MP code
...
///////////////////////////////////////////////////////////////////////////////
	...
	//		player = GameManager.AttachPlayerToVehicle (FalconLocalSession, flight, FalconLocalSession->GetAircraftNum(), FalconLocalSession->GetPilotSlot());
...
///////////////////////////////////////////////////////////////////////////////
			...
			while ((gLeftToDeaggregate) && (delayCounter) && (SimDriver.playerEntity) && (!(SimDriver.playerEntity->IsLocal ())))
			{
				Sleep(100);
				delayCounter--;
			}
...
///////////////////////////////////////////////////////////////////////////////
			...
			ShiAssert ( FalconLocalSession->GetPlayerEntity() );
...
//		MonoPrint("Requesting campain to do a final bubble rebuild\n");
		...
		while (!CampaignAllAsleep()) {
			...
			// 2002-01-02 ADDED BY S.G. We are asked to sleep so keep doing bubble rebuild until all sim objects are sleeping
