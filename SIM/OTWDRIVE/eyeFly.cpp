...
float tmpX, tmpY, tmpZ;
   ...
   if (IO.ReadDigital(0))
   {
      p = Math.DeadBand(IO.ReadAnalog(0), -0.05F, 0.05F) * (1.0F / 0.95F);
      p = max ( min (p, 1.0F), -1.0F) * dT * 1.0f;
      q = Math.DeadBand(IO.ReadAnalog(1), -0.05F, 0.05F) * (1.0F / 0.95F);
      q = max ( min (q, 1.0F), -1.0F) * dT * 1.0f;
      r = -Math.DeadBand(-IO.ReadAnalog(3), -0.05F, 0.05F) * (1.0F / 0.95F);
      r = max ( min (r, 1.0F), -1.0F) * dT * 1.0f;

      if (IO.AnalogIsUsed (2))
      {
         //throttle = (1.0F - Math.DeadBand(IO.ReadAnalog(2), -0.03F, 0.03F)) * 0.5F;
		 throttle = 0.66666F * IO.ReadAnalog(2);
         throttle *= throttle;
      }
      else
         throttle = 0.25F;
