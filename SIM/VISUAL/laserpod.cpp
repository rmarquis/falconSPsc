   ...
   if (privateDisplay)
   {
      ...
      if (hasTarget == TargetLocked)
      {
		  ...
		  else
		  {
			  ...
			  if(curFOV > 3.0F * DTR)
				  DrawFOV(display);
			  if(SimDriver.playerEntity && SimDriver.playerEntity->FCC && !SimDriver.playerEntity->FCC
	  ...
	  if(g_bRealisticAvionics)
	  {
		  ...
		  {
			  if(SimDriver.playerEntity->FCC->designateCmd || IsLocked())
				  DrawBox(display);
		  }
	  }
   }
...
Tpoint cameraPos = {13.0F, -3.0F, 3.0F};
...
void LaserPodClass::SetDesiredTarget (SimObjectType *newTarget)
{
	...
	{
		msg = new FalconLaserDesignateMsg (platform->Id(), FalconLocalSession);
...
if (!theObject) return retval;//me123 ctd
   ...
   {
      if (theObject->sensorArray && theObject->sensorArray[i] && theObject->sensorArray[i]->Type() == SensorClass::Visual)
      {
