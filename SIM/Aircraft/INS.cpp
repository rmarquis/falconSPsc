...
void AircraftClass::RunINS(void)
{
	...
	{
		if((OnGround() && Kias() <= 2.0F && !INS60kts) ||
	...
	if(Kias() >= 60 && OnGround() && INSState(INS_AlignNorm))
...
void AircraftClass::CalcINSDrift(void)
{
	...
	*/
	...
	INSTimeDiff = vuxGameTime - INSAlignmentStart;
...
void AircraftClass::CalcINSOffset(void)
{
	if(OTWDriver.pCockpitManager && OTWDriver.pCockpitManager->mpIcp)
	{
		...
		float feetperdeg = circumfence * 3.281/360;
