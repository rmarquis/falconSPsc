...
void AircraftClass::OnGroundInit(SimInitDataClass* initData)
{
	...
	// Find our airbase;
	ent = ((Flight)(initData->campUnit))->GetUnitAirbase();
	...
	if ((ent) && (ent->IsTaskForce()))
	{
		...
		if ( !ent->IsAggregate() &&
			 ent->GetComponents() &&
			 (carrier = ent->GetComponentLead()) != NULL )
		{
			psi = carrier->Yaw();
	...
	af->initialPsi = psi;
	af->initialMach = 0.0F;
	af->initialZ    = initData->z - 6.0F;
	SetPosition (initData->x, initData->y, initData->z);
	SetYPR(psi, 0.0F, 0.0F);
	// SetDelta ((nextX-initData->x)/30.0F, (nextY-initData->y)/30.0F, 0.0F);
...
BOOL AircraftClass::LandingCheck( float noseAngle, float impactAngle, int groundType )
{
	...
	{
		F4SoundFXSetPos(af->auxaeroData->sndTouchDown, TRUE, XPos(), YPos(), ZPos(), 1.0f );
	...
	else if(af->vt * impactAngle < sinkRate * 3.0F *(1.0F - (!af->IsSet(AirframeClass::OverRunway) && !onFlatFeature && groundType != COVERAGE_ROAD) * 0.5F) && af->gearPos > 0.8F)
	{
		...
		F4SoundFXSetPos( af->auxaeroData->sndTouchDown, TRUE, XPos(), YPos(), ZPos(), 1.0f );
		F4SoundFXSetPos( SFX_IMPACTG3 + rand() % 4, TRUE, XPos(), YPos(), ZPos(), 1.0f );
		...
		{				    
			...
			if(!af->IsSet( AirframeClass::GearBroken ))
			{
				...
				// gear breaks sound
				F4SoundFXSetPos( SFX_BIND, TRUE, XPos(), YPos(), ZPos(), 1.0f );
	...
	{		
		...
		if(!IsSetFalcFlag(FEC_INVULNERABLE))
		{
			...
			{				    
				...
				mFaults->SetFault(FaultClass::gear_fault,
					FaultClass::ldgr, FaultClass::fail, this == SimDriver.playerEntity);
				
				// gear breaks sound
				F4SoundFXSetPos( af->auxaeroData->sndWheelBrakes, TRUE, XPos(), YPos(), ZPos(), 1.0f );
...
*/
void
AircraftClass::CheckPersistantCollision( void )
{
	int i;
