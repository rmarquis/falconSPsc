...
float breakRange;
   ...
   if (curMode != MergeMode)
   {
      ...
      float dx, dy;
         ...
         breakRange = ((targetPtr->BaseData()->Kias() * self->Kias())) * //me123 
...
float curRoll = self->Roll();
   ...
   if (lastMode != MergeMode)
   {
      ...
      {
         case CanLevelTurn:
            if ((mnverFlags & CanOneCircle) && (self->Kias() < cornerSpeed))//me123
            {
         ...
         case CanSlice:
			...
			{
               newroll = 135.0F * DTR;
		      MachHold (cornerSpeed, self->Kias(), FALSE);//me123
			}
            else
			{
               newroll = -135.0F * DTR;
         ...
         case CanLevelTurn | CanSlice:
            // level turn or slice?
            if ((self->Kias() > cornerSpeed) && -self->ZPos() >3000.0f )//me123
            {
               // Level Turn
               if ((mnverFlags & CanOneCircle) && (self->Kias() < cornerSpeed))
               {
                  // One Circle, turn away from the target
                  newroll = (targetData->az > 0.0F ? -90.0F * DTR : 90.0F * DTR);
  			      MachHold (0.7f* cornerSpeed, self->Kias(), FALSE);//me123 addet *0.4
               }
               else
               {
                  // Two Circle, turn towards the target
         ...
         case CanLevelTurn | CanUseVertical:
            ...
            if (self->Kias() < cornerSpeed* 1.2)//me123
            {
               // Level Turn
               if ((mnverFlags & CanOneCircle) && (self->Kias() < cornerSpeed))
               {
                  // One Circle, turn away from the target
                  newroll = (targetData->az > 0.0F ? -90.0F * DTR : 90.0F * DTR);
				  MachHold (0.7f*cornerSpeed, self->Kias(), FALSE);
               }
               else
               {
                  // Two Circle, turn towards the target
         ...
         case CanLevelTurn | CanSlice | CanUseVertical:
            ...
            if ((self->Kias() < cornerSpeed*0.7)&& -self->ZPos() >3000.0f)//me123
            {
            ...
            else if ((self->Kias() < cornerSpeed * 1.2))//me123
            {
               // Level Turn
               if ((mnverFlags & CanOneCircle) && (self->Kias() < cornerSpeed))
               {
            ...
            {
               newroll = 0.0F;
			   MachHold (cornerSpeed* 1.2f, self->Kias(), TRUE);
               // Full burner for the pull
            }
         break;
      }
...
void DigitalBrain::AccelCheck(void)
{
	...
	{
	   if ((self->Pitch() > 50.0F * DTR && self->Kias() < cornerSpeed * 0.4F) ||//me123 150kias
		  (self->Pitch() > 0.0F * DTR && self->Kias() < cornerSpeed * 0.35F))//me123 100 kias
	   {
		  AddMode (AccelMode);
	   }
	   else if (curMode == AccelMode && self->Kias() < cornerSpeed * 0.447F && 
		   self->Pitch() > 0.0F * DTR && self->Kias())//me123 180kias
	   {
		  AddMode (AccelMode);
	   }
	}
