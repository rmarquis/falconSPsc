...
void DigitalBrain::FollowWaypoints(void)
{
   ...
   if (self->curWaypoint == NULL)
   {
	...
	if (isWing && (atcstatus != noATC)) {						// if we are a wingman and we are taking off or landing
		mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;			// don't follow formation
	}
	else if (isWing && !pobj) {									// if we are a wingman and we can't find the leader's pointer
		mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;			// don't follow formation
	}
	else if (isWing && (pobj && pobj->OnGround())) {			// if we are a wingman and our leader is on the ground
		mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;			// don't follow formation
		mLeaderTookOff = FALSE;										// reset leader take off flag

		if (self->curWaypoint->GetWPAction() == WP_ASSEMBLE && (onStation == Arrived || onStation == Stabalizing || onStation == OnStation)) { // if we are at the assemble point
			if(SimLibElapsedTime < self->curWaypoint->GetWPDepartureTime() + 300000) { // if we have time to kill
				AddMode (LoiterMode);																	// hang out for a while
			}
		}
	}
	else if(isWing && mLeaderTookOff == FALSE) {				// if i'm a wing and we think that the leader hasn't taken off
	...
	if (isWing && (self->curWaypoint->GetWPAction() == WP_ASSEMBLE) && (onStation == Arrived || onStation == Stabalizing || onStation == OnStation))
	{
		SimBaseClass* pobj = self->GetCampaignObject()->GetComponentEntity(0);

		if(pobj->OnGround())	// if the leader hasn't taken off
		{
			mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;
		
			if(SimLibElapsedTime < self->curWaypoint->GetWPDepartureTime() + 300000) {// wait 3 minutes for
				AddMode (LoiterMode);
			}
		}
		else {
   ...
   if ( (self->curWaypoint->GetWPFlags() & WPF_IP ) &&
   	    agDoctrine == AGD_NONE )
   {
		if(self != SimDriver.playerEntity || SimDriver.playerEntity->FCC->GetStptMode() == FireControlComputer::FCCWaypoint) { // VWF 5/25/98 for E3
...
// switch (self->curWaypoint->GetWPAction())
   switch (self->curWaypoint->GetWPAction() == WP_NOTHING ? self->curWaypoint->GetWPRouteAction() : self->curWaypoint->GetWPAction())
   {
      ...
      case WP_BOMB:
      case WP_SAD:
	  // edg: added CASCP, SEAD (is this right?)
      case WP_SEAD:
      case WP_CASCP:
      case WP_RECON:
	      ...
	      FlightClass					*flight;
            ...
            onStation = Arrived;
	         ...
	         else 
	         {
		         ...
		         if(!flight->IsFlight())
		         {
			         flight = SimDriver.FindTanker(SimDriver.playerEntity);
		         }
	         }
...
float				rangeFactor;
   ...
   if ( agDoctrine == AGD_NONE )
   {
   	self->curWaypoint->GetLocation (&trackX, &trackY, &trackZ);
   ...
   if (curMode != lastMode)
   {
	...
	/*---------------------------*/
	if(rng < (2000.0F * 2000.0F) || (onStation != NotThereYet) || (SimLibElapsedTime > self->curWaypoint->GetWPDepartureTime()))
	{
     // Should we repeat?
...
beyond our scope. */
										// mind the ! check here !!!
			if (onStation == OnStation || !(g_bAGTargetWPFix && self->curWaypoint->GetWPFlags() & WPF_TARGET && !missionComplete && missionClass == AGMission && curMode != RTBMode && curMode != LandingMode))
			{
...
//				if (onStation == OnStation || self->curWaypoint->GetNextWP() && self->curWaypoint->GetNextWP()->GetWPAction() != WP_LAND)
      ...
      if (ttg < 0.0F) {			// If we're late
         desSpeed = 700.0F;	// Knots, Go fast
		}
      else {
...
long timeDelta;
   ...
   if (curMode != lastMode)
   {
	...
	/* Reached the next waypoint */
	/*---------------------------*/
	if (rng < (3000.0F * 3000.0F) || (onStation != NotThereYet) ||
		SimLibElapsedTime > self->curWaypoint->GetWPDepartureTime())
	{
      // Should we repeat?
...
have not yet completed the mission, are not in RTB mode or landing mode, don't skip the target waypoint. */
   ...
   if (curMode == RTBMode)
	...
	{
		if (self->Kias() < 0.8F * cornerSpeed && onStation == Arrived)
      {
			AltitudeHold(-wpZ);
   ...
   else
   {
      ...
      /*--------------------------*/
      AltitudeHold(-trackZ);
      SetYpedal( desHeading * 0.05F * RTD * self->Vt()/cornerSpeed);
	...
	MachHold(desSpeed, self->Kias(), FALSE);
}
...
void DigitalBrain::SelectNextWaypoint(void)
{
   ...
   /*
   if (self->curWaypoint->GetWPAction() == WP_LAND || (atcstatus != noATC && atcstatus < lCrashed))
   {
	 
   	FalconATCMessage* atcMsg;
   	// Find our airbase
   	ObjectiveClass* theATC = (ObjectiveClass*)vuDatabase->Find(self->HomeAirbase());
      if (theATC)
      {
         atcMsg = new FalconATCMessage (theATC->Id(), FalconLocalGame);
         atcMsg->dataBlock.type = FalconATCMessage::RequestClearance;
         atcMsg->dataBlock.from = self->Id();
...
//            MonoPrint ("Why am I here (Digi GetBrain)\n");
   ...
   if (SimDriver.playerEntity != self && !isWing && self->curWaypoint->GetWPFormation() != mCurFormation)
   {
