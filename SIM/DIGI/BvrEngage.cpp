...
float engageRange;
   ...
   if (mpActionFlags[AI_ENGAGE_TARGET] != AI_AIR_TARGET && missionClass != AAMission && !missionComplete) { // not assigned a target, on a A2G mission that is not over yet...  // 2002-03-04 MODIFIED BY S.G. Use new enum type
	   ...
	   if (groundTargetPtr && agDoctrine != AGD_NONE && !threatPtr)
		   return; // Concentrate at hitting this bastard
...
*/	   // END OF ADDED SECTION 2002-03-04
   ...
   if (curMode != BVREngageMode)
   {
      ...
      if ((classPtr->vuClassData.classInfo_[VU_TYPE] == TYPE_AIRPLANE ||
           ...
           targetPtr->localData->range < engageRange &&
...
#endif
			  trackX = targetPtr->BaseData()->XPos();
			  trackY = targetPtr->BaseData()->YPos();
			  trackZ = min (targetPtr->BaseData()->ZPos() - 100.0F, -4000.0f);
...
//	int numChoices;
	...
	// 2002-02-10 ADDED BY S.G. For starter, I'll set the digiRadarMode to DigiTWS and leave it as such. Later on, it will depend on the BVR tactic.
	RadarClass* theRadar = (RadarClass*)FindSensor(self, SensorClass::Radar);
	if (theRadar) {
	...
	// END OF ADDED SECTION 2002-02-10

//me123 basic intercept choice
   // Look up intercept type for all A/C
// if (targetPtr->BaseData()->IsSim() && ((SimVehicleClass*)targetPtr->BaseData())->IsAirplane() )
...
/*		hisCombatClass = targetPtr->BaseData()->CombatClass(); // 2002-02-26 MODIFIED BY S.G. Removed the AircraftClass cast
		...
		numChoices = theIntercept->numIntercepts;
// 2002-02-20 MN added check for no choices -> no BVR
//me123 bvr engage also does defensive maneuvering
//the isue with awac flights ect atacking is hopefully fixed now by
//modifying the "Pdefensive" tactic.
		if (!numChoices)
		{
			bvrCurrTactic = BvrNoIntercept;
			return;
...
// OVERIDE SELFDEFENCE  AND MISSILE SUPPORT CODE
		...
		{
// supporting a missile
			if (SkillLevel() >= 3) bvrCurrTactic =BvrCrank;//return;
			else bvrCurrTactic = BvrPursuit ;
			bvrTacticTimer = SimLibElapsedTime + 3 * CampaignSeconds;
		}
		
		else if(spiked && targetData->range < MAR/2 && SkillLevel() >2)
		{
...
*/
	// ARE WE outnumbered
// 2002-02-19 MODIFIED BY S.G. GetCampaignObject of a non SIM object might yield to garbage. Fetch the campaign object if it's a sim only otherwise use 'as is'.
//	if (((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject()) 
//		targetstrength = ((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject()->NumberOfComponents();
//	if (self->GetCampaignObject()) // We're ALWAYS a SIM object so this test is not necessary and not valid anyway (see above)
	CampBaseClass *campBaseObj;
...
// END OF MODIFIED SECTION
	...
	ownstrength = self->GetCampaignObject()->NumberOfComponents();
	if (targetstrength > ownstrength) outnumbered = true;

	if (MAR && outnumbered) outstrengthed = true;
	if (MAR > TGTMAR) outranged = true;
...
// so try to avoid, but be aggresive if the bandits wanna play
		...
		{
			if (!outranged && (g_bUseAggresiveIncompleteA2G || (!g_bUseAggresiveIncompleteA2G && missionComplete))) // 2002-03-22 MODIFIED BY S.G. Needs to have g_bUseAggresiveIncompleteA2G or missionComplete set as well otherwise be defensive
			{
			...
			else
			{
				if (Isflightlead) AiSendCommand (self, FalconWingmanMsg::WMBox, AiFlight);
				int choice = rand()%(SkillLevel()+1);
				if (choice == 1 || choice == 0) bvrCurrProfile = PDefensive;
				else if (choice == 2) bvrCurrProfile = PDefensive;
				else if (choice == 3) bvrCurrProfile = PDefensive;
				else bvrCurrProfile = PDefensive;
			}
		}
		break;
	default:
...
{
	if ((bvractionstep != 0 || targetData->range > TGTMAR )&& WhoIsSpiked()>=15)
	bvractionstep = 0;
	else bvractionstep = 1;
	if (!HowManyHotnosed())bvractionstep = 2;
...
if ((!Isflightlead && !IsElementlead) && WhoIsSpiked()<7 && bvrCurrTactic == BvrFlyFormation) return;
	...
	{
		if ((bvractionstep != 0  && targetData->range > TGTMAR) && WhoIsSpiked()>7)
		bvractionstep = 0;
		else bvractionstep = 1;
	}
	   ...
	   case 2:// we have launched a weapon
		   {
			   ...
			   if (Isflightlead)
			   {
				bvrCurrTactic = BvrPump ;
				bvrTacticTimer = SimLibElapsedTime + 40 * CampaignSeconds;
				if (targetData->range > DOR || WhoIsSpiked() < 3) // pump til DOR  OR naked and then recomit
					bvractionstep = 1;
			   }
			   else if (IsElementlead)
			   {
...
if (!Isflightlead && !IsElementlead) return;
 if (WhoIsSpiked() || WhoIsHotnosed() )
	{		
		...
		if (Isflightlead && targetData->range < DOR + 10*NM_TO_FT)
		{
			if (targetData->range < DOR)
				{
				bvrCurrTactic = BvrNotch;
				bvrTacticTimer = SimLibElapsedTime + 15 * CampaignSeconds;
					short edata[3];
			...
			if (targetData->range < MAR + 1.0f*NM_TO_FT)
			{
				bvrCurrTactic = BvrPump;
		...
		if (spiked && targetData->range < MAR - MAR/3.0f)
...
{
// find MAR and DOR
//	if (((AircraftClass*)targetPtr)->DBrain())
//	MAR =  ((AircraftClass*)targetPtr)->DBrain()->maxAAWpnRange/2;
	MAR = 5.0f * NM_TO_FT;
	...
	{// we have Type ID.
		...
		if (!targetPtr->BaseData()->IsAirplane() || !((AircraftClass *)targetPtr->BaseData())->af || ((AircraftClass *)targetPtr->BaseData())->af->GetMaxMARIdedStart() == 0.0f) {
			...
			{
				// 2002-03-24 MODIFIED BY S.G. Reshuffled because -20000 is less than -5000 so it never went into the -18000 and -28000 tests
				MAR  = max(MAR,5.0f*NM_TO_FT) ;
/*				if (targetPtr->BaseData()->ZPos() <-5000.0f)
					MAR += 3.0f* NM_TO_FT;
				else if (targetPtr->BaseData()->ZPos() <-18000.0f)
					MAR += 5.0f* NM_TO_FT;
				else if (targetPtr->BaseData()->ZPos() <-28000.0f)
					MAR += 8.0f* NM_TO_FT;
*/
...
*/
	...
	{// we Don't have Type ID, we have SA on their Speed and altitude though since the ent is spotted
		...
		if (
			(targetPtr->BaseData()->Vt() * FTPSEC_TO_KNOTS > 300.0f ||
...
*/
		...
		else if (targetPtr->BaseData()->Vt() * FTPSEC_TO_KNOTS > 250.0f)
...
// ADDED BY S.G. SO DIGI PILOT SHOOTING SARH WONT GET SCARED RIGHT AWAY
	...
	if (missileFiredEntity && ((SimWeaponClass *)missileFiredEntity)->sensorArray[0]->Type() == SensorClass::RadarHoming)
		result = 1;
/*	if (missileFiredEntity && 
...
mlTrig trig;
   ...
   mlSinCos(&trig, az);
   trackX = self->XPos() + 4.0F*NM_TO_FT*trig.cos;
   trackY = self->YPos() + 4.0F*NM_TO_FT*trig.sin;

	static VU_TIME heighttimer = 0;

	if (heighttimer + 15 * CampaignSeconds < SimLibElapsedTime&&
	...
	if (!direction && bvrTacticTimer > SimLibElapsedTime + 5 * CampaignSeconds - SkillLevel()* CampaignSeconds&&
   ...
   /*----------------------------------------------------------*/
	 ...
	 if (SpikeCheck (self) && SpikeCheck (self) == targetPtr->BaseData() &&
...
void DigitalBrain::CrankManeuver(int direction)//me123
{
int retval = FALSE;
float nh1,nh2,az;
mlTrig trig;
   ...
   trackX = self->XPos() + 10.0F*NM_TO_FT*trig.cos;
   trackY = self->YPos() + 10.0F*NM_TO_FT*trig.sin;
	if (self->ZPos() < -3000.0f) trackZ = self->ZPos() -5000.0f;
	else trackZ = min (self->ZPos(), -3000.0f) ;
...
void DigitalBrain::StickandThrottle (float DesiredSpeed, float DesiredAltitude)
{
	...
	if (DesiredSpeed == -1)
	{
		DesiredSpeed = ((AircraftClass*)self)->af->GetOptKias(0)*1.05; // 2002-03-15 MODIFIED BY S.G. From 1.4 to 1.05. Plane will have trouble getting there and will burn too much fuel trying to
		MaxEnergyMode = 2;
	}
	if (DesiredSpeed == -2)
	{
...
//		DesiredSpeed = ((AircraftClass*)self)->af->MinVcas();
	...
	speeddifference = DesiredSpeed - self->Kias();

	if(self->Kias() < DesiredSpeed)
	{// we need speed nomatter what sell altitude...speed has priority here
	trackZ = min (self->ZPos() + (speeddifference*sensitivityDecent) , -4000.0f);
	}
	else
	{
...
//		trackZ = max (trackZ,-45000.0f);
	...
	AutoTrack (4.0f);
//	MachHold(DesiredSpeed*MaxEnergyMode, self->Kias(), TRUE);
	MachHold(DesiredSpeed, self->Kias(), TRUE); // 2002-03-14 S.G. Not sure what RIK was trying to do here but this 'MaxEnergyMode' makes MachHold go into overtime trying to go really really fast :-(
	ShiAssert(trackZ < 0.0F);
}
...
ShiAssert(trackZ < 0.0F);
	...
	//	SimpleTrack(SimpleTrackDist, 0.0F);
		...
		if (flightLead)
			SimpleTrackDistance(flightLead->Vt(), (float)sqrt(xft*xft+yft*yft));
		AutoTrack (4.0f);
}