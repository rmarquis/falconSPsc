...
// If not, continue to follow waypoints.
		   case RTBMode:
			   ...
			   if (theRadar)	// 2002-02-10 ADDED BY S.G. Defaults to RWS when not doing anything special
...
if (g_nShowDebugLabels & 0x01)
{
	...
	{
		if (SimDriver.playerEntity && self->GetCampaignObject() && self->GetCampaignObject()->GetIdentified(SimDriver.playerEntity->GetTeam()))
			strcat(label, "IDed");
		else
