...
{
	// 2002-02-10 ADDED BY S.G. Change our radarMode to digiSTT if we have been detected and we have a radar to start with (not sure if this one is required but better be safe than sorry)
	if (targetPtr && SimLibElapsedTime > radarModeTest) {
...
void DigitalBrain::AiExecBreakRL(void)
{
   ...
   trackX = self->XPos() + 1000.0F * trig.cos;
   trackY = self->YPos() + 1000.0F * trig.sin;
