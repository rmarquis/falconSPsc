...
{
float engageRange;
     engageRange = 8.0F * NM_TO_FT;
	 if (curMode == BVREngageMode) engageRange = 3.0F * NM_TO_FT;
 /* me123 let's make this 8nm nomater what

   if (IsSetATC(AceGunsEngage) || curMissile)
   {
      engageRange = 10.0F * NM_TO_FT;
   }
   else
   {
      engageRange = maxAAWpnRange;
   }
/*
...
//      if ( self->Kias() < cornerSpeed * 0.3F )
   ...
   {
      if ( self->Kias() < cornerSpeed * 0.6f )//me123 status test. accelerate if we are below 210cas changed from 0.3
      {
...
float eroll;
 ...
 // me123 hmm nonono we are too fast that's why we need to do this  MachHold(cornerSpeed, self->Kias(), TRUE);
	MachHold(150.0f, self->Kias(), TRUE);
	...
	if (targetPtr->BaseData()->IsAirplane() || targetPtr->BaseData()->IsFlight() || targetPtr->BaseData()->IsHelicopter())
	{
		...
		if (g_bUseNewCanEnage) {
			...
			if (!campBaseObj || campBaseObj->GetIdentified(self->GetTeam())) {
				...
				if ((targetPtr->BaseData()->Vt() * FTPSEC_TO_KNOTS > 300.0f || targetPtr->BaseData()->ZPos() < -10000.0f))  {
