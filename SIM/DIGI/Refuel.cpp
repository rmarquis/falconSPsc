...
void DigitalBrain::AiRefuel(void)
{
	...
	if(tankerId != FalconNullId)
	{
		...
		if(tanker)
		{
			...
			trackX = targetPos.x - tankerRelPositioning.x;
			trackY = targetPos.y - tankerRelPositioning.y;
			trackZ = targetPos.z - tankerRelPositioning.z;
...
// this also helps them in a tanker turn, which really can only do a human ;-)
			...
			if(rx > deceldistance)
			{
				rStick = SimpleTrackAzimuth(rx*0.5F, ry, self->Vt())*0.5F;
...
// 2002-02-28 MN refuel fixes, help the player somewhat more. Simplistic = full AI control 
	...
	if( dist < refuelMode * 100.0F &&
		...
		fabs(tanker->Vt() - self->Vt())*FTPSEC_TO_KNOTS < (5.0F + 45.0F *(refuelMode /* S.G. NO! refuelMode is ALREADY -1! - 1 */  )) &&
...
// Easy means no player input needed at all, also in Simplified when connected to the boom
		...
		// let's give realistic refueling also some help (realistic is just too hard in F4 according to Keith Rosenkrantz)
			{
				af->pstick = (UserStickInputs.pstick - pStick) * 0.5F + pStick;
				af->rstick = (UserStickInputs.rstick - rStick) * 0.5F + rStick;
...
int time=0;
   ...
   if (self->AutopilotType() == AircraftClass::CombatAP)
   {
      ...
      if (!foundSomething)
      {
         ObjectiveClass* nearest = FindNearestFriendlyRunway (self->GetTeam(),
            SimToGrid(self->XPos()), SimToGrid(self->YPos()));
