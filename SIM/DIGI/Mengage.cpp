...
float angLimit;
   ...
   if (curMode != MissileEngageMode )
   {
		if (targetPtr && curMissile &&
		    targetData->range <= maxAAWpnRange  &&
	       (targetData->range >= 3000.0f || !(((AircraftClass *)self)->Guns)) &&
		    targetData->ata < angLimit && CanEngage (self, self->CombatClass(), targetPtr, BVRManeuver | BVRManeuver)) // 2002-03-11 MODIFIED BY S.G. Added parameter "BVRManeuver | BVRManeuver"
		{
   ...
   else
   {
			if (targetData && targetData->range > 2000.0f) // JB 010208
			{
				if (!targetPtr ||
						!curMissile ||
						targetData->range > maxAAWpnRange  ||
						(targetData->range < 3000.0f * NM_TO_FT && (((AircraftClass *)self)->Guns)) &&
...
float tof, rMax;
   ...
   // 2002-03-15 MODIFIED BY S.G. If asked to perform maneuver like chainsaw, do this without doing the BVR stuff. Added the 'mpActionFlags[AI_EXECUTE_MANEUVER] == TRUE+1' condition
	...
	(
		(self->Sms->curWeaponType == wtAim120 && //we are plannign a aim120 shoot
	    !missileFiredEntity && holdlongrangeshot) || //me123 turn to colision course to fire the long range missile
		mpActionFlags[AI_EXECUTE_MANEUVER] == TRUE+1
	)
   {
	   /*--------------------------------------*/
	   /* Zero out the controls for God's sake */
	   /*--------------------------------------*/
	   SetYpedal( 0.0F );
		...
		/*---------------------------------------*/
	   tof = curMissile->GetTOF((-self->ZPos()), self->Vt(), targetData->ataFrom, targetPtr->BaseData()->Vt(),
		  targetData->range);
	   rMax = curMissile->GetRMax((-self->ZPos()), self->Vt(), targetData->az, targetPtr->BaseData()->Vt(), targetData->ataFrom);
		...
		if (holdlongrangeshot)
	   {
		  ...
		  if (SkillLevel() >= 3) trackZ -= targetData->range /3.0f;
		  }
		  else 
		  trackZ = targetPtr->BaseData()->ZPos();

		  desSpeed = 1.7F * cornerSpeed;
	   }
	   /*----------------------------------*/
...
//		  desiredClosure = 0.1F * (targetData->range - rdes);
		  ...
		  desSpeed = self->Kias() + desiredClosure ;//me123 - rngdot;
