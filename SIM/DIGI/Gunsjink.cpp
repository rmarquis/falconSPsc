   ...
   if (curMode != GunsJinkMode)
   {
	  if ( obj == NULL )
			 ...
			 TeamInfo[self->GetTeam()]->TStance(obj->BaseData()->GetTeam()) == War))
		  {
			...
			if (localData->range < 6000.0f)//localData->rangedot > -240.0f * FTPSEC_TO_KNOTS )//me123 don't jink if he's got a high closure, he probaly woun't shoot a low Pk shot
			{
...
float maxPull;
   ...
   /* energy management */
   /*-------------------*/
   MachHold(cornerSpeed,self->Kias(), FALSE);//me123 from TRUE
   /*--------------------*/
   /* find target aspect */
   /*--------------------*/
   ...
   if (jinkTime == -1)
   {
      ...
      if (self->Kias() < 0.85F * cornerSpeed && self->CombatClass() != MnvrClassBomber)
      {
         randVal = rand() % 100;
      ...
      else 
      {
         ...
         /*--------------------------------------------*/
         if (self->Kias() <= 0.8F * cornerSpeed)//me123 status test. changed from 0.6
         {
