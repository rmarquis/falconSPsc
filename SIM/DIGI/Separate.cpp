...
*///me123
   ...
   if (IsSetATC(SaidBingo)  ||                                 // Bingo Fuel
    //  (curMode == WVREngageMode && engagementTimer > 0 && SimLibElapsedTime > engagementTimer)) // Limited Engagement
		curMode == WVREngageMode &&(aaAbort || agAbort || campAbort || damageAbort))
   {
      ...
      if (curMode != SeparateMode && targetData->range < 2.0f * NM_TO_FT)
      {
         ...
         if (rMaxNe < sepRange && IsSetATC(SaidJoker))
         {
            ...
            turnRadius = self->Vt()*self->Vt() / ((float)sqrt(gs*gs - 1.0F) * GRAVITY);
...
//we don't care if we need fuel on the ground :)
   ...
   if (tmpWaypoint)
   {
      ...
      fuelNeeded = dist / (self->Vt() + 2.0F * GRAVITY * ps/self->Vt()) * fuelBurn / 3600.0F;
