...
float desiredClosure,actualClosure,lagAngle;
   ...
   if (targetPtr == NULL) 
   {
	...
	// 2002-02-10 ADDED BY S.G. Change our radarMode to digiSTT if we have been detected and we have a radar to start with
	if (SimLibElapsedTime > radarModeTest) {
	  ...
	  if (theRadar) {
		  ...
		  if (radarData->MaxTwstargets == 0) // Must be equipped with a radar capable of doing TWS...
			  ...
			  theRadar->digiRadarMode = RadarClass::DigiSTT;
		  else if (self->GetCampaignObject()->GetSpotted(self->GetTeam()))
			  theRadar->digiRadarMode = RadarClass::DigiSTT;
		  else
   ...
   /*---------------------------------------*/
	...
	closure = min (closure, targetPtr->BaseData()->Kias() + 50.0f);
  ...
  //    desiredClosure = (100.0F * targetData->range / 2000.0F) - 100.0F; //me123
      ...
      if( targetData->range < 2000.0F)//me123 changed from 2000
      {
         if( actualClosure > desiredClosure)
         {
				...
				//          AddMode (RoopMode);
			 //me123           FineGunsTrack(self->Kias()+(desiredClosure - actualClosure),&lagAngle);
						//      FineGunsTrack(min (targetPtr->BaseData()->Kias() -250,self->Kias()+(desiredClosure - actualClosure)),&lagAngle);//me123 erhh don't use corner here, it's not good you might overshoot.(max (cornerSpeed, self->Kias()+(desiredClosure - actualClosure)),&lagAngle);
						MachHold(targetPtr->BaseData()->Kias()-100.0f, self->Kias(), FALSE);//me123 addet
						if (targetData->range < 800.0F) // JB 010212 1000.0F)
						//{ // JB 010212
							RollAndPull();
						//	return; // JB 010212
						//} // JB 010212
						FineGunsTrack(min (targetPtr->BaseData()->Kias(),self->Kias()+(desiredClosure - actualClosure)),&lagAngle);//me123 erhh don't use corner here, it's not good you might overshoot.(max (cornerSpeed, self->Kias()+(desiredClosure - actualClosure)),&lagAngle); // JB 010212
         ...
         {
            FineGunsTrack(min (targetPtr->BaseData()->Kias(),self->Kias()+(desiredClosure - actualClosure)),&lagAngle);//me123 erhh don't use corner here, it's not good you might overshoot.(max (cornerSpeed, self->Kias()+(desiredClosure - actualClosure)),&lagAngle);
            if (Stagnated())
            {
      ...
      /*-------------------------------------------------------*/
      {
         ...
         {
            FineGunsTrack(min (targetPtr->BaseData()->Kias() + 30.0f,self->Kias()+(desiredClosure - actualClosure)),&lagAngle);//me123 erhh don't use corner here, it's not good you might overshoot.(max (cornerSpeed, self->Kias()+(desiredClosure - actualClosure)),&lagAngle);
...
float multiplier;
   ...
   gunFactor = leadTof * targetData->range/(self->Guns->initBulletVelocity + self->Vt());
   ...
   trackX = targetPtr->BaseData()->XPos() + targetPtr->BaseData()->XDelta() * gunFactor;
   trackY = targetPtr->BaseData()->YPos() + targetPtr->BaseData()->YDelta() * gunFactor;
   trackZ = targetPtr->BaseData()->ZPos() + targetPtr->BaseData()->ZDelta() * gunFactor;
   trackZ -= 0.5F * GRAVITY * gunFactor * gunFactor * 4.0F;
...
float lastStick=0.0F, leadTime=0.0F;
   ...
   tf = rangeEst / (self->Guns->initBulletVelocity + self->Vt());
	 if (localTarget && localTarget->localData)
		 tf = rangeEst / (self->Guns->initBulletVelocity + self->Vt() - localTarget->localData->rangedot); // JB 010211
	 else
		 tf = rangeEst / (self->Guns->initBulletVelocity + self->Vt());
   ...
   /*------------------------------*/
   dx = self->Vt() * self->platformAngles->cosgam * self->platformAngles->cossig * tf;
   dy = self->Vt() * self->platformAngles->cosgam * self->platformAngles->sinsig * tf;
   dz = -(self->Vt() * self->platformAngles->singam * tf + 0.5F * GRAVITY * tf * tf);
   ...
   pipperEl = (float)atan2(-rz,(float)sqrt(rx*rx+ry*ry+.1));
...
// JB     if (elerr < 3.0F * DTR && fabs(elerr) < 2.5F * DTR && fabs(azerr) < 3.0F /* ADDED BY S.G. - IT'S IN RADIAN! */ * DTR &&
   ...
   lastStick = pStick;
   MachHold(speed, self->Kias(), FALSE);//me123 true
...
float pullFact;
   ...
   zft = trackZ - self->ZPos();
	...
	// Guess TOF
	if (localTarget && localTarget->localData)
		tof = realRange / (self->Guns->initBulletVelocity + self->Vt() - localTarget->localData->rangedot);
	else
		tof = realRange / (self->Guns->initBulletVelocity + self->Vt());
   ...
   for (i=0; i<self->Sms->NumHardpoints(); i++)
   {
          ...
          (theGun = self->Sms->hardPoint[i]->GetGun()))
      {
         ...
         if (angles && SimLibElapsedTime % 2000 < 500)
         {
						...
						// Guess TOF
						tof = realRange / (theGun->initBulletVelocity + self->Vt() - localTarget->localData->rangedot);
