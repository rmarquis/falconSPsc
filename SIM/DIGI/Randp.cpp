...
//		trackY -= targetPtr->BaseData()->YDelta() *0.6f ;//me123 let's pull to target's 6 oclock
		trackX = targetPtr->BaseData()->XPos();
//		trackX -= targetPtr->BaseData()->XDelta() *0.6f;//me123 let's pull to target's 6 oclock
		trackZ = targetPtr->BaseData()->ZPos();
//		trackZ -= targetPtr->BaseData()->ZDelta() *0.6f;//me123 let's pull to target's 6 oclock
//			trackZ-= 500.0f;//me123 let's pull the nose up above the target to slow down faster
#ifdef MANEUVER_DEBUG
strcpy(tmpchr, "R&P slow fly comp not beh.3/9");
		...
		{
		trackY = targetPtr->BaseData()->YPos();	
//		trackY += targetPtr->BaseData()->YDelta() *0.1f ;//me123 let's pull to target's 12 oclock
		trackX = targetPtr->BaseData()->XPos();
//		trackX += targetPtr->BaseData()->XDelta() *0.1f;//me123 let's pull to target's 12 oclock
		trackZ = targetPtr->BaseData()->ZPos();
//		trackZ += targetPtr->BaseData()->ZDelta() *0.1f;//me123 let's pull to target's 12 oclock
#ifdef MANEUVER_DEBUG
strcpy(tmpchr, "R&P slow fly comp behind 3/9");
		...
		MachHold(0.36F * cornerSpeed, self->Kias(), FALSE);
#ifdef DEBUG_BFM 
...
MonoPrint ("head on");   
			...
			trackX = targetPtr->BaseData()->XPos();
			trackY = targetPtr->BaseData()->YPos();
//			trackX -= targetPtr->BaseData()->XDelta() *0.5f;//me123 let's pull to target's 6 oclock
//			trackY -= targetPtr->BaseData()->YDelta() *0.5f ;//me123 let's pull to target's 6 oclock
			trackZ = targetPtr->BaseData()->ZPos();
...
strcpy(tmpchr, "R&P head on range > 15.0");
				...
				// END OF ADDED SECTION 2002-03-14
					MachHold(2.0f * cornerSpeed,self->Kias(), TRUE);
#ifdef MANEUVER_DEBUG
strcpy(tmpchr, "R&P head on range > 6.0");
		...
		//	MonoPrint ("between 6 and 1.5nm trying to force a merge nose up");
			trackZ += 4000.0f;       
			MachHold(1.05f * cornerSpeed, self->Kias(), TRUE); // 2002-03-14 MODIFIED BY S.G. from 2 * cornerSpeed to 1.05f * cornerSpeed. Don't over do it
			AutoTrack(maxGs);
#ifdef MANEUVER_DEBUG
strcpy(tmpchr, "R&P head on range < 6.0");
#endif
...
#endif
 			trackY = targetPtr->BaseData()->YPos();		
//			trackY += targetPtr->BaseData()->YDelta() *0.5f ; 
			trackX = targetPtr->BaseData()->XPos();		
//			trackX += targetPtr->BaseData()->XDelta() *0.5f; 
			trackZ = targetPtr->BaseData()->ZPos();
//			trackZ += targetPtr->BaseData()->ZDelta() *0.5f; 
			AutoTrack(maxGs);
			 /*-------------------*/
...
MonoPrint ("Overshoot your basted ");
		...
		//NOT EMIDIATLY THREATENED
					...
					self->Kias() <= cornerSpeed * 0.9f && 
					self->Pitch() < -5*DTR)
		{//we are not threathened at the moment so let's get some energy if we need it
			trackY = targetPtr->BaseData()->YPos();		
//			trackY -= targetPtr->BaseData()->YDelta() *0.5f ;//me123 let's pull to target's 6 oclock
			trackX = targetPtr->BaseData()->XPos();		
//			trackX -= targetPtr->BaseData()->XDelta() *0.5f;//me123 let's pull to target's 6 oclock
...
strcpy(tmpchr, "R&P Defens. NOT IMMED. THREATND");
			...
			MachHold(1.05f * cornerSpeed, self->Kias(), TRUE); // 2002-03-14 MODIFIED BY S.G. Little energy? cornerSpeed * 2 is 840 knots for the F16, drop this to 
...
MonoPrint ("fighter is above ");
	...
	//horizontal fight
				else 
				{
					...
					{//nose/nose
					MachHold(0.6f * cornerSpeed, self->Kias(), FALSE);
#ifdef DEBUG_BFM
MonoPrint ("NOSE/NOSE ");
#endif	
					}
...
//target is non maneuvering.
			else
			{
				...
				MachHold(cornerSpeed, self->Kias(), FALSE);
				else
				MaintainClosure();
#ifdef DEBUG_BFM
...
{
		MachHold(1.05f * cornerSpeed,  self->Kias(), FALSE); // 2002-03-14 MODIFIED BY S.G. Went from 1.1 to 1.05 (465 to 440 for the F16)
	...
	{// we are on the way up
		...
		MachHold(1.05f * cornerSpeed,  self->Kias(), TRUE); // 2002-03-14 MODIFIED BY S.G. Went from 1.1 to 1.05 (465 to 440 for the F16)
#ifdef DEBUG_BFM
...
float newX, newY, newZ;
   ...
   if (lastMode != curMode)
   {
      if (tc > 0.0)
      {
         ...
         trackY = targetPtr->BaseData()->YPos();
         trackZ = targetPtr->BaseData()->ZPos();
         trackX += targetPtr->BaseData()->XDelta() * tc;
         trackY += targetPtr->BaseData()->YDelta() * tc;
         if (targetPtr->BaseData()->ZDelta() > ALT_RATE_DEADBAND )
            trackZ += (targetPtr->BaseData()->ZDelta() - ALT_RATE_DEADBAND) * tc;
      ...
      else
      {
         ...
         trackY = targetPtr->BaseData()->YPos();
         trackZ = targetPtr->BaseData()->ZPos();
         trackX += targetPtr->BaseData()->XDelta() * MAGIC_NUMBER;
         trackY += targetPtr->BaseData()->YDelta() * MAGIC_NUMBER;
         if (targetPtr->BaseData()->ZDelta() > ALT_RATE_DEADBAND )
            trackZ += (targetPtr->BaseData()->ZDelta() - ALT_RATE_DEADBAND) * MAGIC_NUMBER;
         else if (targetPtr->BaseData()->ZDelta() < -ALT_RATE_DEADBAND )
            trackZ += (targetPtr->BaseData()->ZDelta() + ALT_RATE_DEADBAND) * MAGIC_NUMBER;
      }
   ...
   else
   {
      ...
      trackX = 0.1F * newX + 0.9F * trackX;
      trackY = 0.1F * newY + 0.9F * trackY;
      trackZ = 0.1F * newZ + 0.9F * trackZ;
   }
   AutoTrack(maxGs);
}
...
float rng,closure,rngdot;
   ...
   if (closure - rngdot > 0.0f) {
	   ...
	   // MachHold ( (self->Kias() + (closure - rngdot)), self->Kias(), FALSE);  //me123 from MachHold(max (cornerSpeed, (self->Kias() + (closure - rngdot))), self->Kias(), TRUE);
	   MachHold(targetData->range > 2.0f * NM_TO_FT && rngdot > 0 && self->Kias() + (closure - rngdot) > cornerSpeed ? cornerSpeed : self->Kias() + (closure - rngdot), self->Kias(), FALSE);  //me123 from MachHold(max (cornerSpeed, (self->Kias() + (closure - rngdot))), self->Kias(), TRUE);
   }
   else if (targetData->range < 5000.0f)
	   MachHold ( (min( cornerSpeed /* *1.2f S.G. */ , self->Kias() + (closure - rngdot)) ), self->Kias(), FALSE) ; //me123 from MachHold(max (cornerSpeed, (self->Kias() + (closure - rngdot))), self->Kias(), FALSE);
   else
	   MachHold ( (self->Kias() + (closure - rngdot)), self->Kias(), FALSE); //me123 from MachHold(max (cornerSpeed, (self->Kias() + (closure - rngdot))), self->Kias(), FALSE);
...
//   tc = min (targetData->range / -targetData->rangedot, 5.0F);
   tc = min (targetData->range / self->Vt(), 0.5F);//me123 from 1.5
