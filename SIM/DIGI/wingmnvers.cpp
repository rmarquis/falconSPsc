...
float desiredClosure, actualClosure;
      ...
      //desiredClosure = 200.0F * rx/(1.0F*NM_TO_FT) - 200.0F;
			desiredClosure = 350.0F * rx/(1.0F*NM_TO_FT) - 350.0F; // JB 011016 Increase closure rates to adjust for the vt/kias bug fix below
...
float	DigitalBrain::SimpleTrackElevation(float zft, float scale)
{
	...
	if (g_bPitchLimiterForAI &&														// We are asking to limit AI's turn agressiveness when flying to waypoints
		...
		altErr > 0.0f && self->Pitch() > 0.0f) {									// Pitching up and going up
