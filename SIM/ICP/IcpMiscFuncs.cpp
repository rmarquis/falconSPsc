...
void ICPClass::PushedSame(int LastMode)
{
	...
	InputsMade = 0;
	MadeInput = FALSE;
	SimDriver.playerEntity->FCC->SetStptMode(FireControlComputer::FCCWaypoint);
...
void ICPClass::CheckAutoSTPT(void)
{
	...
	if(SimDriver.playerEntity && SimDriver.playerEntity->FCC && SimDriver.playerEntity->FCC->GetMasterMode() == FireControlComputer::AirGroundBomb)
		return;
