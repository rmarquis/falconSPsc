...
void ICPClass::SetOwnship(void) 
{
	...
	if(SimDriver.playerEntity && SimDriver.playerEntity->IsAirplane()) // 2002-02-15 MODIFIED BY S.G. Added the IsAirplane test since it could be an ejected pilot
	{

		// Waypoint Stuff
		mpWayPoints				= SimDriver.playerEntity->waypoint; // head of the waypoint list
		mNumWayPts				= (BYTE) SimDriver.playerEntity->numWaypoints;
		...
		else 
		{
			...
			else 
			{
				...
				case DLINK_BUTTON:
					...
					break;
				case MARK_BUTTON:
					SimDriver.playerEntity->FCC->SetStptMode(FireControlComputer::FCCMarkpoint);
					SimDriver.playerEntity->FCC->waypointStepCmd = 127;
	...
	else
	{
		...
		else if(mode == FACK_BUTTON)
		{
			...
			if (m_FaultDisplay == false) { // was off
				...
				SimDriver.playerEntity->mFaults->GetNextFault(&m_subsystem, &m_function) == FALSE)
		...
		else if(mode == PREV_BUTTON || mode == NEXT_BUTTON) 
		{
			...
			else if(IsICPSet(ICPClass::MODE_COMM1))
			{
				if(mode == NEXT_BUTTON)
				{
					if(PREUHF == 7)
