...
int CalcFuelOnSta(float distanceToSta) {
	...
	float	fuelConsumed;
	int	fuelOnStation;
	float speed = max(1.0F, SimDriver.playerEntity->Vt());

	//fuelConsumed	= distanceToSta / SimDriver.playerEntity->Vt() * SimDriver.playerEntity->af->FuelFlow() / 3600.0F;
	fuelConsumed	= distanceToSta / speed * SimDriver.playerEntity->af->FuelFlow() / 3600.0F;
	//MI this will not take any external tanks
#if 0
	fuelOnStation	= (int) (SimDriver.playerEntity->af->Fuel() - fuelConsumed);
#else
	fuelOnStation	= (int) (SimDriver.playerEntity->GetTotalFuel() - fuelConsumed);
#endif
	...
	if(!g_bRealisticAvionics)
	{
		if(mUpdateFlags & CRUS_UPDATE) {
			...
			break;
				 ...
				 Tpoint			pos;
				 pos.x = SimDriver.playerEntity->af->x;
				 pos.y = SimDriver.playerEntity->af->y;
				 pos.z = SimDriver.playerEntity->af->z;

			heading		= FloatToInt32(((WeatherClass*)TheWeather)->WindHeadingAt(&pos) * RTD);
			...
			windSpeed	= ((WeatherClass*)TheWeather)->WindSpeedInFeetPerSecond(&pos) * FTPSEC_TO_KNOTS;
		...
		case STPT_LIST:
			...
			deltaX			= xpos - SimDriver.playerEntity->XPos();
			deltaY			= ypos - SimDriver.playerEntity->YPos();
		...
		if(pwaypoint) {
			cruise			= SimDriver.playerEntity->af->GetOptimumCruise();
			altitude			= FloatToInt32(SimDriver.playerEntity->af->GetOptimumAltitude());
...
void ICPClass::PNUpdateCRUSMode(int button, int) {
	...
	case STPT_LIST:
		...
		if(button == PREV_BUTTON) {
			...
			if(mpCruiseWP == NULL) {
				mpCruiseWP		= SimDriver.playerEntity->waypoint;
				mCruiseWPIndex = 0;
...
void ICPClass::CruiseRNG(void)
{
	...
	if(!SimDriver.playerEntity)
		return;
	//WayPointClass *wp = SimDriver.playerEntity->GetWayPointNo(RangeWP);
	ClearStrings();
	//Line1
...
void ICPClass::CruiseHOME(void)
{
	...
	if(!SimDriver.playerEntity)
		return;
	WayPointClass *wp = SimDriver.playerEntity->GetWayPointNo(HomeWP);
	ClearStrings();
	//Line1
	FillDEDMatrix(0,8,"CRUS");
	FillDEDMatrix(0,13,"\x02",2);
	...
	float distanceToSta	= (float)sqrt(deltaX * deltaX + deltaY * deltaY);
	int fos				= max(0, CalcFuelOnSta(distanceToSta));
	if (SimDriver.playerEntity->af->platform->IsF16())
	{
		float	fuelConsumed;
	 	int	fuelOnStation;
	 	fuelConsumed	= distanceToSta / 6000.0f * 10.0f * 0.67f;
		fuelConsumed	+= min(1,distanceToSta / 6000.0f / 80.0f) *(500.0f - (-SimDriver.playerEntity->ZPos()) /40.0f*0.5f);
	 	fuelOnStation	= (int) (SimDriver.playerEntity->GetTotalFuel() - fuelConsumed);
		fos = fuelOnStation;
	}
	sprintf(tempstr,"%dLBS",fos);
	...
	int altitude		= FloatToInt32(SimDriver.playerEntity->af->GetOptimumAltitude());
...
void ICPClass::CruiseEDR(void)
{
	...
	level = (long)((AircraftClass*)(SimDriver.playerEntity))->GetBingoFuel();
	//Total fuel
	total = (long)((AircraftClass*)(SimDriver.playerEntity))->GetTotalFuel();
	float EDR = (float)total - (float)level;
	long FF = (long)((AircraftClass*)(SimDriver.playerEntity))->af->FuelFlow();
	EDR = EDR / FF;
	FindEDR((long)(EDR * 3600) ,tempstr);
	FillDEDMatrix(2,14,tempstr);
	//Line4
	FillDEDMatrix(3,4,"OPT MACH");
	float cruise = SimDriver.playerEntity->af->GetOptimumCruise();
...
void ICPClass::CruiseTOS(void)
{
	...
	if(!SimDriver.playerEntity)
		return;
	//WayPointClass *wp = SimDriver.playerEntity->GetWayPointNo(TOSWP);
	ClearStrings();
	char timeStr[16] = "";
	int hr, minute, sec;
	VU_TIME ETA;
	...
	float ttg = 0.0F;
	if(SimDriver.playerEntity->curWaypoint)
		ttg = (float)SimDriver.playerEntity->curWaypoint->GetWPArrivalTime() / SEC_TO_MSEC;
	...
	ETA	= SimLibElapsedTime / SEC_TO_MSEC + FloatToInt32(Distance(SimDriver.playerEntity->XPos(),
		SimDriver.playerEntity->YPos(), xCurr, yCurr) / SimDriver.playerEntity->af->vt);
...
void ICPClass::StepHOMERNGSTPT(int mode)
{
	...
	{
		if(mode == PREV_BUTTON)
			((AircraftClass*)(SimDriver.playerEntity))->FCC->waypointStepCmd = -1;
		else
			((AircraftClass*)(SimDriver.playerEntity))->FCC->waypointStepCmd = 1;
	}
}
int ICPClass::GetHOMERNGSTPTNum(int var, int mode)
{
	...
	if(mode == NEXT_BUTTON)
	{
		curWaypoint	= SimDriver.playerEntity->GetWayPointNo(tempvar);
