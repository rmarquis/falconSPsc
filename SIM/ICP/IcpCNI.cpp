...
{
	if (!SimDriver.playerEntity) return;//me123 ctd fix
	if(!g_bRealisticAvionics)
	{
...

		{
			...
			if(SimDriver.playerEntity->FCC->GetStptMode() == FireControlComputer::FCCWaypoint) 
			{
			 sprintf(mpLine1, "COMM1 : %-12s STPT %-3d", (VM ? RadioStrings[VM->GetRadioFreq(0)] : "XXXX"), mWPIndex + 1);
			}
			else if(SimDriver.playerEntity->FCC->GetStptMode() == FireControlComputer::FCCDLinkpoint) 
			{
				sprintf(mpLine1, "COMM1 : %-12s LINK %-3d", (VM ? RadioStrings[VM->GetRadioFreq(0)] : "XXXX"), gNavigationSys->GetDLinkIndex() + 1);
			}
			else if(SimDriver.playerEntity->FCC->GetStptMode() == FireControlComputer::FCCMarkpoint) 
			{
