...
void ICPClass::ExecINSMode(void)
{
	...
	if(g_bINS)
	{
		...
		if(SimDriver.playerEntity)
		{
			INSTime = SimDriver.playerEntity->INSAlignmentTimer;
			int State = SimDriver.playerEntity->INSStatus;
			...
			if(!SimDriver.playerEntity->HasAligned && 
				SimDriver.playerEntity->INSState(AircraftClass::INS_Nav))
			{
				FillDEDMatrix(0,14,"00");
			}
			else if(!SimDriver.playerEntity->INSState(AircraftClass::INS_Aligned))
			{
				if(State <= 70)
				...
				SimDriver.playerEntity->INSState(AircraftClass::INS_Aligned))
			{
...
void ICPClass::ExecEWSMode(void)
{
	...
	FillDEDMatrix(2,13,"EDBK");
	FillDEDMatrix(2,21,"OFF");
	//Line4
	FillDEDMatrix(3,2,"O1");
	FillDEDMatrix(3,9,"0");
	FillDEDMatrix(3,12,"REQCTR");
	FillDEDMatrix(3,21,"OFF");
	//Line5
...
void ICPClass::ExecINTGMode(void)
{
	...
	//Line1
	if(SimDriver.playerEntity && !SimDriver.playerEntity->HasPower(AircraftClass::IFFPower))
