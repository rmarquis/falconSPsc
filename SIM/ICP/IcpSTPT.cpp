...
void ICPClass::ExecSTPTMode() {
	...
	{
		//MI Original code
		if (SimDriver.playerEntity && SimDriver.playerEntity->curWaypoint) {// && !SimDriver.playerEntity->FCC->InTransistion()
			...
			xOwn							= SimDriver.playerEntity->XPos();
			yOwn							= SimDriver.playerEntity->YPos();
			vtOwn							= SimDriver.playerEntity->af->vt;
			previous						= SimDriver.playerEntity->curWaypoint->GetPrevWP();
			altitude						= SimDriver.playerEntity->curWaypoint->GetWPAltitude();
			arrive						= SimDriver.playerEntity->curWaypoint->GetWPArrivalTime() / SEC_TO_MSEC;
			action						= SimDriver.playerEntity->curWaypoint->GetWPAction();
			wpflags						= SimDriver.playerEntity->curWaypoint->GetWPFlags();
	...
	else
	{
		...
		if(SimDriver.playerEntity)
			...
			SimDriver.playerEntity->XPos(), SimDriver.playerEntity->YPos(), xCurr, yCurr) 
			/ SimDriver.playerEntity->af->vt);
		FormatTime(ETA, timeStr);
