...
void NavigationSystem::ExecIls(void) {
	...
	if(SimDriver.playerEntity && mpCurrentIls.rwyidx != 0 && mpCurrentIls.vuID != FalconNullId && (GetInstrumentMode() == NavigationSystem::ILS_TACAN || GetInstrumentMode() == NavigationSystem::ILS_NAV)) {
					
		xfrontdiff						= mpCurrentIls.frontx - SimDriver.playerEntity->XPos();
		yfrontdiff						= mpCurrentIls.fronty - SimDriver.playerEntity->YPos();
		xbackdiff						= mpCurrentIls.backx - SimDriver.playerEntity->XPos();
		ybackdiff						= mpCurrentIls.backy - SimDriver.playerEntity->YPos();
	
		zdiff								= (float)fabs(mpCurrentIls.z - SimDriver.playerEntity->ZPos());
		...
		if(attribute == GP_DEV) {
			*value = mpCurrentIls.gpDeviation;
			...
			if(g_bRealisticAvionics && g_bINS && SimDriver.playerEntity)
				SimDriver.playerEntity->LOCValid = TRUE;	//Flag not visible
		}
		else if(attribute == GS_DEV) {
			*value = mpCurrentIls.gsDeviation;
			returnVal = TRUE;
			if(g_bRealisticAvionics && g_bINS && SimDriver.playerEntity)
				SimDriver.playerEntity->GSValid = TRUE;	//Flag not visible
		}
	}
	else
	{
		...
		if(g_bRealisticAvionics && g_bINS && SimDriver.playerEntity)
		{
			SimDriver.playerEntity->LOCValid = FALSE;	//Flag visible
			SimDriver.playerEntity->GSValid = FALSE;	//Flag visible
		}
	}
...
BOOL NavigationSystem::GetTCNAttribute(Attribute attribute, float* value) 
{
    ...
    switch (attribute) {
    case RANGE:
	*value = mpCurrentTCN[mCurrentTCNSrc].range;
	return TRUE;
    case ILSFREQ:
	...
	if(GetInstrumentMode() == TACAN || GetInstrumentMode() == ILS_TACAN) {
		...
		if(pcurrentWaypoint && pcurrentWaypoint->GetWPAction() == WP_LAND) {
			*pATCId	= pcurrentWaypoint->GetWPTargetID();		
			...
			airbase = FindNearestFriendlyAirbase (SimDriver.playerEntity->GetTeam(), x, y);
