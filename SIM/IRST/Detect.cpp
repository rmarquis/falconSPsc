	...
	else if (platform->IsMissile())//me123 status bad. make the heatseekers limited to 28 degree radar slew and 6 degree cone in bore.
	{
		...
		// Marco Edit - here it is either uncaged or boresighted
		else//me123 bore/uncaged
			{
			 if (tracking)
				{//me123  tracking
					if (obj->localData->ata < typeData->GimbalLimitHalfAngle)
					{
						return TRUE;
					}
					else
				...
				{
					if (fabs(obj->localData->el - seekerElCenter) <  (typeData->FOVHalfAngle / 3) && 
						fabs(obj->localData->az - seekerAzCenter) <  (typeData->FOVHalfAngle / 3 ) ) 
					{
						tracking = TRUE ;
						return TRUE;
					}
