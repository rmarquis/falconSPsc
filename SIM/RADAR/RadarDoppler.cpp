...
static const struct RadarMenus cmenu[20] = {
    ...
    {"PM", "OFF", RadarClass::RadarMode::OFF}, // 9
    NoLabel,
    NoLabel,
    NoLabel,
    NoLabel,
...
int testTime;
   ...
   if (lastLocked)
	   lastLocked->Reference(SIM_OBJ_REF_ARGS);
   ...
   if (lastLocked)
	   lastLocked->Release(SIM_OBJ_REF_ARGS);
   lastLocked = lockedTarget;

	// JB 010224 Start Enable the CombatAP to shoot A2A missiles
	if (g_bSmartCombatAP && lockedTarget && 
		//((AircraftClass*) lockedTarget->BaseData())->isDigital && // Don't shoot at other players
		...
		)
	{
		...
		switch (mode)
		{
		case RWS: case LRS: case TWS: case VS: case ACM_30x20: case ACM_SLEW: case ACM_BORE: case ACM_10x60: case
					SAM: case SAM_AUTO_MODE: case SAM_MANUAL_MODE: case STT:
				digimode = AA;
				break;
			default:
				digimode = 0;
	...
	if (g_bRealisticAvionics && platform->OnGround()) { // JPO - radar doesn't scan with WOW
	    // do nothing
	}
...
// END OF ADDED SECTION
      ...
      if (rdrData->sensorLoopCount[Radar] < testTime)
      {
         ...
         if (mode == TWS )
         {
            if ((SimLibElapsedTime - rdrData->rdrLastHit) < TwsExtrapolateTime)
            {
