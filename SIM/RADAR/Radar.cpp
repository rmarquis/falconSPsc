...
// END OF ADDED SECTION
	...
	Vr = (float)cos(target->localData->ataFrom) * target->BaseData()->Vt();
...
// 2001-05-14 MODIFIED BY S.G. SINCE THE RETURN VALUE IS ALWAYS LESS THAN SOMETHING AND -1 IS LESS THAN 0, I'LL USE IT TO FLAG 'NoLOS' TO THE RadarDigi FUNCTION
//		S = 0.0f;
		S = -1.0f;
...
void RadarClass::SendTrackMsg ( SimObjectType* tgtptr, unsigned int trackType, unsigned int hardpoint)
{
	...
	if (tgtptr->localData->lockmsgsend == trackType && (trackType != Track_Lock || tgtptr->localData->lastRadarMode == hardpoint)) return; // 2002-02-10 MODIFIED BY S.G. Need to send a 'Track_Lock if the radar mode has changed
	if (tgtptr->localData->lockmsgsend == Track_None && trackType == 2) return;
	if (tgtptr->localData->lockmsgsend == Track_Launch && trackType == Track_Lock) return;

	if (!((SimBaseClass*)tgtptr->BaseData())->IsAirplane()) return;
	tgtptr->localData->lockmsgsend = trackType;
	if (trackType == Track_Lock) // 2002-02-10 ADDED BY S.G. Keep track of the last radar mode if it's a Track_Lock message
	...
	while (sess)
	{
		if (sess->Camera(0) && sess->Camera(0)->Id() == platform->Id()||
			sess->Camera(0) && sess->Camera(0)->Id() == id)
		{
		reliable = 2;
		break;
		}
	...
	countb ++;
//	 MonoPrint ("RadarClass::SendTrackMsg %d %d %d %08x %08x %08x%08x, %d\n",reliable,count,countb, this, platform, id, trackType);
