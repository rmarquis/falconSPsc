...
float az, el, theta;
	...
	switch(mode)
	{
	  case GM:
		if (flags & DBS1)
		{
			curScanRate = scanRate * 0.25F;	// We're fudging -- Up the scan rate to reduce the latency
		}
		else if (flags & DBS2)
		{
			curScanRate = scanRate * 0.05F;	// We're fudging -- Up the scan rate to reduce the latency
		}
		else
		{
