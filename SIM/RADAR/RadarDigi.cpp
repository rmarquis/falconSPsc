	...
	if (!isEmitting)
	{
		...
		if (g_nShowDebugLabels & 0x200) {
			if (platform->drawPointer)
				((DrawableBSP*)platform->drawPointer)->SetLabel("Not emitting",((DrawableBSP*)platform->drawPointer)->LabelColor());
			else
				((DrawableBSP*)platform->drawPointer)->SetLabel("            ",((DrawableBSP*)platform->drawPointer)->LabelColor());
		}
#endif
...
// 2001-05-14 MODIFIED BY S.G. IF ReturnStrength RETURNED -1.0f, THEN IT MEANS WE HAVE NO LOS ON THE TARGET. DON'T SAY IT'S JAMMING
		...
		if (canSee) {
			...
			if (tmpPtr->localData->sensorState[Radar] == NoTrack) 
			{
				...
				tmpPtr->localData->sensorLoopCount[Radar] = SimLibElapsedTime;  // 2002-02-10 ADDED BY S.G. SensorFusion uses that field
				if (!(flag & FirstSweep))// 2002-03-21 ADDED BY S.G. When a radar is doing its first sweep after creation, don't fade the signal or the SARH missile launched by an aggregated battalion that just deaggregated will lose its sensor lock
					canSee |= SG_FADING; // this will make the sensor state max set to detection
			}

			// 2002-03-10 ADDED BY S.G. Added the "(flag & FirstSweep) && " so when a radar is doing its first sweep after creation, don't fade the signal or the SARH missile launched by an aggregated battalion that just deaggregated will lose its sensor lock
		...
		if (lockedTarget && tmpPtr->BaseData() == lockedTarget->BaseData()) {
			...
			if (g_nShowDebugLabels & 0x400)
			{
				...
				if (platform->drawPointer)
					((DrawableBSP*)platform->drawPointer)->SetLabel(label,((DrawableBSP*)platform->drawPointer)->LabelColor());
			}
#endif
...
#ifdef DEBUG
				SetDesiredTarget(new SimObjectType(OBJ_TAG, platform, cm));
#else
