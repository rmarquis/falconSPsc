...
class DigitalBrain : public BaseBrain
{
   ...
   protected:
	   ...
	   void CrankManeuver(int direction);//me123
...
public:
	enum MissionClassEnum {AGMission, AAMission, SupportMission, AirliftMission}; // 2002-03-05 MODIFIED BY S.G. Added 'Enum' to differentiate it from the getter MissionClass
protected:
...
public:
   ...
   float PIDLoop(float error, float K, float KD, float KI, float Ts, float *lastErr, float *MX, float Output_Top, float Output_Bottom, bool LimitMX);
   ...
   void ReSetLabel (SimBaseClass *theObject);
	...
	MissionClassEnum MissionClass(void)					{return missionClass;} // 2002-03-05 ADDED BY S.G. Need it public
...
private:
	...
	// Mode Arrays
	int						mpActionFlags[AI_TOTAL_ACTION_TYPES]; // 2002-03-04 MODIFIED BY S.G. Used to be type BOOL and now needs 0, 1 or 2 so for best practice, make it an int
