...
/*
** Destructor for MissileInFlightData
*/
MissileInFlightData::~MissileInFlightData( void )
{
}

MissileClass::MissileClass(VU_BYTE** stream) : SimWeaponClass (stream)
{
   InitData();
...
void MissileClass::InitData (void)
{
   ifd = NULL;
   trail = NULL;
   engGlow = NULL;
...
void MissileClass::Init (SimInitDataClass* initData)
{
   ...
   {
	   // Set the campaign parent unit, if any
	   if (initData->campUnit)
	   {
      ...
      switch (inputData->displayType)
      {
         ...
         case DisplayHTS:
		 	if ( parent->IsCampaign() )
            	...
            	display = FindSensor((SimMoverClass*)parent, SensorClass::HTS);
         break;
...
void MissileClass::Start(SimObjectType *tgt)
{
   ...
   ShiAssert( parent );		// I believe a parent is _required_ now
   VuReferenceEntity (parent);
   parentReferenced = TRUE;
   ...
   if (!isSlave || !isCaged || tgt == NULL)// && sensorArray[0]->Type() == SensorClass::IRST) //me123 make sure only ir's are unchaged for now
   {
		...
		if (tgt)
		{
			tgt->Reference( SIM_OBJ_REF_ARGS );
			hasref = true;
		}
   }
...
//         MonoPrint ("%8.2f %8.2f %8d\n", targetPtr->localData->az*57.29F, targetPtr->localData->el*57.29F, SimLibElapsedTime);
		 ...
		 if (((DrawableBSP*)drawPointer)->GetNumSwitches() > 0 ) {
			 if (ifd && ifd->stage2gone)	// 2002-03-04 MN CTD fix
				 ((DrawableBSP *)drawPointer)->SetSwitchMask( 0, 0 );
			 else 
				 ((DrawableBSP *)drawPointer)->SetSwitchMask( 0, 1 );
		 }
...
// Hope this finally fixes floating missiles
		 if ( done == FalconMissileEndMessage::Missed && runTime < 15.0f && !((g_nMissileFix & 0x10) && (flags & ClosestApprch)))
		 {
#ifndef MISSILE_TEST_PROG
         	done = FalconMissileEndMessage::NotDone;
#endif
...
// remove				misPos.data.teamColor = TeamInfo[GetTeam()]->GetColor();
   ...
   { 
	   ...
	   FireControlComputer* theFCC = ((SimVehicleClass*)parent)->GetFCC();
	   if(theFCC)
	   {
	   ...
	   {
		   FalconRadioChatterMessage* radioMessage = new FalconRadioChatterMessage( parent->Id(), FalconLocalSession );
		   radioMessage->dataBlock.from = parent->Id();
		   radioMessage->dataBlock.to = MESSAGE_FOR_TEAM;
		   radioMessage->dataBlock.voice_id = ((FlightClass*)(((AircraftClass*)parent)->GetCampaignObject()))->GetPilotVoiceID(((AircraftClass*)parent)->GetCampaignObject()->GetComponentIndex(((AircraftClass*)parent)));
		   radioMessage->dataBlock.edata[0] = ((AircraftClass*)parent)->GetCampaignObject()->GetComponentIndex(((AircraftClass*)parent));
...
//	float rmax = Math.ThreedInterp (alt, vt, az,
	float rmax = Math.ThreedInterp (alt, vt, ataFrom,
...
float MissileClass::GetTOF (float alt, float vt, float ataFrom, float targetVt, float range)
{
	...
	tof += -5.0 * (float) sin(.07 * tof); // JB 010215
...
//	{
		memcpy (dmx, ((SimBaseClass*)parent)->dmx, sizeof (TransformMatrix));
//	}
...
void MissileClass::UpdatePosition(void)
{
Tpoint newPos;
...
int retval = 0;
   ...
   if (parentReferenced)
   {
	  ...
	  CampBaseClass *campobj = ((GroundClass*)parent)->GetCampaignObject();
	  if (campobj -> IsBattalion())
	      ((BattalionClass*)campobj)->DecrementMissileCount();
	  else if (campobj -> IsTaskForce()) // add in naval stuff
	      ((TaskForceClass*)campobj)->DecrementMissileCount();
      } else if (parent->IsBattalion()) {
		  ((BattalionClass*)parent)->DecrementMissileCount();
      }
      else if (parent->IsTaskForce()) {
	  ((TaskForceClass*)parent)->DecrementMissileCount();
      }
...
void MissileClass::ClearReferences (void)
{
...
int MissileClass::Wake (void)
{
    ...
    if (parent) //Campaign launched vehicles don't necessarily have a parent
    {
	if (parent->IsGroundVehicle()) { // JPO more checks please
	    CampBaseClass *campobj = ((GroundClass*)parent)->GetCampaignObject();
	    if (campobj -> IsBattalion())
	    ...
	    ((BattalionClass*)parent)->IncrementMissileCount();
	}
	else if (parent->IsTaskForce()) { // and again
	    ((TaskForceClass*)parent)->IncrementMissileCount();
	}
    }
    return retval;
}

void MissileClass::SetVuPosition (void)
{
   SetPosition (x, y, z);
   SetDelta(xdot, ydot, zdot);
   SetYPR(psi, theta, phi);
...
// 2002-03-28 MN added BombImpact for "bomb-like" missiles (JSOW...)
	if (done == FalconMissileEndMessage::MissileKill || done == FalconMissileEndMessage::BombImpact)
	{
		if (targetPtr)
		{
		...
		while ( objective )
		{
			if (objective->GetComponents())
			{
				...
				while (testObject)
				{
					...
					if (!targetPtr || testObject != targetPtr->BaseData() ||
// 2002-03-28 MN apply proximity damage if it IS the target, but we have got no missile kill or BombImpact - or the target can not be damaged by proximity damage
						(g_nMissileFix & 0x08) && testObject == targetPtr->BaseData() && done != FalconMissileEndMessage::MissileKill && done != FalconMissileEndMessage::BombImpact)
					{
						tmpX = testObject->XPos() - XPos();
						tmpY = testObject->YPos() - YPos();
