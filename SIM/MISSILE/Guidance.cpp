...
void MissileClass::CheckGuidePhase(void)
{
	...
	if (!guidencephase)//boost
	{
		if (runTime > inputData->boostguidesec)
		guidencephase = 1;
	}
	else if (guidencephase == 1)//sustain
	{
...
////me123 test stuff
 ...
 if (g_nboostguidesec)  inputData->boostguidesec= g_nboostguidesec;//me123 how many sec we are in boostguide mode
 if (g_nterminalguiderange)  inputData->terminalguiderange= g_nterminalguiderange;//me123 what range we transfere to terminal guidence
 if (g_nboostguideSensorPrecision)   inputData->boostguideSensorPrecision=  g_nboostguideSensorPrecision;//me123
 if (g_nsustainguideSensorPrecision)   inputData->sustainguideSensorPrecision= g_nsustainguideSensorPrecision ;//me123
 if (g_nterminalguideSensorPrecision)   inputData->terminalguideSensorPrecision= g_nterminalguideSensorPrecision ; //me123
 if (g_nboostguideLead)   inputData->boostguideLead = g_nboostguideLead ;//me123
 if (g_nsustainguideLead)   inputData->sustainguideLead = g_nsustainguideLead ;//me123
 if (g_nterminalguideLead)   inputData->terminalguideLead = g_nterminalguideLead ;//me123
 if (g_nboostguideGnav)   inputData->boostguideGnav = g_nboostguideGnav ;//me123
 if (g_nsustainguideGnav)   inputData->sustainguideGnav = g_nsustainguideGnav ;//me123
 if (g_nterminalguideGnav)   inputData->terminalguideGnav = g_nterminalguideGnav ;//me123
 if (g_nboostguideBwap)   inputData->boostguideBwap = g_nboostguideBwap;//me123
 if (g_nsustainguideBwap)   inputData->sustainguideBwap =g_nsustainguideBwap;//me123
 if (g_nterminalguideBwap)   inputData->terminalguideBwap = g_nterminalguideBwap ;//me123
...
//////////
   ...
   float altitude = -z;
		...
		if (g_nShowDebugLabels & 0x02)
		{
			if ( drawPointer )
			{
				...
				if (flags & SensorLostLock)
					strcat(tmpStr,"OTgtL ");
				sprintf(label,"B %4.1f %5.0f", vcas, altitude);
				strcat (tmpStr,label);
				if (wentActive)
					strcat(tmpStr," Activ");
	...
	else if (guidencephase == 1)//sustain
	{
		...
		if (g_nShowDebugLabels & 0x02)
		{
			if ( drawPointer )
			{
				...
				if (flags & SensorLostLock)
					strcat(tmpStr,"OTgtL ");
				sprintf(label,"S %4.1f %5.0f", vcas, altitude);
				strcat (tmpStr,label);
				if (wentActive)
					strcat(tmpStr," Activ");
				((DrawableBSP*)drawPointer)->SetLabel (tmpStr, ((DrawableBSP*)drawPointer)->LabelColor());
	...
	else if (guidencephase == 2)//terminal
	{
		...
		if (g_nShowDebugLabels & 0x02)
		{
			if ( drawPointer )
			{
				...
				if (flags & SensorLostLock)
					strcat(tmpStr,"OTgtL ");
				sprintf(label,"T %4.1f %5.0f", vcas, altitude);
				strcat (tmpStr,label);
				if (wentActive)
					strcat(tmpStr," Activ");
				((DrawableBSP*)drawPointer)->SetLabel (tmpStr, ((DrawableBSP*)drawPointer)->LabelColor());
   ...
   if (runTime > inputData->guidanceDelay)
   {
	   ...
	   {
		  ifd->augCommand.yaw   =  0;
		  ifd->augCommand.pitch =  0;
		  return;
	   }
      if (targetPtr)
      {
		 ...
		 range = (float)sqrt (dxi*dxi + dyi*dyi + dzi*dzi);
		 timpct = range/(vt + targetPtr->BaseData()->Vt() * (float)cos(targetPtr->localData->ataFrom));
		 if (timpct < 0.0f) {
...
// 2002-04-07 MN diminish targetDZ so missile won't go too ballistic...
   ...
   if (hasTarget)
   {
      ...
      /*--------------------------*/
      omegayReq = Gnav *
			(dmx[1][0]*wic + dmx[1][1]*wjc + dmx[1][2]*wkc);
      omegazReq = Gnav *
			(dmx[2][0]*wic + dmx[2][1]*wjc + dmx[2][2]*wkc);
