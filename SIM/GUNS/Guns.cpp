...
GunClass::GunClass (int type) : SimWeaponClass (type)
{
   ...
   // edg note: The following is a hack until we can use wcPtr->Flags
   // to check for WEAP_TRACER.  I've got a predetermined list of what
   // type indices to make shells
   /*
   if ( wcPtr->Flags & WEAP_TRACER )
...
//	      MonoPrint( "Guns: TRACER BALL = %s\n", wcPtr->Name );
       }
	   else
	   {
	      typeOfGun = GUN_TRACER;
//	      MonoPrint( "Guns: TRACER = %s\n", wcPtr->Name );
	   }
   }
   else
   {
...
//	   MonoPrint( "Guns: SHELL = %s\n", wcPtr->Name );
   ...
   if ( wcPtr->HitChance[ NoMove ] > 0 ||
   		wcPtr->HitChance[ Foot ] > 0 ||
   		wcPtr->HitChance[ Wheeled ] > 0 ||
   		wcPtr->HitChance[ Tracked ] > 0 ||
   		wcPtr->HitChance[ Naval ] > 0 ||
   		wcPtr->HitChance[ Rail ] > 0 )
   {
	   domain = (int)wdGround;
   }

   if ( wcPtr->HitChance[ Air ] > 0 ||
   		wcPtr->HitChance[ LowAir ] > 0 )
   {
	   domain |= (int)wdAir;
   }
   ...
   {
	   minShellRange = 0.0f;
	   maxShellRange = baseRange + baseRange * 0.7f;
   }
   else
   {
	   minShellRange = baseRange - baseRange * 0.7f;
	   maxShellRange = baseRange + baseRange * 0.7f;
   }
    xPos = yPos = zPos = 0;
    pitch = yaw = 0;
...
// We'll use an unused field in the WCD file. The LAST 5 bits will be used and the value will range from 256 to 8192 ft/sec.
...
//	  if ( typeOfGun == GUN_TRACER )
//	      initBulletVelocity = muzzleVel;
//	   else
//	      initBulletVelocity = muzzleVel * .65f;
...
int GunClass::Exec (int* fire, TransformMatrix dmx, ObjectGeometry *geomData, SimObjectType* targetList, BOOL isOwnship )
{
int i;
...
float vt1, dragDelta, cosgam, singam, cosmu, sinmu, alpha;
	...
	if (unlimitedAmmo)
	{
		...
		if (fractionalRoundsRemaining > 1.0f)
		{
			...
			if (*fire && this->parent == FalconLocalSession->GetPlayerEntity())
...
#else
          gridIt = new VuGridIterator(ObjProxList, parent->XPos(), parent->XPos(), 2.5F * NM_TO_FT );
#endif
		 }
...
// END OF MODIFIED SECTION
					...
					fabs (p1.z - fpos.z) < vt + p3.z )
				{
				   ...
				   if ( testFeature->drawPointer->GetRayHit( &org, &vec, &pos, 1.0f ) )
				   {
					  ...
					  message->dataBlock.fPilotID   = ((SimMoverClass*)parent)->pilotSlot;
...
// END OF MODIFIED SECTION
   ...
   if (*fire && (!bulptr->flying || advanceQueue ) )
   {
	  ...
	  if ( parent->IsAirplane() )
	  {
		  if ( !smokeTrail )
		  {
			  smokeTrail = new DrawableTrail(TRAIL_GUN);
