...
GunClass::FireShell( SimObjectType *newTarget )
{
	...
	{
		// release ref
		shellTargetPtr->Release( SIM_OBJ_REF_ARGS );
		shellTargetPtr = NULL;
		return;
	}
...
//		else if ( (rand() & 0x1F ) == 0x1F )
		...
		// else if (40.0f * (float)rand() / 32767.0f * (float)sqrt(shellTargetPtr->localData->range * ((SimMoverClass *)shellTargetPtr->BaseData())->Kias() / 100000.0f) < 0.040625f)
		else if (31.0f * (float)rand() / 32767.0f * (float)sqrt(shellTargetPtr->localData->range * ((SimMoverClass *)shellTargetPtr->BaseData())->Kias() / g_fBiasFactorForFlaks) < 0.0325f)
		{
			hitSomething = TRUE;
			SendDamageMessage(t,
							  0.0f,
...
//			rangeSquare *= 40.0f * PRANDFloatPos();
			rangeSquare *= 40.0f * rand() / 32767.0f * (float)sqrt(shellTargetPtr->localData->range * ((SimMoverClass *)shellTargetPtr->BaseData())->Kias() / g_fBiasFactorForFlaks);

			// 2002-03-12 ADDED BY S.G. Use the ground troop skill if requested
			if (g_bUseSkillForFlaks && parent && parent->IsGroundVehicle())
				rangeSquare *= 7.0 / (float)(4 + ((GroundClass *)parent)->gai->skillLevel * ((GroundClass *)parent)->gai->skillLevel);
		}
	...
	endMessage->dataBlock.fPilotID   = ((SimMoverClass*)parent)->pilotSlot;
