...
EjectedPilotClass::EjectedPilotClass (VU_BYTE** stream) : SimMoverClass(stream)
{
   VU_ID airVuId;
   int ejMode;
   AircraftClass *parent;

   memcpy (&airVuId, *stream, sizeof (VU_ID));
   *stream += sizeof (VU_ID);
   memcpy (&ejMode, *stream, sizeof (int));
   *stream += sizeof (int);

   parent = (AircraftClass *)vuDatabase->Find( airVuId );

   InitData( parent, ejMode, 0);
}
...
////////////////////////////////////////////////////////////////////////////////
//
// class EjectedPilotClass.

void EjectedPilotClass::InitData(AircraftClass *ac, int mode, int no)
{
	DrawableBSP		*acBSP;
	int				labelLen;

//	F4Assert(ac != NULL); - removed since the messages could arrive out of order.
   ...
   F4Assert (_vel[I_Z] > -10000.0F);
	...
	// When mode is set these pointers should be pointing
	// at something.
	F4Assert(_pd != NULL);
	F4Assert(_md != NULL);
	...
	if (ac && no == 0)
		_isPlayer = (SimDriver.playerEntity == ac) ? TRUE : FALSE;
	else
		_isPlayer = FALSE;
	...
	if(_stage >= PD_CHUTE_OPENING)
	{
	   SimMoverClass::ApplyDamage(damageMsg);
		...
		if(_collapseChute == FALSE)
		{
			...
			// ahhhhhhhhhhhhhhhhh
			F4SoundFXSetPos( SFX_SCREAM, TRUE, XPos(), YPos(), ZPos(), 1.0f );
...
void EjectedPilotClass::SetDead(int flag)
{
	if (flag)
   {
//      MonoPrint ("Ejected %s Pilot %d is dead at %8ld\n", DigiOrHumanString(), Id().num_, SimLibElapsedTime);
...
void EjectedPilotClass::HitGround()
{
	...
	{
//		MonoPrint ("Ejected %s Pilot %d lands safely at %8ld\n", DigiOrHumanString(), Id().num_, SimLibElapsedTime);
	}
	else
	{
//		MonoPrint ("Ejected %s Pilot %d lands at %8ld\n", DigiOrHumanString(), Id().num_, SimLibElapsedTime);
...
void EjectedPilotClass::GetFocusPoint
(
	BIG_SCALAR &x,
	BIG_SCALAR &y,
	BIG_SCALAR &z
)
{
	...
	*/
	...
	{
		deltaFocusPoint = finalFocus - _focusPoint;
...
void EjectedPilotClass::InitChuteOpening()
{
	...
	// play the sound
	F4SoundFXSetPos( SFX_CHUTE, TRUE, XPos(), YPos(), ZPos(), 1.0f );
...
//MonoPrint ("Thrust %.2f %.2f %.2f\n", result[I_X], result[I_Y], result[I_Z]);
}
...
void EjectedPilotClass::CalculateDragVector(EP_VECTOR &result) const
{
	...
	windHdg = ((WeatherClass*)TheWeather)->WindHeading;
	
	windVelocity = EP_VECTOR((float)cos(windHdg), (float)sin(windHdg), 0);
	windVelocity *= ((WeatherClass*)TheWeather)->WindSpeedInFeetPerSecond(&pos);
...
char *digiString = "Digi";
char *humanString = "Human";

char *EjectedPilotClass::DigiOrHumanString() const
{
	return (IsPlayerPilot() ? digiString : humanString);
}
