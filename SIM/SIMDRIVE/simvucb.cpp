...
VuEntity* SimVUCreateVehicle (ushort type, ushort, VU_BYTE* data)
{
	...
	if (classPtr->vuClassData.classInfo_[VU_DOMAIN] == DOMAIN_AIR)
	{
      ...
      {
			theVehicle = new MissileClass(&data);
	...
	// JB 010220
			if (classPtr->vuClassData.classInfo_[VU_STYPE] == STYPE_CHAFF &&
				...
				{
				theVehicle = new ChaffClass(&data);
					 if (((SimWeaponClass*)theVehicle)->parent == NULL)
					 {
							delete theVehicle;
							theVehicle = NULL;
				...
				{
				theVehicle = new FlareClass(&data);
					 if (((SimWeaponClass*)theVehicle)->parent == NULL)
					 {
							delete theVehicle;
							theVehicle = NULL;
	...
	// JB 010220
		...
		else if (classPtr->vuClassData.classInfo_[VU_TYPE] == TYPE_EJECT)
		{
			...
			theVehicle = new EjectedPilotClass(&data);
		}
	}
...
VuEntity* SimVUCreateFeature (ushort, ushort, VU_BYTE* data)
{
	SimFeatureClass* theFeature;

	theFeature = new SimFeatureClass (&data);
