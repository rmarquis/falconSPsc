...
// END OF ADDED SECTION
	...
	while (curEmitter)
	{
		...
		if (!IsFiltered (curEmitter) &&
// 2001-03-06 MODIFIED BY S.G. SO OBJECTIVES ARE ALLWAYS CHECKED, WEITHER OR NOT THEY ARE AGGREGATED...
//			curEmitter->IsAggregate() &&                    // A campaign thing
//			(curEmitter->IsAggregate() || curEmitter->IsObjective()) && // A campaign thing or an objective (sim objectives have no sensor routine so they can never make it to the contact list by themself)
			((curEmitter->IsObjective() && curEmitter->GetElectronicDetectionRange(Air) != 0) || curEmitter->IsAggregate()) && // An objective (sim objectives have no sensor routine so they can never make it to the contact list by themself) that has a working radar or a campaign thing
			...
			CanDetectObject (curEmitter))                   // And there is line of sight
		{
			...
			//if(!curEmitter->IsSim() && curEmitter->GetRadarMode()> FEC_RADAR_SEARCH_1 &&curEmitter->GetRadarMode() != FEC_RADAR_SEARCH_100)
...
// END OF ADDED SECTION
		...
		curEmitter = (CampBaseClass*)emitters.GetNext();
	}
	...
	if(g_bRealisticAvionics)
	{
		if(ReleaseManual)
		{
			if(SimDriver.playerEntity)
				SimDriver.playerEntity->ReleaseManualProgram();
		}
		else if(SimDriver.playerEntity && SimDriver.playerEntity->HasPower(AircraftClass::EWSRWRPower))
		{
			if(SimDriver.playerEntity->EWSPGM() != AircraftClass::EWSPGMSwitch::Off &&
				...
				SimDriver.playerEntity->EWSPGM() != AircraftClass::EWSPGMSwitch::Man)
				CheckEWS();
		}
	}
...
//		if(record->entity->IsAirplane())
//me123	now done via symbols in rwr.cpp		DrawStatusSymbol(Hat);
	}
}
...
void PlayerRwrClass::DoAudio( void ) // 2002-02-20 S.G. Just flagging it as not currently being used so there are no confusion
{
	...
	{
		int SEARCH_PERIOD = ((SimBaseClass*)detectionList[i].entity)->RdrCycleTime() * SEC_TO_MSEC;
		if (SEARCH_PERIOD == 0)
		...
		if (detectionList[i].playIt || (!detectionList[i].cantPlay && (!detectionList[i].entity->IsSim() || !detectionList[i].entity->IsDead()))) 
		{
			if ((detectionList[i].missileActivity) ||
				...
				((detectionList[i].isLocked) || 
				 (SimLibElapsedTime - detectionList[i].lastPlayed > SEARCH_PERIOD))))
			{
...
void PlayerRwrClass::DoAudio( DetectListElement *record )
{
	...
	mode = ((BattalionClass*)((SimBaseClass*)record->entity)->GetCampaignObject())->GetRadarMode();
		...
		switch (mode)
			{
			case FEC_RADAR_SEARCH_1:
				SEARCH_PERIOD = radarfileData->Sweeptimesearch1; //me123 from 120
				break;
				
			case FEC_RADAR_SEARCH_2:
				SEARCH_PERIOD = radarfileData->Sweeptimesearch2; //me123 from 60
				break;
				
			case FEC_RADAR_SEARCH_3:
				SEARCH_PERIOD = radarfileData->Sweeptimesearch3; //me123 from 120
				break;
...
void PlayerRwrClass::CheckEWS(void)
{
	...
	if (F4IsBadReadPtr(SimDriver.playerEntity, sizeof(AircraftClass))) // JB 010408 CTD
		return;
	...
	if(LaunchIndication() && !InEWSLoop && !LaunchDetected)
	{
		...
		if(OTWDriver.pCockpitManager->mpIcp->iCHAFF_BQ[SimDriver.playerEntity->EWSProgNum] > 1 ||
			OTWDriver.pCockpitManager->mpIcp->iFLARE_BQ[SimDriver.playerEntity->EWSProgNum] > 1)
		{
			InEWSLoop = TRUE;
			LaunchDetected = TRUE;
		}
	...
	if(!LaunchIndication())
		LaunchDetected = FALSE;
	if(InEWSLoop && !SimDriver.playerEntity->IsExploding())
	{
		if(SimDriver.playerEntity->ChaffCount >= OTWDriver.pCockpitManager->mpIcp->iCHAFF_BQ[SimDriver.playerEntity->EWSProgNum] &&
			SimDriver.playerEntity->ChaffSalvoCount != 0 && !ChaffCheck)
		{
			//Set our timer
			SimDriver.playerEntity->ChaffBurstInterval = SimLibElapsedTime + (OTWDriver.pCockpitManager->mpIcp->fCHAFF_SI[SimDriver.playerEntity->EWSProgNum] * CampaignSeconds);
			//Reset our count
			SimDriver.playerEntity->ChaffCount = 0;
			//Mark us with one less to go
			SimDriver.playerEntity->ChaffSalvoCount--;
			//We've been here already
			ChaffCheck = TRUE;							
		}
		if(SimDriver.playerEntity->FlareCount >= OTWDriver.pCockpitManager->mpIcp->iFLARE_BQ[SimDriver.playerEntity->EWSProgNum] &&
			SimDriver.playerEntity->FlareSalvoCount > 0 && !FlareCheck)
		{
			//Reset our count
			SimDriver.playerEntity->FlareCount = 0;
			//Set our timer
			SimDriver.playerEntity->FlareBurstInterval = SimLibElapsedTime + (OTWDriver.pCockpitManager->mpIcp->fFLARE_SI[SimDriver.playerEntity->EWSProgNum] * CampaignSeconds);
			//Mark us with one less to go
			SimDriver.playerEntity->FlareSalvoCount--;
			//We've been here already
			FlareCheck = TRUE;
		}
		if(SimLibElapsedTime >= SimDriver.playerEntity->ChaffBurstInterval && 
			(SimDriver.playerEntity->ChaffCount < OTWDriver.pCockpitManager->mpIcp->iCHAFF_BQ[SimDriver.playerEntity->EWSProgNum]))
		{					
			SimDriver.playerEntity->EWSChaffBurst();
			ChaffCheck = FALSE;
		}
		if(SimLibElapsedTime >= SimDriver.playerEntity->FlareBurstInterval &&
			(SimDriver.playerEntity->FlareCount < OTWDriver.pCockpitManager->mpIcp->iFLARE_BQ[SimDriver.playerEntity->EWSProgNum]))
		{
			SimDriver.playerEntity->EWSFlareBurst();
			FlareCheck = FALSE;
		}
		if(SimDriver.playerEntity->FlareCount == OTWDriver.pCockpitManager->mpIcp->iFLARE_BQ[SimDriver.playerEntity->EWSProgNum] &&
			SimDriver.playerEntity->ChaffCount == OTWDriver.pCockpitManager->mpIcp->iCHAFF_BQ[SimDriver.playerEntity->EWSProgNum] &&
			SimDriver.playerEntity->ChaffSalvoCount <= 0 && SimDriver.playerEntity->FlareSalvoCount <= 0)
		{
			InEWSLoop = FALSE;
		}
	}
	...
	{ 
		if(HasActivity())
		{
			...
			if (SimDriver.playerEntity->HasSPJamming() && !SimDriver.playerEntity->mFaults->GetFault(FaultClass::blkr_fault) &&
				!SimDriver.playerEntity->ManualECM)
			{
				if(SimDriver.playerEntity->HasPower(AircraftClass::EWSJammerPower))
				{
					if(!SimDriver.playerEntity->IsSetFlag(ECM_ON))
					{
						//Only if fully auto
						if(SimDriver.playerEntity->EWSPGM() == AircraftClass::EWSPGMSwitch::Auto)
						{
							SimDriver.playerEntity->SetFlag(ECM_ON);
						}

						else if(SimDriver.playerEntity->EWSPGM() == AircraftClass::EWSPGMSwitch::Semi)	//ask if we want turn it on
						{
							if(!SaidJammer)
							{
		...
		{ 
			if(!SimDriver.playerEntity->ManualECM && SimDriver.playerEntity->EWSPGM() == AircraftClass::EWSPGMSwitch::Auto)
			{
