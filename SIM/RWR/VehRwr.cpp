...
SimObjectType* VehRwrClass::Exec (SimObjectType*)
{
...
//				((SimBaseClass*)detectionList[i].entity)->RdrCycleTime() * 2.0F * SEC_TO_MSEC)
			if (SimLibElapsedTime > detectionList[i].lastHit + ((SimBaseClass*)detectionList[i].entity)->RdrCycleTime() * SEC_TO_MSEC + 1000 && 
				SimLibElapsedTime > detectionList[i].lastHit + (platform == SimDriver.playerEntity ? 3 * SEC_TO_MSEC : 30 * SEC_TO_MSEC))
			{
...
// 2001-02-19 ADDED BY S.G. SO TARGET WITH A LOCK HAVE AN INCREASE IN LETHALITY (ONLY FOR THE PLAYER)
	if (platform == SimDriver.playerEntity && trackType == Track_Lock)
		lethality += 1.0F;
// END OF ADDED SECTION
...
// END OF ADDED SECTION
		...
		switch (trackType) {
           case Track_Launch:
             if (platform == SimDriver.playerEntity && listElement->missileActivity == 0)
			 {
...
// END OF ADDED SECTION
		...
		{
			if (platform == SimDriver.playerEntity && listElement->missileLaunch == 0)
// 2000-09-03 S.G. SO ARH DON'T GET A LAUNCH WARNING
