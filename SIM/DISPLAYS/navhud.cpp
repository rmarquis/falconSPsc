...
int i=0, a=0;
   ...
   if (PlayerOptions.GetAvionicsType() == ATRealistic || PlayerOptions.GetAvionicsType() == ATRealisticAV)
      a = max (a, 60);
   sprintf (tmpStr, "%4d", min(a, 9999));
   //MI hack to make the new code working when below 100 kts :-(
   if(a < 100)
   {
	   sprintf (tmpStr, " %d", min(a, 9999));
	   tmpStr[4] = '\0';
   }
   ...
   {  
	  ...
	  if(g_bRealisticAvionics)
	  {
		  display->Line(rightEdge * 0.95F, winCenter,
			  rightEdge * 0.95F + hudWinWidth[AIRSPEED_WINDOW] * 0.5F, winCenter);
	  }
	  else
	  {
		  display->Line(rightEdge * 0.95F, winCenter,
	   ...
	   if(((AircraftClass*)ownship)->af->platform->IsPlayer())
	   {
		   ...
		   else if(radarSwitch == RADAR_AUTO)
		   {
			   ...
			   else
			   {
				   ...
				   {
					   if(flash && !((AircraftClass*)ownship)->OnGround())
						   DrawALString();
...
// CLOSING BRACE ADDED BY S.G.
   ...
   else
   {
	   ...
	   if(fpmSwitch != FPM_OFF && scalesSwitch == VV_VAH)
		   ...
		   DrawRollCue();
#else
	   if(FCC && FCC->IsAGMasterMode() || (SimDriver.playerEntity->af->gearPos > 0.5F &&
...
static float HEADING_BOTTOM;
   ...
   switch (FCC->GetMasterMode())
   {
      ...
      case FireControlComputer::AirGroundCamera:
         if (fabs(waypointAz) < (90.0F * DTR))
         {
	         xPos = RadToHudUnits(waypointAz);
	         yPos = RadToHudUnits(waypointEl);
...
//MI draw our WP symbol centered on the WP, not edge-on
			...
			if(g_bRealisticAvionics)
			{
				...
				if(SimDriver.playerEntity && SimDriver.playerEntity->curWaypoint &&
					SimDriver.playerEntity->curWaypoint->GetWPFlags() & WPF_TARGET &&
...
float ttg;
   ...
   if (waypointArrival > 0.0F)
   {
      ...
      // burn any days in the number
	   ...
	   if (hr != 0)
		   sprintf (tmpStr, "%02d %02d:%02d", hr, abs(minute), abs(sec));
	   else if (sec >= 0)
	   {
		   if(!g_bRealisticAvionics)
   ...
   if(!g_bRealisticAvionics)
	   ...
	   display->TextLeft(0.40F, -0.44F, tmpStr1 );
}
...
int HudClass::FindRollAngle(float Alt)
{
	...
	if(Alt <= 3000)
	{
		...
		if(((AircraftClass*)ownship)->af->platform->IsPlayer())
		{
			if((cockpitFlightData.roll * RTD) <= 60 &&
...
void HudClass::DrawOA(void)
{
	...
	else
	{
		...
		case FireControlComputer::AirGroundCamera:
			...
			{
				xPos = RadToHudUnits(OA1Az);
				yPos = RadToHudUnits(OA1Elev);
				display->AdjustOriginInViewport (0.0F, (hudWinY[BORESIGHT_CROSS_WINDOW] +
					hudWinHeight[BORESIGHT_CROSS_WINDOW] * 0.5F));
	...
	else
	{
		...
		case FireControlComputer::AirGroundCamera:
			...
			{
				xPos = RadToHudUnits(OA2Az);
				yPos = RadToHudUnits(OA2Elev);
				display->AdjustOriginInViewport (0.0F, (hudWinY[BORESIGHT_CROSS_WINDOW] +
					hudWinHeight[BORESIGHT_CROSS_WINDOW] * 0.5F));
				display->Line(xPos - TrigWidth/2, yPos - TrigHeight/2, xPos + TrigWidth/2, yPos - TrigHeight/2);
				display->Line(xPos + TrigWidth/2, yPos - TrigHeight/2, xPos, yPos + TrigHeight/2);
...
void HudClass::DrawVIP(void)
{
	...
	else
	{
		...
		case FireControlComputer::AirGroundCamera:
			...
			{
				xPos = RadToHudUnits(VIPAz);
				yPos = RadToHudUnits(VIPElev);
				display->AdjustOriginInViewport (0.0F, (hudWinY[BORESIGHT_CROSS_WINDOW] +
					hudWinHeight[BORESIGHT_CROSS_WINDOW] * 0.5F));
				display->Circle(xPos - Diam/2, yPos, Diam);
				display->AdjustOriginInViewport (0.0F, -(hudWinY[BORESIGHT_CROSS_WINDOW] +
					hudWinHeight[BORESIGHT_CROSS_WINDOW] * 0.5F));
			}
			break;
...
void HudClass::DrawVRP(void)
{
	...
	else
	{
		...
		case FireControlComputer::AirGroundCamera:
			...
			{
				xPos = RadToHudUnits(VRPAz);
				yPos = RadToHudUnits(VRPElev);
				display->AdjustOriginInViewport (0.0F, (hudWinY[BORESIGHT_CROSS_WINDOW] +
					hudWinHeight[BORESIGHT_CROSS_WINDOW] * 0.5F));
...
void HudClass::DrawRALTBox(void)
{
	...
	else
	{
		...
		{ 
			if(radarSwitch == RADAR_AUTO)
				display->TextLeft(0.40F,YRALTText,"AR");
			else if(radarSwitch == BARO)
				display->TextLeft(0.40F,YRALTText,"R");	
		}
	}	   
}
...
void HudClass::DrawCruiseIndexes(void)
{
	...
	case 1:	//Cruise RNG
		//Speed
		if(SimDriver.playerEntity &&  SimDriver.playerEntity->af)
		{
			DrawAirSpeedCarret(SimDriver.playerEntity->af->GetOptKias(2));
			sprintf(SpeedText, "%.0f", max(min(SimDriver.playerEntity->af->GetOptKias(2), 9999.0F), 0.0F));
		}
		break;
	case 2:	//Cruise Home
		//opt speed and alt with climb profile and such
		if(OTWDriver.pCockpitManager && OTWDriver.pCockpitManager->mpIcp && SimDriver.playerEntity)
		{
			float xCurr, yCurr, zCurr = 0;
			WayPointClass *wp = SimDriver.playerEntity->GetWayPointNo(
				OTWDriver.pCockpitManager->mpIcp->HomeWP);
			if(wp)
			{		
				wp->GetLocation(&xCurr, &yCurr,&zCurr);
				float deltaX			= xCurr - SimDriver.playerEntity->XPos();
				float deltaY			= yCurr - SimDriver.playerEntity->YPos();
				float distanceToSta	= (float)sqrt(deltaX * deltaX + deltaY * deltaY);

				if (SimDriver.playerEntity->af)
				{
					// we are within glide distance let's go max rng and decent
					float decentpoint = ((5000.0f - SimDriver.playerEntity->ZPos()*0.8f)/1000.0f)/
						(max(100.0f,(float)SimDriver.playerEntity->af->GetDragIndex())/100.0f);
					...
					else if (fabs(-SimDriver.playerEntity->ZPos() - SimDriver.playerEntity->af->GetOptimumAltitude()) < 200.0f  &&
						fabs(SimDriver.playerEntity->ZDelta())< 50.0f ||
						fabs(-SimDriver.playerEntity->ZPos() - SimDriver.playerEntity->af->GetOptimumAltitude()) < 1000.0f  &&
						SimDriver.playerEntity->ZDelta()> 0.0f ||
						-SimDriver.playerEntity->ZPos() > SimDriver.playerEntity->af->GetOptimumAltitude())
					{
						DrawAirSpeedCarret(SimDriver.playerEntity->af->GetOptKias(2));
						DrawAltCarret(SimDriver.playerEntity->af->GetOptimumAltitude());
						sprintf(SpeedText, "%.0f", max(min(SimDriver.playerEntity->af->GetOptKias(2), 9999.0F), 0.0F));
					}
					// we are in the climp towards best cruice altitude
					else
					{
						DrawAirSpeedCarret(SimDriver.playerEntity->af->GetOptKias(0));
						DrawAltCarret(SimDriver.playerEntity->af->GetOptimumAltitude());
						sprintf(SpeedText, "%.0f", max(min(SimDriver.playerEntity->af->GetOptKias(0), 9999.0F), 0.0F));
					}
				}
			}
		}	
	...
	case 3:	//Cruise Edr
		...
		if(SimDriver.playerEntity &&  SimDriver.playerEntity->af)
		{
			DrawAirSpeedCarret(SimDriver.playerEntity->af->GetOptKias(1));
			sprintf(SpeedText, "%.0f", max(min(SimDriver.playerEntity->af->GetOptKias(1), 9999.0F), 0.0F));
		}
		break;
	default:
		sprintf(SpeedText, "");
