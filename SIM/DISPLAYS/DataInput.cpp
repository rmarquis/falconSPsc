...
void SmsDrawable::CNTLPage(void)
{
	...
	display->TextLeft(x + 0.1, y, "AD1 2.00SEC");
	y -= step;
	display->TextLeft(x, y, "4", C4Weap ? 2 : 0);
	display->TextLeft(x + 0.1, y, "AD2 2.50SEC");
	y -= step;
	display->TextLeft(x + 0.1, y, "BA 75FT");

	//OSB18
	GetButtonPos(17, &x, &y);
	y += step;
	display->TextLeft(x, y, "C", C3Weap ? 2 : 0);
	display->TextLeft(x + 0.1, y, "AD 12.25SEC");
	y -= step;
	display->TextLeft(x, y, "3", C3Weap ? 2 : 0);
	display->TextLeft(x + 0.1, y, "BA 100FT");
...
void SmsDrawable::CDisplay(void)
{
	...
	if(InputModus == C1)
	{
		...
		if(InputLine <= 0)
		{
			//Line1
			if(!Manual_Input)
				sprintf(inputstr, "%.2fSEC", Sms->C1AD1 / 100);
			else
				FillInputString();
