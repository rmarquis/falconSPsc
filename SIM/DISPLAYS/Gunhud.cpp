...
// 2001-04-09 ADDED  BY S.G. IF WE HAVE NO GUNS ONBOARD, DON'T DO GUNS DISPLAY STUFF
   ...
   if (targetPtr)
   {
	  ...
	  if(!g_bRealisticAvionics)
		  ...
		  display->TextLeft(0.40F, -0.46F, tmpStr);
   ...
   if(!g_bRealisticAvionics)
	   ...
	   display->TextLeft(0.40F, -0.39F, tmpStr);
   ...
   if(g_bRealisticAvionics)
   {
	   if(ownship->fireGun && ownship->Sms->MasterArm() != SMSBaseClass::Safe)
	   {
...
float		rx, ry, rz;
	...
	for (i=0; i<NumEEGSSegments; i++)
	{
		...
		bulletH[i] = RadToHudUnits((float)atan2 (ry,rx));
		bulletV[i] = RadToHudUnits((float)atan (-rz/(float)sqrt(rx*rx+ry*ry+.1)));
...
float offset = 0.04F;
	...
	xPos = RadToHudUnits(targetData->az);
	yPos = RadToHudUnits(targetData->el);
