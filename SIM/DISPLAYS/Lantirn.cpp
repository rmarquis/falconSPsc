...
void LantirnClass::DrawTerrain() 
{
	...
	mlTrig tha, psi, phi;
	mlSinCos(&tha, SimDriver.playerEntity->Pitch()+m_dpitch);
	mlSinCos(&psi, SimDriver.playerEntity->Yaw());
	mlSinCos(&phi, SimDriver.playerEntity->Roll());
...
void LantirnClass::GetCameraPos(Tpoint *pos)
{
    ...
    pos->z = Origin.z;
		...
		{
			pos->x = SimDriver.playerEntity->af->GetLantirnCameraX() * OTWDriver.Scale();	//external variables in a/c .dat file
			pos->y = SimDriver.playerEntity->af->GetLantirnCameraY() * OTWDriver.Scale();	//that describe camera position for each
			pos->z = SimDriver.playerEntity->af->GetLantirnCameraZ() * OTWDriver.Scale();	//aircraft that has LANTIRN
		}
		else
		{
...
void LantirnClass::Exec(AircraftClass* self)
{
	if(g_bTFRFixes)
	{
		...
		if (self->Kias() < self->af->GetTFR_Corner() * self->af->GetSlowPercent())
			SpeedUp = TRUE;
...
float LantirnClass::GetGLimit ()
{
	if(g_bTFRFixes)
	{
		if(SimDriver.playerEntity && SimDriver.playerEntity->af)
		{
			...
			{
			case TFR_SOFT:
				return SimDriver.playerEntity->af->GetTFR_SoftG();
			default:
			case TFR_MED:
				return SimDriver.playerEntity->af->GetTFR_MedG();
			case TFR_HARD:
				return SimDriver.playerEntity->af->GetTFR_HardG();
			}
		}
		else
		{
