...
float hat, theAlt;
   ...
   hat = cockpitFlightData.z - OTWDriver.GetGroundLevel (SimDriver.playerEntity->XPos(), SimDriver.playerEntity->YPos());
   // Max hat if no rad alt
   if (SimDriver.playerEntity->mFaults && SimDriver.playerEntity->mFaults->GetFault(FaultClass::ralt_fault))
      hat = -999999.9F;
   ...
   else
   {
	...
	airspeed		= FloatToInt32(SimDriver.playerEntity->Kias());
