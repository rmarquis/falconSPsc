...
void MenuManager::InitPage() {
	mCallerIdx		= SimDriver.playerEntity->GetCampaignObject()->GetComponentIndex(SimDriver.playerEntity);
	mNumInFlight	= ((FlightClass*) SimDriver.playerEntity->GetCampaignObject())->GetTotalVehicles();
...
// 2002-03-07 MN of course only AWACS from our team - doh!
		if (!cf->IsFlight() || cf->Dead())
			continue;
		if (cf->GetUnitMission() == AMIS_AWACS && cf->GetTeam() == SimDriver.playerEntity->GetTeam())
		{
			mAWACSavail = true;
		}
	}
...
void MenuManager::CheckItemConditions(BOOL poll)
{
	...
	for(i = 0; i < mNumItems; i++) {
		if(poll == FALSE || mpPage->mpItems[i].mPoll) {
		...
		if(mIsActive) {
			...
			if(item != -1) {
				...
				if(pitem->mIsAvailable) {
					...
					mTargetId = AiDesignateTarget(SimDriver.playerEntity);
...
{	
	...
	if (sendRequest && PlayerOptions.PlayerRadioVoice)
	{
		if( SimDriver.playerEntity)
		{
			...
			case FalconATCMessage::RequestClearance:
				SendCallToATC(SimDriver.playerEntity, ATCId, rcLANDCLEARANCE, FalconLocalGame);
				break;
			case FalconATCMessage::RequestEmerClearance:
				SendCallToATC(SimDriver.playerEntity, ATCId, rcLANDCLEAREMERGENCY, FalconLocalGame);
				break;
			case FalconATCMessage::RequestTakeoff:
				SendCallToATC(SimDriver.playerEntity, ATCId, rcREADYFORDERARTURE, FalconLocalGame);
				break;
			case FalconATCMessage::RequestTaxi:
				SendCallToATC(SimDriver.playerEntity, ATCId, rcREADYFORDERARTURE, FalconLocalGame);
				break;
			case FalconATCMessage::AbortApproach:
				SendCallToATC(SimDriver.playerEntity, ATCId, rcABORTAPPROACH, FalconLocalGame);
				break;
	...
	if(SimDriver.playerEntity)
		atcMsg->dataBlock.from	= SimDriver.playerEntity->Id();
	else
...
void MenuSendAwacs(int enumId, VU_ID targetId, int sendRequest)
{
	...
	if (sendRequest && PlayerOptions.PlayerRadioVoice)
		{
		AircraftClass *plane = SimDriver.playerEntity;
		switch (enumId)
			{
			case FalconAWACSMessage::Unable:
				SendCallToAWACS(plane, rcUNABLE, FalconLocalGame);
				break;
			case FalconAWACSMessage::Wilco:
				SendCallToAWACS(plane, rcCOPY, FalconLocalGame);
				break;
			case FalconAWACSMessage::Judy:
//				SendCallToAWACS(plane, rcJUDY);
				break;
			case FalconAWACSMessage::RequestPicture:
				SendCallToAWACS (plane, rcPICTUREQUERY, FalconLocalGame);
				break;
			case FalconAWACSMessage::GivePicture:
				break;
			case FalconAWACSMessage::RequestHelp:
				radioMessage = CreateCallToAWACS (plane, rcNEEDHELP, FalconLocalGame);
				radioMessage->dataBlock.edata[1] -= VF_FLIGHTNUMBER_OFFSET; // jpo - rewrite this...
				radioMessage->dataBlock.edata[2] = SimToGrid(plane->YPos());
				radioMessage->dataBlock.edata[3] = SimToGrid(plane->XPos());
			...
			case FalconAWACSMessage::RequestRelief:
				...
				if(flight)
					...
					if (!hasWeaps)
						SendCallToAWACS(plane, rcENDCAPARMS, FalconLocalGame);
					else
						SendCallToAWACS(plane, rcENDCAPFUEL, FalconLocalGame);
					}

				break;
			case FalconAWACSMessage::RequestDivert:
				SendCallToAWACS(plane, rcREQUESTTASK, FalconLocalGame);
				break;
			case FalconAWACSMessage::RequestSAR:
				SendCallToAWACS(plane, rcSENDCHOPPERS, FalconLocalGame);
...
//				SendCallToAWACS(plane, rcVAMOOSE);
			...
			case FalconAWACSMessage::VectorHome:
				SendCallToAWACS(plane, rcVECTORHOMEPLATE, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToAltAirfield: 
				SendCallToAWACS(plane, rcDIVERTFIELD, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToPackage:
				SendCallToAWACS(plane, rcVECTORTOPACKAGE, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToTanker:
				SendCallToAWACS (plane, rcREQUESTVECTORTOTANKER, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToCarrier:	// Carrier
				SendCallToAWACS (plane, rcVECTORTOCARRIER, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToThreat:
				SendCallToAWACS(plane, rcVECTORTOTHREAT, FalconLocalGame);
				break;
			case FalconAWACSMessage::VectorToTarget:
				SendCallToAWACS(plane, rcVECTORTOTARGET, FalconLocalGame);
				break;
			case FalconAWACSMessage::DeclareAircraft:
				SendCallToAWACS(plane, rcDECLARE, FalconLocalGame);
				break;
			default:
				break;
			}
	...
	pawacsMsg						= new FalconAWACSMessage (SimDriver.playerEntity->Id(), FalconLocalGame);
...
{
	AiSendPlayerCommand(enumId, extent, AiDesignateTarget(SimDriver.playerEntity));
}
...
void MenuSendTanker(int enumId)
{
	...
	TankerMsg->dataBlock.caller	= SimDriver.playerEntity->Id();
