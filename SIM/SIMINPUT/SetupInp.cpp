...
int flags, buttonId, mouseSide;
   ...
   {
      flags = (state & MODS_MASK) + (CommandsKeyCombo << SECOND_KEY_SHIFT) + (CommandsKeyComboMod << SECOND_KEY_MOD_SHIFT);
      theFunc = UserFunctionTable.GetFunction(val, flags, &buttonId, &mouseSide);

      // Cancel the combo, whether it is handled or not
      if (CommandsKeyCombo && keyDown && theFunc != ScreenShot && theFunc != RadioMessageSend && theFunc != OTWRadioMenuStep && theFunc != OTWRadioMenuStepBack)
      {
         CommandsKeyCombo = 0;
      ...
      if (theFunc) {
			...
			if(buttonId < 0) {
				...
				if(SimDriver.playerEntity && 
				   ...
				   !((AircraftClass*)SimDriver.playerEntity)->ejectTriggered) {
