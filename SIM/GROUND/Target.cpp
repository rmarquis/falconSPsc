...
void GroundClass::SelectWeapon(int gun_only)
	...
	if (targetPtr->BaseData()->IsSim())
      ...
      else if (!targetPtr->BaseData()->IsStatic() && ((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject())
      {
		UnitClassDataType	*ud;

      	classPtr = (Falcon4EntityClassType*) ((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject()->EntityType();
	...
	//range = FloatToInt32(Distance(XPos(),YPos(),targetPtr->BaseData()->XPos(),targetPtr->BaseData()->YPos()) * FT_TO_KM + 0.5F);
	float dx, dy, dz;
	...
	Sms->SelectBestWeapon (dam, mt, range, gun_only, (int)dz); // 2002-03-09 MODIDIED BY S.G. Passed dz to the function so weapon is chosen depending on its altitude capacity as well (dz is positive if we are below targetPtr)
	}
