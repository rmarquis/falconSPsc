		...
		if (Gun)
			...
			if ( Gun->IsTracer() )
			{
				...
				if ( Gun->numFlying )
				{
					...
					{
						fire = TRUE;
						F4SoundFXSetPos( SFX_MCGUN, 0, XPos(), YPos(), ZPos(), 1.0f );
	...
	if (theWeapon->IsGun())
	{
		...
		if (GunTrack())
		{
			if (!IsGunFiring(Sms->GetCurrentWeaponHardpoint()) || Gun->IsShell() )
			{
				...
				if 	(!Gun->FiremsgsendTime) SendFireMessage ((SimWeaponClass*)Gun, FalconWeaponsFire::GUN, TRUE, targetPtr);
...
// 2000-10-27 ADDED BY S.G. SO GUNS CAN HAVE DIFFERENT FIRE RATE... BUT NOT IN RP4 SO REMOVED FROM HERE AS WELL
...
// END OF MODIFIED SECTION
	...
	else if (theWeapon->IsMissile())
	{
		...
		MissileClass	*theMissile = (MissileClass*)theWeapon;
		if (MissileTrack())
		{
			if(gai->battalionCommand && /* targetPtr->BaseData()->IsSim() &&*/ !targetPtr->BaseData()->OnGround()) // 2002-02-28 MODIFIED BY S.G. Valid for campaign object as well, not just sim! We're testing inside for SIM or CAMPAIGN ANYWAY! Otherwise nextSamFireTime is NEVER adjusted
			{
				gai->battalionCommand->self->allowSamFire = FALSE;
				int numVeh = 1;
				if(targetPtr->BaseData()->IsSim()) { // 2002-02-28 MODIFIED BY S.G. Different way of doing Joel's "011018" CTD fix. If it's not a sim object, it doesn't have a campaignObject member variable so that test isn't valid
					if (((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject())
						numVeh = max(1, ((Unit)((SimBaseClass*)targetPtr->BaseData())->GetCampaignObject())->GetTotalVehicles()/2);
				}
				else
					numVeh = max(1, ((Unit)targetPtr->BaseData())->GetTotalVehicles()/2);
				
				int rate;
// 2002-02-28 MODIFIED BY S.G. A GetRadarType() will return 0 (RDR_NO_RADAR) if it has no radar but radarDatFileTable has something there so it's being used. That's not right.
//				if (radarData->AirFireRate) rate = radarData->AirFireRate;
				...
				ShiAssert (vc);
				// If the vehicle's VEH_USES_UNIT_RADAR flag is set, forget about your own (if you have one) and use the unit's radar vehicle
...
//				   target->OnGround() ? "Ground Unit" : "Air Unit" );
		...
		// sound effect
		if ( rand() & 1 )
			F4SoundFXSetPos( SFX_BIGGUN1, 1, XPos(), YPos(), ZPos(), 1.0f );
		else
			F4SoundFXSetPos( SFX_BIGGUN2, 1, XPos(), YPos(), ZPos(), 1.0f );
...
//			return FALSE;
		if (targetPtr->localData->range > theMissile->GetRMax(-target->ZPos(), 0, 
			targetPtr->localData->az, targetPtr->BaseData()->Vt(), targetPtr->localData->ataFrom))
			return FALSE;
...
/*	static const float BUMP_AMOUNT = 30.0f*DTR;
	...
	//if (el < 45.0f*DTR) {
	float BUMP;
	if (!((BattalionClass*)GetCampaignObject())->GetMissilesFlying())
	BUMP = radarData->Elevationbumpamounta;
	else
	BUMP = radarData->Elevationbumpamountb;
