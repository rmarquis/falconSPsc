...
#include "update.h"
#include "aircrft.h"
/* S.G. */#include "simbase.h"
/* S.G. */#include "Object.h"
/* MN */#include "Find.h"
/* MN */#include "Classtbl.h"
...
//		MonoPrint("BATALLION LEAD.  Squad = %d Platoon = %d Company = %d\n",
//				squad_id,
//				platoon_id,
//				company_id );
	...
	else if (rank & GNDAI_COMPANY_LEADER)
	{
		...
		company_id = unit_id;
//		MonoPrint("COMPANY LEAD.  Squad = %d Platoon = %d Company = %d\n",
//				squad_id,
//				platoon_id,
//				company_id );
	}
...
//		MonoPrint("PLATOON LEAD.  Squad = %d Platoon = %d Company = %d\n",
//				squad_id,
//				platoon_id,
//				company_id );
	}
	else
	{
		...
		company_id = l->company_id;
//		MonoPrint("SQUAD LEAD.  Squad = %d Platoon = %d Company = %d\n",
//				squad_id,
//				platoon_id,
//				company_id );
	}
...
{
	ShiAssert (battalionCommand != this);
	...
	// to this.
#ifdef DEBUG
	if (self->GetCampaignObject() != NULL) {
	VuListIterator	vehicleWalker( self->GetCampaignObject()->GetComponents() );
	GroundClass*	theObj = (GroundClass*)vehicleWalker.GetFirst();
	
	while (theObj)
		{
		ShiAssert( theObj->gai != this );
		ShiAssert( theObj->gai->battalionCommand != this );
		ShiAssert( theObj->gai->leader != this );
...
void GNDAIClass::SetGroundTarget( SimObjectType *newTarget )
{
	...
	if (gndTargetPtr) {
		gndTargetPtr->Release( SIM_OBJ_REF_ARGS );
	}
...
void GNDAIClass::ProcessTargeting ( void )
	...
	ShiAssert( battalionCommand );
	ShiAssert( (int)battalionCommand != 0xdddddddd );
	ShiAssert( (int)battalionCommand->self != 0xdddddddd );
	...
	if ( this == battalionCommand )
		...
		if (campTargetEntity)
		{
			...
			{
#ifdef DEBUG
				SetGroundTarget( new SimObjectType(OBJ_TAG, self, campTargetEntity) );
#else
		...
		if (campTargetEntity)
		{
			...
			{
#ifdef DEBUG
				SetAirTarget( new SimObjectType(OBJ_TAG, self, campTargetEntity) );
#else
...
// END OF ADDED SECTION
			...
			{
#ifdef DEBUG
				SetAirTarget( new SimObjectType(OBJ_TAG, self, best) );
#else
	...
	procTime = GetTickCount();
		...
		if (self->isEmitter && !self->targetPtr->BaseData()->OnGround())
		{
			...
			float range = 0.0f;
			int i =0;
			/* 2002-03-21 MODIFIED BY S.G. If we're the emitter, we're also the radar's platform. Therefore radar->platform->targetPtr is self->targetPtr. There's an easier way to do this plus do some checks about our target state
			for (i=0; i<((SimMoverClass*)(SimVehicleClass*)self)->numSensors; i++)
			{
				...
				if (radar->platform->targetPtr->localData->sensorState[i]>= SensorClass::Detection)
				detecting = TRUE;
				if (radar->platform->targetPtr->localData->range > range) range = radar->platform->targetPtr->localData->range ;
			}
			*/

			// If handoff switched target, localData will have zeros only, fix this by calling CalcRelAzElRangeAta and then run a sensor sweep
			...
			if (localData->ataFrom == 0.0f && localData->az == 0.0f  && localData->el == 0.0f && localData->range == 0.0f) {
				...
				radar->SetFlag(RadarClass::FirstSweep); // Flag it as a first sweep so it doesn't have to wait before locking
				radar->Exec(self->targetList);
			}
			// If our radar has no target or if it has the SAME base object as us but different target pointers, switch our radar target pointer to our target pointer
			else if (!radar->CurrentTarget() || (radar->CurrentTarget() != self->targetPtr && radar->CurrentTarget()->BaseData() == self->targetPtr->BaseData())) {
				radar->SetDesiredTarget(self->targetPtr);
...
sprintf(label,"OFF");
	...
	if (tracking)
		sprintf (label,"Track");
			switch (self->GetCampaignObject()->StepRadar(tracking,detecting, range) )//me123 modifyed to take tracking/detection parameter))
			{
...
//					ideal_x = (short)GridToSim(gridY + dy[moveDir]);
					ideal_y = (float)GridToSim(gridX + dx[moveDir]);
					ideal_x = (float)GridToSim(gridY + dy[moveDir]);
					deltaX = ideal_x - self->XPos() ;
					deltaY = ideal_y - self->YPos() ;
...
void FindVehiclePosition(SimInitDataClass *initData)
{
	...
	int		formation = ((UnitClass*)(initData->campUnit))->GetUnitFormation();
	int		cur_leader_idx;
