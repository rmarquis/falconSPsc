...
//	MonoPrint ("Feature %d took %d hits at %8ld (%d%% damaged)\n", Id().num_, (int)hitPoints, SimLibElapsedTime,(int)(100.0F * (1.0F - pctStrength)));
...
//					vec.z  );
				...
				// test explosion
				OTWDriver.AddSfxRequest(
					new SfxClass (SFX_RISING_GROUNDHIT_EXPLOSION_DEBRISTRAIL,				// type
					&ppos,							// world pos
					&vec,
...
void UpdateDrawableObject (SimFeatureClass *theFeature)
{
	...
	if (theFeature->IsAwake())
		OTWDriver.CreateVisualObject(theFeature, OTWDriver.Scale());

	// if we're damaged,, use the damaged texture set
	if ( (theFeature->Status() & VIS_TYPE_MASK) == VIS_DAMAGED)
	{
		 if ( theFeature->drawPointer )
         	((DrawableBSP*)(theFeature->drawPointer))->SetTextureSet(1);
	}
}
