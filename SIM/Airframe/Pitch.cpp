...
void AirframeClass::Pitch(void)
{
	...
	float aoacmd, ptcmd, maxNegGs, maxCmd, minCmd;
...
// 2002-03-12 MN use 9.0f G's only for F-16 and player entity - AI planes can use this code, too
			if(SimDriver.playerEntity && platform == SimDriver.playerEntity && platform->IsAirplane() && platform->IsF16()) // 2002-03-19 MODIFIED BY S.G. Lets make sure it's an airplane first. Seems odd to happen here but it CTD after ejecting (BT 1071)
...
// 2002-03-12 MN Check for player - if not, use curMaxGs (which is the case for all AI planes)
			if(SimDriver.playerEntity && SimDriver.playerEntity->IsAirplane() && this == SimDriver.playerEntity->af && platform->IsF16())  // 2002-03-19 MODIFIED BY S.G. Lets make sure it's an airplane first. SimDriver.playerEntity->af might be invalid if the player ejects
