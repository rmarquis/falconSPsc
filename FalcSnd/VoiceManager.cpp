...
DWORD WINAPI VoiceManagementThread( LPVOID lpvThreadParm ) 
{
	...
	while( !killThread )
	{
		...
		for ( i = NUM_VOICE_CHANNELS -1 ; i > -1 ; i-- )
		{
			...
			while(pVC && (pVC->node->playTime <= vuxGameTime) )
			{
				pVCnext = pVC->next;
				...
				{
					//MonoPrint("Dropping old message: %d  Hold Time: %d\n", pVC->node->message, holdTime/CampaignSeconds);
				...
				else if(FilterMessage(pVC->node))
				{
					...
					else if( best && best->node->from == FalconLocalSession->GetPlayerEntityID())
					{
						if(	pVC->node->from == FalconLocalSession->GetPlayerEntityID())
						{
							VM->VMListRemoveVCQ(&voiceChannelQueue[i], best);
							best = pVC;
						}
					}
					else if(pVC->node->from == FalconLocalSession->GetPlayerEntityID())
					{
						best = pVC;
					}
					else if( best && best->node->to == FalconLocalSession->GetPlayerEntityID())
					{
						if(	pVC->node->to == FalconLocalSession->GetPlayerEntityID() && pVC->node->priority < best->node->priority)
							best = pVC;
					}
					else if( pVC->node->to == FalconLocalSession->GetPlayerEntityID() )
					{
						best = pVC;
					}
					else if(best && pVC->node->priority < best->node->priority )
					{
						best = pVC;
					}
				}
			...
			if(SimDriver.playerEntity && best)
			{	
				if(	(VM->decompQueue[i].from != FalconLocalSession->GetPlayerEntityID() &&
					best->node->priority == rpLifeThreatening && VM->decompQueue[i].priority != rpLifeThreatening) ||
					best->node->from == FalconLocalSession->GetPlayerEntityID())
						breakin = TRUE;
			}
			else if(best && best->node->priority == rpLifeThreatening && VM->decompQueue[i].priority != rpLifeThreatening)
			{
				breakin = TRUE;
			}
...
#ifdef DAVE_DBG
			//if(best)
				//MonoPrint("Message chosen   chan: %d mess: %d brkin: %d curChan: %d  chanSatus: %d\n",i,best->node->message,breakin, curChannel, VM->decompQueue[i].status);
#endif

			if( curChannel <= i )
			{
				//if there is a message queued from PlayRadioMessage and the appropriate decompQueue is available
				if( best &&  (VM->decompQueue[i].status == SLOT_IS_AVAILABLE  || breakin) && !VM->falconVoices[curChannel].exitChannel )
				{	
					...
					// this tells the Acton Camera System who's current
					// message is from
...
#ifdef DAVE_DBG
					//MonoPrint("Conversation ready to decompress channel: %d message: %d\n",i,VM->decompQueue[i].message);
#endif
...
/*		if(vuxGameTime - oldtime > 2000)
		{
			MonoPrint(	"Buffer Status: Ch0:%d %d Ch1:%d %d\n",
				VM->falconVoices[0].voiceBuffers[0].status,
				VM->falconVoices[0].voiceBuffers[1].status,
				VM->falconVoices[1].voiceBuffers[0].status,
				VM->falconVoices[1].voiceBuffers[1].status);
			oldtime = vuxGameTime;
		...
		{
			int buffChnl=curBuffer,leave = FALSE;
...
{
	return ( VM->falconVoices[channel].voiceBuffers[0].status == BUFFER_NOT_IN_QUEUE && \
...
void VoiceManager::AddNoise(VOICE_STREAM_BUFFER *streamBuffer, VU_ID from, int channel)
{
	...
	{
		flight = (Flight)SimDriver.playerEntity->GetCampaignObject();
		if(flight)
		{
			// AWACS/FAC callsign
	...
	if(fromEnt && ownship && fromEnt != SimDriver.playerEntity && !nonoise && SimDriver.InSim())
	{
