...
enum
{
	...
	SND_LOOP_SAMPLE     =DSBSTATUS_LOOPING,
	SND_USE_3D          =0x00000100,
	SND_OVERRIDE        =0x00000400,
	SND_EXCLUSIVE       =0x00000800,
...
// Modifying sample playback
	...
	BOOL SetSamplePosition(long ID, float x, float y, float z);
	BOOL Disable3dSample(long ID);

// Querying a Sample
	int GetSampleVolume(long ID);
	BOOL IsSamplePlaying(long ID);
...
// Function to pass data into the Stream buffer
	...
	// 3d stuff
	void SetCameraPostion(Tpoint *camPos, Trotation *camRot);
