...
****************************************************************************/
void VoiceFilter::PlayRadioMessage( char talker, short msgid, short *data, VU_TIME playTime, 
...
//	char			*dcommPtr;
	...
	//if( (SimDriver.playerEntity && SimDriver.playerEntity->IsEject()) || (VM && VM->falconVoices[channel].exitChannel) || killThread )
	if( (SimDriver.playerEntity && !F4IsBadReadPtr(SimDriver.playerEntity, sizeof(AircraftClass)) && SimDriver.playerEntity->IsEject()) || (VM && !F4IsBadReadPtr(VM, sizeof(VoiceManager)) && VM->falconVoices[channel].exitChannel) || killThread ) // JB 010405 CTD
	{
		int player = 0, exit = 0;
		...
		//MonoPrint("Dumping Message: %d  Player: %d  ExitChan: %d KillThread: %d\n", msgid, player, exit, killThread);
		return;
	}
	...
	if(msgid < 0)
		msgid = 0;
	else if(msgid>= commfile.MaxComms()) // jpo dynamic calc
		msgid = 0;
...
//	commHdrInfo = (COMM_FILE_INFO *)dcommPtr;
	...
	ShiAssert(FALSE == IsBadReadPtr(commHdrInfo, sizeof *commHdrInfo));
...
//	dcommPtr = commData + commHdrInfo->commOffset;
	...
	ShiAssert(FALSE == IsBadReadPtr(commInfo, sizeof *commInfo * commHdrInfo->totalElements));
    ...
    #ifdef USE_SH_POOLS
	dfileNum = message.conversations = (short *)MemAllocPtr( gTextMemPool,  sizeof(short)*(message.sizeofConv), FALSE );
	#else
...
//  dEvalData = evalData + (evalHdrNumber * sizeof( EVAL_FILE_INFO ));
  ...
  ShiAssert(FALSE == IsBadReadPtr(eEvalData, sizeof *eEvalData));
...
//  dEvalData = evalData + ((EVAL_FILE_INFO *)dEvalData)->evalOffset;
  ...
  ShiAssert(FALSE == IsBadReadPtr(eEvalElem, sizeof *eEvalElem));
...
//  fragHdrInfo = (FRAG_FILE_INFO *)(fragData + (sizeof(FRAG_FILE_INFO)*fragNumber));
  ...
  ShiAssert(FALSE == IsBadReadPtr(fragHdrInfo, sizeof *fragHdrInfo));
...
//  dFragData = (SPEAKER_TO_FILE*)(fragData + fragHdrInfo->fragOffset);
  ...
  ShiAssert(FALSE == IsBadReadPtr(dFragData, sizeof *dFragData));
  ...
  for(index = 0; index < fragHdrInfo->totalSpeakers; index++)
  {
	  ...
	  headerInfo++;
	  ShiAssert(FALSE == IsBadReadPtr(headerInfo, sizeof *dFragData));
  }
