	...
	while(cur)
	{
		if(cur->Control_->GetFlags() & C_BIT_ABSOLUTE)
		{
			if(cur->Control_->MouseOver(relx,rely,lastover))
				Found_=cur->Control_->GetMe(); // 99% of the time this will return cur->Control_ except in the case of piggyback controls (like trees)
		}
		else if(relx >= ClientArea_[cur->Control_->GetClient()].left && relx <= ClientArea_[cur->Control_->GetClient()].right && rely >= ClientArea_[cur->Control_->GetClient()].top && rely <= ClientArea_[cur->Control_->GetClient()].bottom)
		{
				if(cur->Control_->MouseOver(relx-VX_[cur->Control_->GetClient()],rely-VY_[cur->Control_->GetClient()],lastover))
...
void C_Window::BlitTranslucent(SCREEN *surface,COLORREF color,long Perc,UI95_RECT *rect,long Flags,long Client)
{
	...
	for(i=d.top;i < d.bottom;i++)
	{
		...
		{
			r=UIColorTable[operc][rc + UIColorTable[bgperc][(surface->mem[didx] >> r_shift_) & 0x1f]];
			g=UIColorTable[operc][gc + UIColorTable[bgperc][(surface->mem[didx] >> g_shift_) & 0x1f]];
			b=UIColorTable[operc][bc + UIColorTable[bgperc][(surface->mem[didx] >> b_shift_) & 0x1f]];
...
void C_Window::CustomBlitTranslucent(SCREEN *surface,COLORREF color,long Perc,UI95_RECT *rect,long Flags,long Client)
{
	...
	for(i=d.top;i < d.bottom;i++)
	{
		...
		for(j=d.left;j < d.right;j++)
		{
			...
			if(subx && suby)
				sub=(long)sqrt((double)(subx*subx + suby*suby));
			else
				sub=subx+suby;
			sub=TableVal[sub];
			...
			if(sub < 17)
			{
				r=UIColorTable[operc][UIColorTable[Perc-sub][rc] + UIColorTable[bgperc+sub][(surface->mem[didx] >> r_shift_) & 0x1f]];
				g=UIColorTable[operc][UIColorTable[Perc-sub][gc] + UIColorTable[bgperc+sub][(surface->mem[didx] >> g_shift_) & 0x1f]];
				b=UIColorTable[operc][UIColorTable[Perc-sub][bc] + UIColorTable[bgperc+sub][(surface->mem[didx] >> b_shift_) & 0x1f]];
