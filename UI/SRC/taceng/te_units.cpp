...
void tactical_create_battalion (long, short hittype, C_Base *)
{
	...
	vuDatabase->QuickInsert(new_battalion);
...
static void update_new_battalion_window (void)
{
	...
	for (loop = 0; loop < VEHICLES_PER_UNIT; loop ++)
	{
		types[loop] = 0;
		num[loop] = 0;
	}
	...
	if (uc)
	{
		for (loop = 0; loop < VEHICLES_PER_UNIT; loop ++)
		{
			...
			{
				for (prev = 0; prev < VEHICLES_PER_UNIT; prev ++)
				{
