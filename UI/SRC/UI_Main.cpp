...
//M.N.
int			NumberOfSkyColors;
SkyColorDataType* skycolor;
int			prevskycol;
int			weatherPattern = 0; // Default from the camp file
int			NumWeatherPatterns;
WeatherPatternDataType* weatherPatternData;
...
{
	if( _LOAD_ART_RESOURCES_)
		gMainParser->LoadImageList("resimgs.lst");
	else
		gMainParser->LoadImageList("images.lst");
}
...
//    SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_ABOVE_NORMAL);
	...
	FalconMessageFilter UIFilter(FalconEvent::UIThread,VU_DELETE_EVENT_BITS);
...
//	FalconDisplay.EnterMode(FalconDisplayConfiguration::UI);
	...
	FalconDisplay.EnterMode(FalconDisplayConfiguration::UILarge,
		DisplayOptions.DispVideoCard, DisplayOptions.DispVideoDriver);
	else 
	FalconDisplay.EnterMode(FalconDisplayConfiguration::UI,
		DisplayOptions.DispVideoCard, DisplayOptions.DispVideoDriver);
	...
	UI95_SetScreenColorInfo(static_cast<WORD>(r_mask), static_cast<WORD>(g_mask), static_cast<WORD>(b_mask));

#ifdef USE_SH_POOLS
	// Init UI Memory Pools
	UI_Pools[UI_GENERAL_POOL]=MemPoolInit(MEM_POOL_DEFAULT);
	UI_Pools[UI_CONTROL_POOL]=MemPoolInit(MEM_POOL_DEFAULT);
	UI_Pools[UI_ART_POOL]=MemPoolInit(MEM_POOL_DEFAULT);
	UI_Pools[UI_SOUND_POOL]=MemPoolInit(MEM_POOL_DEFAULT);
	dbgMemSetCheckpoint(10);
#endif
...
#if 0 // The art got screwed up... don't do this anymore
// KLUDGE CODE for TRANSITION SCREENS
	if(MainLastGroup == 4000)
	{
		if(LoadTargaFile("art\\transition\\ctranfro.tga",&cptr,&bmi))
		{
			color=(WORD*)cptr;
			count=bmi.bmiHeader.biWidth * bmi.bmiHeader.biHeight;
			count--;
			while(count >= 0)
			{
				color[count]=UI95_RGB15Bit(color[count]);
				count--;
			}
			count=bmi.bmiHeader.biWidth * bmi.bmiHeader.biHeight;
			count--;
			...
			{
				dest=(WORD*)Primary->Lock();
				for(i=0;i<Primary->targetYres();i++)
				{
					memcpy(dest,&color[(Primary->targetYres() - i -1)*Primary->targetXres()],Primary->targetXres()*sizeof(WORD));
					dest += Primary->targetXres();
				}
				Primary->Unlock();
			}
			else
			...
			MonoPrint("Failed to load transition screen\n");
	}
	else
	{
		if(LoadTargaFile("art\\transition\\mtranfro.tga",&cptr,&bmi))
		{
			color=(WORD*)cptr;
			count=bmi.bmiHeader.biWidth * bmi.bmiHeader.biHeight;
			count--;
