...
void UI_Refresher::UpdateMapItem(CampEntity entity)
{
	...
	if(entity->IsUnit())
	{
		...
		if(entity->IsFlight())
		{
			if(!entity->IsDead())
			{
				// 2002-02-21 ADDED BY S.G. 'Fog of war code'. If an enemy flight and not identified and not editing a TE, change the IconIndex of the unit to ICON_UKN and its label to 'Bandit'
				...
				if (g_bAWACSSupport) {
					...
					if(g_bAWACSFuel)
						sprintf (buffer, "%d*%s \n%03dx%d \nAlt %d \nSpd %d \nFuel %lu",
							...
							(int)(-flt->ZPos()/1000.0), (int)flt->Kias(),(long)flt->CalculateFuelAvailable(255));
					else
						sprintf (buffer, "%d*%s \n%03dx%d \nAlt %d \nSpd %d",
//						sprintf (buffer, "%d*%s@%03dx%d/A%d/V%d",
							flt->GetACCount(),  name, bearing, range, 
							(int)(-flt->ZPos()/1000.0), (int)flt->Kias());
