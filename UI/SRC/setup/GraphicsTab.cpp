...
#include "classtbl.h"
#include "Campaign\include\Cmpclass.h" // for weather UI
...
enum
{
	SKYCOLOR_BITMAP_ID = 999989987,
	WEATHER_TYPES_ID   = 999989986,
	WEATHER_PATTERN_ID = 999989985,
};
...
void SetAdvanced()
{
	...
	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FAST2DCOCKPIT);
	if(button)
	{
		if(pD3DDI->IsHardware()) button->SetFlagBitOn(C_BIT_ENABLED);
		else button->SetFlagBitOff(C_BIT_ENABLED);

		button->SetState(PlayerOptions.bFast2DCockpit ? C_STATE_1 : C_STATE_0);
	}

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_HQFILTERING);
	if(button)
	{
		if(pD3DDI->CanFilterAnisotropic() || g_bAlwaysAnisotropic) button->SetFlagBitOn(C_BIT_ENABLED);
		else button->SetFlagBitOff(C_BIT_ENABLED);
		
		button->SetState(PlayerOptions.bHQFiltering ? C_STATE_1 : C_STATE_0);
	}

#if 0	// OW
	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FORCE_DX_MULTITHREADED);
	if(button) button->SetState(PlayerOptions.bDXMultiThreaded ? C_STATE_1 : C_STATE_0);
#endif

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_ENABLE_PROCESSOR_EXTENSIONS);
	if(button) button->SetState(PlayerOptions.bAllowProcessorExtensions ? C_STATE_1 : C_STATE_0);

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FILTERED_OBJECTS);
	if(button) button->SetState(PlayerOptions.bFilteredObjects ? C_STATE_1 : C_STATE_0);

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FASTGMRADAR);
	if(button)
	{
		...
		button->SetState(PlayerOptions.bFastGMRadar ? C_STATE_1 : C_STATE_0);
	}

	lbox = (C_ListBox *) win->FindControl(SETUP_ADVANCED_FORCE_RGB_TEXTURES_MODE);
	if(lbox) lbox->SetValue(PlayerOptions.m_eTexMode);
	// OW end of new stuff
}
...
// Choose weather pattern

void SetWeatherPattern()
{
	C_Window	*win;
	C_ListBox	*lbox;
	C_Button	*btn;

	win=gMainHandler->FindWindow(SETUP_WEATHER_WIN);
	if(!win) return;
	
	lbox=(C_ListBox *)win->FindControl(SETUP_WEATHER_PATTERN);
	if (lbox)
	{
		lbox->SetValue(weatherPattern);
		lbox->Refresh();
	}

	btn=(C_Button*)win->FindControl(WEATHER_TYPES_PIC);
	if(btn)
	{
		LoadBitmap(WEATHER_TYPES_ID,btn,"WeatherTypes");
	}

	btn=(C_Button*)win->FindControl(SETUP_WEATHER_PIC);
	if(btn && weatherPatternData)
	{
		LoadBitmap(WEATHER_PATTERN_ID,btn,weatherPatternData[lbox->GetTextID()-1].picfname);
	}
}

void SelectWeatherPatternCB(long ID,short hittype,C_Base *control)
{
	C_ListBox *lbox=(C_ListBox*)control;
	...
	ID=lbox->GetTextID();
	weatherPattern = lbox->GetTextID();
	SetWeatherPattern();
}

void WeatherDefaultCB(long ID,short hittype,C_Base *control)
{
	if(hittype != C_TYPE_LMOUSEUP)
		return;
	
	weatherPattern = 0; // default = CampFileWeather

	if(control->GetGroup())	// close window
	{
...
void WeatherRandomCB(long ID,short hittype,C_Base *control)
{
	if(hittype != C_TYPE_LMOUSEUP)
		return;

	weatherPattern = -1; // random
...
void SetupGraphicsControls(void)
{
	...
	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FAST2DCOCKPIT);
	if(button) button->SetState(PlayerOptions.bFast2DCockpit ? C_STATE_1 : C_STATE_0);

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_HQFILTERING);
	if(button) button->SetState(PlayerOptions.bHQFiltering ? C_STATE_1 : C_STATE_0);
...
void GraphicsDefaultsCB(long,short hittype,C_Base *)
{
	...
	else
	{
		...
		DisplayOptions.DispDepth = 16;	// OW
	...
	if(Player.LoadOptions("default"))
	{
		...
		PlayerOptions.bFast2DCockpit = true;
		PlayerOptions.bHQFiltering = false;
		PlayerOptions.bDXMultiThreaded = false;
		PlayerOptions.bAllowProcessorExtensions = false;
		PlayerOptions.m_eTexMode = PlayerOptionsClass::TEX_MODE_LOW;
		PlayerOptions.bFilteredObjects = false;
		PlayerOptions.bFastGMRadar = false;
		// OW - End of new stuff
	}
...
// M.N. Weather Pattern
void WeatherPatternCB(long ID,short hittype, C_Base *control)
{
	if(hittype != C_TYPE_LMOUSEUP)
		return;
	C_Window *win;

	if (!TheCampaign.InMainUI)	// we have loaded a campaign/tac file. Allow weather changes only in the main UI
	{
		win=gMainHandler->FindWindow(DENY_WEATHER_SELECTION);
		if (!win) return;
