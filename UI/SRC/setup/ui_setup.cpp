...
void STPSetupControls(void)
{
	...
	if (lbox)
	{
		...
		{
			lbox->AddItem(i+1,C_TYPE_ITEM,weatherPatternData[i].name);
		}
...
/*
	...
	if(gTotalJoy)
	{
		...
		ShiAssert(FALSE == F4IsBadReadPtr(gpDIDevice[gCurController], sizeof *gpDIDevice[gCurController])); // JPO CTD
		if (gpDIDevice[gCurController])
...
static void SaveValues(void)
{
	...
	if(button) PlayerOptions.bFast2DCockpit = button->GetState() == C_STATE_1;

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_HQFILTERING);
	if(button) PlayerOptions.bHQFiltering = button->GetState() == C_STATE_1;

#if 0	// OW
	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FORCE_DX_MULTITHREADED);
	if(button) PlayerOptions.bDXMultiThreaded = button->GetState() == C_STATE_1;
#endif

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_ENABLE_PROCESSOR_EXTENSIONS);
	if(button) PlayerOptions.bAllowProcessorExtensions = button->GetState() == C_STATE_1;

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FILTERED_OBJECTS);
	if(button) PlayerOptions.bFilteredObjects = button->GetState() == C_STATE_1;

	button = (C_Button *) win->FindControl(SETUP_ADVANCED_FASTGMRADAR);
	if(button) PlayerOptions.bFastGMRadar = button->GetState() == C_STATE_1;

	lbox = (C_ListBox *) win->FindControl(SETUP_ADVANCED_FORCE_RGB_TEXTURES_MODE);
	if(lbox) *(int *) &PlayerOptions.m_eTexMode = lbox->GetTextID();
	// OW end of new stuff
	...
	if (win) {
	    lbox = (C_ListBox *) win->FindControl(SETUP_SKY_COLOR);
	    if(lbox) *(char*) &PlayerOptions.skycol = lbox->GetTextID();
	}
	skycolortime = 0;
...
// Help GUIDE thing
	...
	// M.N.
	button=(C_Button *)win->FindControl(SET_WEATHER_PATTERN);
	if(button != NULL)
	{
		button->SetCallback(WeatherPatternCB);
	}
	...
	// disable parent notification for close and cancel button
	listbox=(C_ListBox *)win->FindControl(SETUP_WEATHER_PATTERN);
	if(listbox) listbox->SetCallback(SelectWeatherPatternCB);

	button=(C_Button *)win->FindControl(CANCEL);
	if(button) button->SetCallback(CloseWindowCB);

	button=(C_Button *)win->FindControl(SET_WEATHER_DEFAULT);
	if(button) button->SetCallback(WeatherDefaultCB);

	button=(C_Button *)win->FindControl(SET_WEATHER_RANDOM);
	if(button) button->SetCallback(WeatherRandomCB);

	// M.N. end weather pattern
